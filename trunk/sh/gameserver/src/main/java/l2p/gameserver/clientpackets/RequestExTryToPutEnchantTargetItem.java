	...
	private static void doPutOld(Player activeChar, ItemInstance itemToEnchant, ItemInstance scroll)
	{
		...
		switch(itemToEnchant.getItemId())
		{
			...
			case 21580:
				...
				break;
			default:
				if (itemToEnchant.getTemplate().getType2() == ItemTemplate.TYPE2_WEAPON)
				{
					if(itemToEnchant.getEnchantLevel() >= Config.ENCHANT_WEAPON)
						fail = true;
				}
				else if (itemToEnchant.getTemplate().getType2() == ItemTemplate.TYPE2_ACCESSORY)
				{
					if(itemToEnchant.getEnchantLevel() >= Config.ENCHANT_MAX_ACCESSORY)
						fail = true;
				}
				else if (itemToEnchant.getTemplate().getType2() == ItemTemplate.TYPE2_SHIELD_ARMOR)
				{
