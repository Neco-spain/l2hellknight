...
public class SelectorThread<T extends MMOClient> extends Thread
{
	...
	private final byte[] compressedBuffer = new byte[0xFFFF];
	...
	protected void writePacket(SelectionKey key)
	{
		...
		Deflater compressor;
		if((compressor = con.getClient().getDeflater()) == null)
			prepareWriteBuffer(con);
		else if(con.isTimeToWrite())
			prepareWriteBufferDrinCompression(con, compressor);
		else
			return;
	...
	protected void prepareWriteBufferDrinCompression(MMOConnection<T> con, Deflater compressor)
	{
		DIRECT_WRITE_BUFFER.clear();
		...
		if(DIRECT_WRITE_BUFFER.remaining() > 1 && !con.hasPendingWriteBuffer())
		{
			...
			{
				WRITE_CLIENT = con.getClient();
				while((sp = sendQueue.poll()) != null)
					try
					{
						putPacketIntoWriteBuffer(con.getClient(), sp, false); // записываем пакет в WRITE_BUFFER
						WRITE_BUFFER.flip();
						if(DIRECT_WRITE_BUFFER.remaining() >= WRITE_BUFFER.limit() && DIRECT_WRITE_BUFFER.position() + WRITE_BUFFER.limit() < Config.PROTECT_COMPRESSION_SPLITSIZE)
						{
							DIRECT_WRITE_BUFFER.put(WRITE_BUFFER);
							k++;
						}
		...
		int size = DIRECT_WRITE_BUFFER.position();
		...
		if(size < Config.PROTECT_COMPRESSION_MINSIZE)
		{
			DIRECT_WRITE_BUFFER.position(0);
			while(DIRECT_WRITE_BUFFER.position() < size)
			{
				tmpSize = DIRECT_WRITE_BUFFER.getShort() & 0xFFFF;
				con.getClient().encrypt(DIRECT_WRITE_BUFFER, tmpSize - 2);
			}
			incStats(size, size);
			return;
		}

		compressor.setInput(DIRECT_WRITE_BUFFER.array(), 0, size);
		compressor.flush();
		tmpSize = compressor.deflateUnsafe(compressedBuffer);
		incStats(tmpSize + 3, size);
		//System.out.println("deflated " + size + " -> " + (tmpSize + 3)); //TODO remove debug
		DIRECT_WRITE_BUFFER.clear();
		DIRECT_WRITE_BUFFER.putShort((short) (tmpSize + 3));
		DIRECT_WRITE_BUFFER.put((byte) 0x8D);
		DIRECT_WRITE_BUFFER.put(compressedBuffer, 0, tmpSize);
		DIRECT_WRITE_BUFFER.position(2);
		con.getClient().encrypt(DIRECT_WRITE_BUFFER, tmpSize + 1);
	}
	...
	private synchronized static void incStats(int incWrited, int incWriteUncompressed)
	{
		statWrited++;
		statWritedCompressed += incWrited;
		statWriteUncompressed += incWriteUncompressed;
	}
