...
import l2rt.gameserver.communitybbs.BB.Post;
import l2rt.gameserver.communitybbs.BB.Topic;
...
public class TopicBBSManager extends BaseBBSManager
{
	...
	{
		TextBuilder htmlCode = new TextBuilder("<html><body><center>");
		...
		if(!activeChar.isLangRus())
			htmlCode.append("<a action=\"bypass _bbshome\">Home</a>&nbsp;&nbsp;&gt;&nbsp;<a action=\"bypass _bbsmemo\">Memo Form</a>&nbsp;&nbsp;&gt;&nbsp;Написать");
		else
			htmlCode.append("<a action=\"bypass _bbshome\">Главная</a>&nbsp;&nbsp;&gt;&nbsp;<a action=\"bypass _bbsmemo\">Памятка</a>&nbsp;&nbsp;&gt;&nbsp;Написать");
		...
		htmlCode.append("<td align=center FIXWIDTH=70><button value=\"&$141;\" action=\"bypass _bbsmemo\" back=\"l2ui_ct1.button.button_df_small_down\" width=70 height=25 fore=\"l2ui_ct1.button.button_df_small\"></td>");
	...
	private void ShowMemoTopics(Forum forum, L2Player activeChar, int index)
	{
		...
		htmlCode.append("<td width=755 height=30 align=left>");
		if(!activeChar.isLangRus())
			htmlCode.append("<a action=\"bypass _bbshome\">Home</a>&nbsp;&nbsp;&gt;&nbsp;Memo Form");
		else
			htmlCode.append("<a action=\"bypass _bbshome\">Главная</a>&nbsp;&nbsp;&gt;&nbsp;Памятка");
		htmlCode.append("</td></tr></table>");
		...
		for(int i = 0, j = getMaxID(forum) + 1; i < 12 * index; j--)
		{
			...
			if(t != null)
			{
				...
				if(i >= 12 * (index - 1))
				{
					...
					htmlCode.append("<td FIXWIDTH=500><a action=\"bypass _bbsposts;read;" + forum.getID() + ";" + t.getID() + "\">" + t.getName() + "</a></td><td FIXWIDTH=150 align=center></td>");
		...
		htmlCode.append("<td width=50><button value=\"&$422;\" action=\"bypass _bbsmemo\" back=\"l2ui_ct1.button.button_df_small_down\" width=70 height=25 fore=\"l2ui_ct1.button.button_df_small\"></td>");
		...
		if(index == 1)
			...
			htmlCode.append("<td><button action=\"bypass _bbstopics;read;" + forum.getID() + ";" + (index - 1) + "\" back=\"L2UI_CT1.Button_DF_Left_Down\" fore=\"L2UI_CT1.Button_DF_Left\" width=15 height=15 ></td>");
			...
			if(index <= 11)
			{
				for(i = 1; i <= (10 + index); i++)
				{
					if(i == index)
						...
						htmlCode.append("<td height=15><a action=\"bypass _bbstopics;read;" + forum.getID() + ";" + i + "\"> " + i + " </a></td>");
				}
			}
		...
		if(index == maxpage)
			...
			htmlCode.append("<tr><td><button action=\"bypass _bbstopics;read;" + forum.getID() + ";" + (index + 1) + "\" back=\"L2UI_CT1.Button_DF_Right_Down\" fore=\"L2UI_CT1.Button_DF_Right\" width=15 height=15 ></td></tr>");

		htmlCode.append("</table></td></tr></table></td>");
		htmlCode.append("<td align=right><button value=\"&$421;\" action=\"bypass _bbstopics;crea;" + forum.getID() + "\" back=\"l2ui_ct1.button.button_df_small_down\" width=70 height=25 fore=\"l2ui_ct1.button.button_df_small\" ></td></tr>");
		htmlCode.append("<tr><td width=5 height=10></td></tr>");
		htmlCode.append("<tr>");
