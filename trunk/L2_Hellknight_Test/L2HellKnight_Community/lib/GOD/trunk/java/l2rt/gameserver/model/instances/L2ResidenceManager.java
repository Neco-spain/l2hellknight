...
import l2rt.gameserver.model.entity.residence.TeleportLocation;
...
public abstract class L2ResidenceManager extends L2MerchantInstance
{
	...
	private boolean useSkill(int id, int level, L2Player player)
	{
		...
		if(ConfigSystem.getBoolean("BuffSummon") && skill.getSkillType() != SkillType.SUMMON && player.getPet() != null && !player.getPet().isDead())
	...
	private void showDepositWindowClan(L2Player player)
	{
		...
		if(!(player.isClanLeader() || (Config.ALT_ALLOW_OTHERS_WITHDRAW_FROM_CLAN_WAREHOUSE || player.getVarB("canWhWithdraw")) && (player.getClanPrivileges() & L2Clan.CP_CL_WAREHOUSE_SEARCH) == L2Clan.CP_CL_WAREHOUSE_SEARCH))
