...
import l2rt.gameserver.model.instances.L2PenaltyMonsterInstance;
import l2rt.gameserver.model.items.L2ItemInstance;
import l2rt.gameserver.network.serverpackets.ExFishingHpRegen;
import l2rt.gameserver.network.serverpackets.ExFishingStartCombat;
...
public class L2Fishing implements Runnable
{
	...
	public L2Fishing(L2Player fisher, FishData fish, boolean isNoob, boolean isUpperGrade)
	{
		...
		if(_fishAItask == null)
			_fishAItask = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(this, 1000, 1000, true);
	...
	public void doDie(boolean win)
	{
		...
		if(win)
			if(!fisher.isInVehicle() && Rnd.chance(5))
				penaltyMonster();
			else
			{
