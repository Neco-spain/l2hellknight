	...
	private CharTemplateTable()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				set.set("basePAtk", classId.isMage() ? 3 : 4);					// PTS Freya Script
				set.set("basePDef", classId.isMage() ? 54 : 80);				// PTS Freya Script
				set.set("baseMAtk", 6);											// PTS Freya Script
				set.set("baseMDef", 41);										// PTS Freya Script
				set.set("classBaseLevel", rset.getInt("lvlupgain.class_lvl"));
				set.set("basePAtkSpd", 288);									// PTS Freya Script 300
				set.set("baseMAtkSpd", classId.isMage() ? 199 : 398 );			// Not Tested
				...
				// PTS Freya Script baseAtkRange = 20!, Orcs 25!
				if (rset.getByte("char_templates.RaceId") == 3)
