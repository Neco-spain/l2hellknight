 ...
 */
package l2rt.gameserver.security;
...
import l2rt.gameserver.network.serverpackets.Ex2ndPasswordAck;
import l2rt.gameserver.network.serverpackets.Ex2ndPasswordCheck;
import l2rt.gameserver.network.serverpackets.Ex2ndPasswordVerify;
...
public class SecondaryPasswordAuth
{
	...
	public boolean checkPassword(String password, boolean skipAuth)
	{
		...
		if (!password.equals(_password))
		{
			...
			{
				LSConnection.getInstance().sendPacket(new ChangeAccessLevel(_activeClient.getLoginName(), -66, "Walker Autoban", Config.SECOND_AUTH_BAN_TIME));
				
				_log.warning(_activeClient.getLoginName()+" - ("+
						_activeClient.getIpAddr()+") has inputted the wrong password "+
