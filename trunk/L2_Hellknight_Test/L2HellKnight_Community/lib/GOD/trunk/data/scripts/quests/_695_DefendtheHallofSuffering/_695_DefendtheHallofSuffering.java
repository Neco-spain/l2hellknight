...
{
	private class UpdateTumor implements Runnable
	{
		...
		public void run()
		{
			...
			// FIXME ThreadPoolManager.getInstance().scheduleGeneral(new BuffMobs(world.tumor, 1000, 5000), 5000);
			ThreadPoolManager.getInstance().scheduleGeneral(new RunNewWaveDefence(world), waves_time[world.status]);
		}
	}
	...
	public String onKill(L2NpcInstance npc, QuestState qs)
	{
		...
		if(npcId == TUMOR_OF_DEATH)
			ThreadPoolManager.getInstance().scheduleGeneral(new RespawnTumor(world), 1 * 60 * 1000);
	...
	private void startInstance(long reflectionId)
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new UpdateTumor(world), 60000);
	}
