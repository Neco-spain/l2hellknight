...
public class _10275_ContainingTheAttributePower extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		{
			st.set("cond", "3");
			st.giveItems(YinSword, 1, L2Item.ATTRIBUTE_FIRE, 10, new int[]{0,0,0,0,0,0});
			st.playSound(SOUND_MIDDLE);
		}
		else if(event.equalsIgnoreCase("32326-03.htm"))
		{
			st.set("cond", "8");
			st.giveItems(YangSword, 1, L2Item.ATTRIBUTE_EARTH, 10, new int[]{0,0,0,0,0,0});
			st.playSound(SOUND_MIDDLE);
		}
		...
		else
		{
			...
			if(item > 0)
			{
				...
				st.addExpAndSp(202160, 20375, true);
