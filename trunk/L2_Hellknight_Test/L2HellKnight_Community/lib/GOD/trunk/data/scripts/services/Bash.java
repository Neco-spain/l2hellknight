...
public class Bash extends Functions implements IAdminCommandHandler, ScriptFile
{
	...
	public void loadData()
	{
		...
		if(data == null)
		{
			System.out.println("Service: Bash - RSS data download failed.");
			return;
		}
		...
		if(!Config.SERVICES_BASH_SKIP_DOWNLOAD)
		{
			...
			System.out.println("Service: Bash - RSS data download completed.");
		}

		int parse = parseRSS();
		if(parse == 0)
		{
			System.out.println("Service: Bash - RSS data parse error.");
			return;
		}
		System.out.println("Service: Bash - RSS data parsed: loaded " + parse + " quotes.");
	}

	public void onLoad()
	{
		System.out.println("Loaded Service: Bash [" + (Config.SERVICES_BASH_ENABLED ? "enabled]" : "disabled]"));
