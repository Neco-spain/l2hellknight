...
public class _623_TheFinestFood extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("623_3"))
		{
			...
			st.giveItems(ADENA_ID, 73000 * ConfigSystem.getInt("RateQuestsReward"));
			st.addExpAndSp(230000 * ConfigSystem.getInt("RateQuestsReward"), 18250 * ConfigSystem.getInt("RateQuestsReward"));
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if(cond == 1) // Like off C4 PTS AI (убрали  && Rnd.chance(50))
			if(npcId == HOT_SPRINGS_BUFFALO)
			{
				...
				{
					st.giveItems(BUFFALO_MEAT, ConfigSystem.getInt("RateQuestsDrop"));
					if(st.getQuestItemsCount(BUFFALO_MEAT) == 100)
					{
						if(summ(st) >= 300)
							st.set("cond", "2");
			...
			else if(npcId == HOT_SPRINGS_FLAVA)
			{
				...
				{
					st.giveItems(LEAF_OF_FLAVA, ConfigSystem.getInt("RateQuestsDrop"));
					if(st.getQuestItemsCount(LEAF_OF_FLAVA) == 100)
					{
						if(summ(st) >= 300)
							st.set("cond", "2");
				...
				{
					st.giveItems(ANTELOPE_HORN, ConfigSystem.getInt("RateQuestsDrop"));
					if(st.getQuestItemsCount(ANTELOPE_HORN) == 100)
					{
						if(summ(st) >= 300)
							st.set("cond", "2");
