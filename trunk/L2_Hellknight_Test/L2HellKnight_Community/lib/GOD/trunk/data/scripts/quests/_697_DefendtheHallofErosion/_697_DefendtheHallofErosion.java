...
import l2rt.gameserver.model.entity.SeedOfInfinity;
...
public class _697_DefendtheHallofErosion extends SeedOfInfinity implements ScriptFile
{
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == MOUTH_OF_EKIMUS)
		{
			...
			if(cond == 1 && cycle == 5)
			{
				if(checkCondition(player, ReflectionTable.SOI_HALL_OF_EROSION_DEFENCE))
				{
					...
					if(r != null)
					{
						...
						ThreadPoolManager.getInstance().scheduleGeneral(new InitialDelayTask(player), 30 * 1000);
					}
					return null;
				}
				else
	...
	public void initialInstance(L2Player player)
	{
		...
		world.remainingTimeTask = ThreadPoolManager.getInstance().scheduleGeneral(new TimeRemaining(world, commandChannel, "Hall of Erosion Defend %time% minute(s) are remaining."), 1000);
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new RunNewWaveDefence(world), 2 * 60 * 1000);
	...
	public String onKill(L2NpcInstance npc, QuestState qs)
	{
		...
		if(npcId == TUMOR_OF_DEATH)
		{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new RespawnTumor(world, npc.getLoc(), addSpawnToInstance(DESTROYED_TUMOR, npc.getLoc(), 0, world.instanceId)), 300 * 1000);
		}
		else if(Util.isInArray(npcId, monsters))
		{
			if(channel != null && checkKillProgress(npc, world))
			{
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new RunNewWaveDefence(world), 60000); // запускаем новую волну
			}
		}
		return super.onKill(npc, qs);
	}
	...
	private void spawnMobsAroundTumors(HallofErosionWorld world)
	{
		for(Location loc : tumor_death_locations)
			ThreadPoolManager.getInstance().scheduleGeneral(new RespawnTumor(world, loc, addSpawnToInstance(DESTROYED_TUMOR, loc, 0, world.instanceId)), 60 * 1000);
	}
	...
	private void spawnMobsAroundTumor(Location loc, HallofErosionWorld world)
	{
		...
		// FIXME ThreadPoolManager.getInstance().scheduleGeneral(new BuffMobs(tumor, 1000, 5000), 5000);
	...
	private class RespawnTumor implements Runnable
	{
	...
	private class RunNewWaveDefence implements Runnable
	{
		private HallofErosionWorld world;
