...
public class _1004_IceFairySirra extends Quest implements ScriptFile
{
	...
	public _1004_IceFairySirra()
	{
		...
		{
			setBusy(true);
			_respawnTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("respawn", null), remain);
		}
	...
	private static class EventTask implements Runnable
	{
		...
		public void run()
		{
			if(event.equalsIgnoreCase("start"))
			{
				closeGates();
				doSpawns();
				_partyPortTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("Party_Port", player), 2000);
				_endTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("End", player), 1802000);
				_startTask = null;
			}
			else if(event.equalsIgnoreCase("Party_Port"))
			{
				...
				_30MinutesRemainingTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("30MinutesRemaining", player), 300000);
				_partyPortTask = null;
			}
			else if(event.equalsIgnoreCase("30MinutesRemaining"))
			{
				screenMessage(player, "30 minute(s) are remaining.", 10000);
				_20MinutesRemainingTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("20MinutesRemaining", player), 600000);
			...
			else if(event.equalsIgnoreCase("20MinutesRemaining"))
			{
				...
				_10MinutesRemainingTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("10MinutesRemaining", player), 600000);
		...
		if(event.equalsIgnoreCase("check_condition"))
		{
			if(player.isInParty() && player.getParty().getPartyLeaderOID() == player.getObjectId())
			{
				if(checkItems(player))
				{
					cleanUp();
					setBusy(true);
					_startTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("start", player), 100000);
					destroyItems(player);
					st.giveItems(8379, 3);
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		if(npc.getNpcId() == 29056)
		{
			...
			_respawnTask = ThreadPoolManager.getInstance().scheduleGeneral(new EventTask("respawn", null), respawn_delay);
	...
	public void onLoad()
	{
		System.out.println("Loaded Quest: 1004: Ice Fairy Sirra");
	}
