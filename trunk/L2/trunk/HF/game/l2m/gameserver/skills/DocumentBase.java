...
import l2m.gameserver.skills.conditions.ConditionFirstEffectSuccess;
import l2m.gameserver.skills.conditions.ConditionGameTime;
import l2m.gameserver.skills.conditions.ConditionGameTime.CheckGameTime;
import l2m.gameserver.skills.conditions.ConditionHasSkill;
import l2m.gameserver.skills.conditions.ConditionLogicAnd;
import l2m.gameserver.skills.conditions.ConditionLogicNot;
import l2m.gameserver.skills.conditions.ConditionLogicOr;
import l2m.gameserver.skills.conditions.ConditionPlayerAgathion;
import l2m.gameserver.skills.conditions.ConditionPlayerClassId;
import l2m.gameserver.skills.conditions.ConditionPlayerCubic;
import l2m.gameserver.skills.conditions.ConditionPlayerHasBuff;
import l2m.gameserver.skills.conditions.ConditionPlayerHasBuffId;
import l2m.gameserver.skills.conditions.ConditionPlayerInstanceZone;
import l2m.gameserver.skills.conditions.ConditionPlayerMaxLevel;
import l2m.gameserver.skills.conditions.ConditionPlayerMaxPK;
import l2m.gameserver.skills.conditions.ConditionPlayerMinLevel;
import l2m.gameserver.skills.conditions.ConditionPlayerMinMaxDamage;
import l2m.gameserver.skills.conditions.ConditionPlayerOlympiad;
import l2m.gameserver.skills.conditions.ConditionPlayerPercentCp;
import l2m.gameserver.skills.conditions.ConditionPlayerPercentHp;
import l2m.gameserver.skills.conditions.ConditionPlayerPercentMp;
import l2m.gameserver.skills.conditions.ConditionPlayerRace;
import l2m.gameserver.skills.conditions.ConditionPlayerRiding;
import l2m.gameserver.skills.conditions.ConditionPlayerRiding.CheckPlayerRiding;
...
import l2m.gameserver.skills.conditions.ConditionPlayerSummonSiegeGolem;
import l2m.gameserver.skills.conditions.ConditionSlotItemId;
import l2m.gameserver.skills.conditions.ConditionTargetAggro;
import l2m.gameserver.skills.conditions.ConditionTargetCastleDoor;
import l2m.gameserver.skills.conditions.ConditionTargetClan;
import l2m.gameserver.skills.conditions.ConditionTargetDirection;
import l2m.gameserver.skills.conditions.ConditionTargetForbiddenClassId;
import l2m.gameserver.skills.conditions.ConditionTargetHasBuff;
import l2m.gameserver.skills.conditions.ConditionTargetHasBuffId;
import l2m.gameserver.skills.conditions.ConditionTargetHasForbiddenSkill;
import l2m.gameserver.skills.conditions.ConditionTargetMob;
import l2m.gameserver.skills.conditions.ConditionTargetMobId;
import l2m.gameserver.skills.conditions.ConditionTargetNpcClass;
import l2m.gameserver.skills.conditions.ConditionTargetPercentCp;
import l2m.gameserver.skills.conditions.ConditionTargetPercentHp;
import l2m.gameserver.skills.conditions.ConditionTargetPercentMp;
import l2m.gameserver.skills.conditions.ConditionTargetPlayable;
import l2m.gameserver.skills.conditions.ConditionTargetPlayer;
import l2m.gameserver.skills.conditions.ConditionTargetPlayerRace;
import l2m.gameserver.skills.conditions.ConditionTargetRace;
import l2m.gameserver.skills.conditions.ConditionTargetSummon;
import l2m.gameserver.skills.conditions.ConditionUsingArmor;
import l2m.gameserver.skills.conditions.ConditionUsingItemType;
import l2m.gameserver.skills.conditions.ConditionUsingSkill;
import l2m.gameserver.skills.conditions.ConditionZoneType;
...
abstract class DocumentBase
{
  ...
  protected abstract Object getTableValue(String paramString);

  protected abstract Object getTableValue(String paramString, int paramInt);

  protected void resetTable() { tables = new HashMap();
  }
  ...
  protected void attachEffect(Node n, Object template)
  {
    ...
    if (attrs.getNamedItem("time") != null)
    {
      ...
      if ((Config.SKILL_DURATION_ENABLE) && (Config.SKILL_DURATION_LIST.containsKey(((Skill)template).getId())))
      {
        if ((((Skill)template).getLevel() < 100) || (((Skill)template).getLevel() > 140))
    ...
    set.set("value", attrs.getNamedItem("val") != null ? parseNumber(attrs.getNamedItem("val").getNodeValue()).doubleValue() : 0.0D);
  ...
  protected Condition parseTargetCondition(Node n)
  {
    ...
    for (int i = 0; i < attrs.getLength(); i++)
    {
      ...
      if ("aggro".equalsIgnoreCase(nodeName)) {
        cond = joinAnd(cond, new ConditionTargetAggro(Boolean.valueOf(nodeValue).booleanValue()));
      } else if ("pvp".equalsIgnoreCase(nodeName)) {
        cond = joinAnd(cond, new ConditionTargetPlayable(Boolean.valueOf(nodeValue).booleanValue()));
      } else if ("player".equalsIgnoreCase(nodeName)) {
        cond = joinAnd(cond, new ConditionTargetPlayer(Boolean.valueOf(nodeValue).booleanValue()));
      } else if ("summon".equalsIgnoreCase(nodeName)) {
        cond = joinAnd(cond, new ConditionTargetSummon(Boolean.valueOf(nodeValue).booleanValue()));
      } else if ("mob".equalsIgnoreCase(nodeName)) {
        cond = joinAnd(cond, new ConditionTargetMob(Boolean.valueOf(nodeValue).booleanValue()));
        ...
        cond = joinAnd(cond, new ConditionTargetCastleDoor(Boolean.valueOf(nodeValue).booleanValue()));
    ...
    for (int i = 0; i < attrs.getLength(); i++)
    {
      ...
      if (("kind".equalsIgnoreCase(nodeName)) || ("weapon".equalsIgnoreCase(nodeName)))
      {
        ...
        while (st.hasMoreTokens())
        {
          ...
          WeaponTemplate.WeaponType[] arr$ = WeaponTemplate.WeaponType.VALUES; int len$ = arr$.length; for (int i$ = 0; ; i$++) { if (i$ >= len$) break label166; WeaponTemplate.WeaponType wt = arr$[i$];
            if (!wt.toString().equalsIgnoreCase(item))
              continue;
            mask |= wt.mask();
            break;
          }
          label166: ArmorTemplate.ArmorType[] arr$ = ArmorTemplate.ArmorType.VALUES; int len$ = arr$.length; for (int i$ = 0; ; i$++) { if (i$ >= len$) break label225; ArmorTemplate.ArmorType at = arr$[i$];
            if (!at.toString().equalsIgnoreCase(item))
              continue;
            mask |= at.mask();
            break;
          }
          label225: _log.error("Invalid item kind: \"" + item + "\" in " + file);
        }
        if (mask != 0L)
      ...
      else if ("armor".equalsIgnoreCase(nodeName))
      {
        ...
        cond = joinAnd(cond, new ConditionUsingSkill(Integer.parseInt(nodeValue))); } else {
  ...
  protected Condition parseHasCondition(Node n)
  {
    ...
    for (int i = 0; i < attrs.getLength(); i++)
    {
      ...
      {
        StringTokenizer st = new StringTokenizer(nodeValue, ";");
        Integer id = Integer.valueOf(parseNumber(st.nextToken().trim()).intValue());
        ...
        cond = joinAnd(cond, new ConditionFirstEffectSuccess(Boolean.valueOf(nodeValue).booleanValue()));
      }
    }
    if (cond == null)
