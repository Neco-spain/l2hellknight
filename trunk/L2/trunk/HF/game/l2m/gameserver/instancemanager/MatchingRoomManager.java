...
public class MatchingRoomManager
{
  ...
  {
    _holder[MatchingRoom.PARTY_MATCHING] = new RoomsHolder(null);
    _holder[MatchingRoom.CC_MATCHING] = new RoomsHolder(null);
  }
  ...
  public List<MatchingRoom> getMatchingRooms(int type, int region, boolean allLevels, Player activeChar)
  {
    ...
    for (MatchingRoom room : _holder[type]._rooms.values())
    {
      if (((region > 0) && (room.getLocationId() != region)) || 
        ((region == -2) && (room.getLocationId() != getInstance().getLocation(activeChar))) || (
  ...
  {
    return (MatchingRoom)_holder[type]._rooms.get(id);
  }
  ...
  private class RoomsHolder
  {
    ...
    public int addRoom(MatchingRoom r) { int val = _id++;
      _rooms.put(val, r);
      return val;
    }
  }
