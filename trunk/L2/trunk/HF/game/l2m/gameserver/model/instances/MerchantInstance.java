...
import l2m.gameserver.network.serverpackets.ExBuySellList.BuyList;
...
import l2m.gameserver.network.serverpackets.ShopPreviewList;
...
public class MerchantInstance extends NpcInstance
{
  ...
  protected void showShopWindow(Player player, int listId, boolean tax)
  {
    ...
    BuyListHolder.NpcTradeList list = BuyListHolder.getInstance().getBuyList(listId);
    if ((list == null) || (list.getNpcId() == getNpcId())) {
      player.sendPacket(new IStaticPacket[] { new ExBuySellList.BuyList(list, player, taxRate), new ExBuySellList.SellRefundList(player, false) });
    }
    else {
