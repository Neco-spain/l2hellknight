...
import l2m.gameserver.network.serverpackets.ExOlympiadUserInfo;
...
public class OlympiadTeam
{
  ...
  public void addDamage(Player player, double damage)
  {
    ...
    TeamMember member = (TeamMember)_members.get(player.getObjectId());
  ...
  public void preparePlayers()
  {
    ...
    Player leader = (Player)list.get(0);
    if (leader == null) {
      return;
    }
  ...
  {
    List players = new ArrayList(_members.size());
    for (TeamMember member : _members.values())
    {
      Player player = member.getPlayer();
      if (player != null)
  ...
  public void broadcastInfo()
  {
    for (TeamMember member : _members.values())
    {
      Player player = member.getPlayer();
      if (player != null)
        player.broadcastPacket(new L2GameServerPacket[] { new ExOlympiadUserInfo(player, player.getOlympiadSide()) });
    }
  }
