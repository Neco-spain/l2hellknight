...
import l2m.gameserver.model.GameObjectTasks.NotifyAITask;
...
import l2m.gameserver.model.WorldRegion;
import l2m.gameserver.model.Zone.ZoneType;
import l2m.gameserver.model.actor.listener.NpcListenerList;
import l2m.gameserver.model.actor.recorder.NpcStatsChangeRecorder;
...
import l2m.gameserver.network.serverpackets.AcquireSkillDone;
import l2m.gameserver.network.serverpackets.AcquireSkillList;
import l2m.gameserver.network.serverpackets.ActionFail;
import l2m.gameserver.network.serverpackets.AutoAttackStart;
import l2m.gameserver.network.serverpackets.ExChangeNpcState;
import l2m.gameserver.network.serverpackets.ExShowBaseAttributeCancelWindow;
import l2m.gameserver.network.serverpackets.ExShowVariationCancelWindow;
import l2m.gameserver.network.serverpackets.ExShowVariationMakeWindow;
...
import l2m.gameserver.taskmanager.DecayTaskManager;
...
public class NpcInstance extends Creature
{
  ...
  public static void showAcquireList(AcquireType t, Player player)
  {
    ...
    {
      for (SkillLearn s : SkillAcquireHolder.getInstance().getAvailableSkills(player, t)) {
        if (player.getInventory().getItemByItemId(s.getItemId()) != null)
          skills.add(s);
      }
    }
    else {
