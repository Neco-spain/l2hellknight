...
import l2m.gameserver.network.serverpackets.SendMacroList;
...
public class MacroList
{
  ...
  {
    return (Macro[])_macroses.values().toArray(new Macro[_macroses.size()]);
  }

  public Macro getMacro(int id)
  {
    return (Macro)_macroses.get(Integer.valueOf(id - 1));
  }
  ...
  public void registerMacro(Macro macro)
  {
    ...
    {
      Macro old = (Macro)_macroses.put(Integer.valueOf(macro.id), macro);
      if (old != null)
  ...
  {
    Macro toRemove = (Macro)_macroses.get(Integer.valueOf(id));
    if (toRemove != null)
  ...
  private void registerMacroInDb(Macro macro)
  {
    ...
    {
      _log.error(new StringBuilder().append("could not store macro: ").append(macro.toString()).toString(), e);
    }
    finally
    {
  ...
  public void restore()
  {
    ...
    try
    {
      ...
      while (rset.next())
      {
        ...
        Macro m = new Macro(id, icon, name, descr, acronym, (Macro.L2MacroCmd[])commands.toArray(new Macro.L2MacroCmd[commands.size()]));
