...
import l2m.gameserver.network.serverpackets.ExReplyWritePost;
...
public class RequestExSendPost extends L2GameClientPacket
{
  ...
  protected void runImpl()
  {
    ...
    if ((activeChar.isGM()) && (_recieverName.equalsIgnoreCase("ONLINE_ALL")))
    {
      ...
      if ((_items != null) && (_items.length > 0)) {
        for (int i = 0; i < _items.length; i++)
        {
          ...
          map.put(Integer.valueOf(item.getItemId()), Long.valueOf(_itemQ[i]));
        }
      }
    ...
    if (target != null)
    {
      int recieverId = target.getObjectId();
