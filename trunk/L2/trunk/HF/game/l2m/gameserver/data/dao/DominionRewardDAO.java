...
public class DominionRewardDAO
{
  ...
  public void select(Dominion d)
  {
    ...
    try
    {
      ...
      statement = con.prepareStatement("SELECT * FROM dominion_rewards WHERE id=?");
      ...
      while (rset.next())
      {
        ...
        siegeEvent.setReward(playerObjectId, 2, staticBadges);
        siegeEvent.setReward(playerObjectId, 0, killReward);
        siegeEvent.setReward(playerObjectId, 1, onlineReward);
      }
    }
    catch (Exception e)
    {
  ...
  public void insert(Dominion d)
  {
    ...
    {
      con = DatabaseFactory.getInstance().getConnection();
      statement = con.prepareStatement("DELETE FROM dominion_rewards WHERE id=?");
      statement.setInt(1, d.getId());
      statement.execute();

      DominionSiegeEvent siegeEvent = (DominionSiegeEvent)d.getSiegeEvent();
      Collection rewards = siegeEvent.getRewards();

      SqlBatch b = new SqlBatch("INSERT INTO dominion_rewards (id, object_id, static_badges, online_reward, kill_reward) VALUES");
      for (IntObjectMap.Entry entry : rewards)
      {
        StringBuilder sb = new StringBuilder("(");
        sb.append(d.getId()).append(",");
        sb.append(entry.getKey()).append(",");
        sb.append(((int[])entry.getValue())[2]).append(",");
        sb.append(((int[])entry.getValue())[1]).append(",");
        sb.append(((int[])entry.getValue())[0]).append(")");
        b.write(sb.toString());
      }
  ...
  public void delete(Dominion d, int objectId)
  {
    ...
    {
      con = DatabaseFactory.getInstance().getConnection();
      statement = con.prepareStatement("DELETE FROM dominion_rewards WHERE id=? AND object_id=?");
      statement.setInt(1, d.getId());
