...
public class FortressDAO
{
  ...
  public void select(Fortress fortress)
  {
    ...
    try
    {
      ...
      statement = con.prepareStatement("SELECT * FROM fortress WHERE id = ?");
  ...
  private void update0(Fortress fortress)
  {
    ...
    {
      con = DatabaseFactory.getInstance().getConnection();
      statement = con.prepareStatement("UPDATE fortress SET castle_id=?, state=?, cycle=?, reward_count=?, paid_cycle=?, supply_count=?, siege_date=?, last_siege_date=?, own_date=?, facility_0=?, facility_1=?, facility_2=?, facility_3=?, facility_4=? WHERE id=?");
      ...
      statement.setInt(15, fortress.getId());
      statement.execute();
    }
    catch (Exception e)
    {
