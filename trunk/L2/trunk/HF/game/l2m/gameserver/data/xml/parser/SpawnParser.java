...
import l2m.gameserver.data.xml.holder.SpawnHolder;
import l2m.gameserver.model.Territory;
import l2m.gameserver.templates.StatsSet;
import l2m.gameserver.templates.spawn.PeriodOfDay;
...
public final class SpawnParser extends AbstractDirParser<SpawnHolder>
{
  ...
  protected void readData(Element rootElement)
    throws Exception
  {
    Map territories = new HashMap();
    for (Iterator spawnIterator = rootElement.elementIterator(); spawnIterator.hasNext(); )
    {
      ...
      else if (spawnElement.getName().equalsIgnoreCase("spawn"))
      {
        ...
        for (Iterator subIterator = spawnElement.elementIterator(); subIterator.hasNext(); )
        {
          ...
          else if (subElement.getName().equalsIgnoreCase("territory"))
          {
            ...
            if (terName != null)
            {
              Territory g = (Territory)territories.get(terName);
              if (g == null)
              {
          ...
          else if (subElement.getName().equalsIgnoreCase("npc"))
          {
            ...
            MultiValueSet parameters = StatsSet.EMPTY;
        ...
        ((SpawnHolder)getHolder()).addSpawn(group, template);
      }
    }
  }
  ...
  private Territory parseTerritory(String name, Element e)
  {
    ...
    for (Iterator iterator = e.elementIterator("banned_territory"); iterator.hasNext(); ) {
  ...
  private Polygon parsePolygon0(String name, Element e)
  {
    ...
    for (Iterator addIterator = e.elementIterator("add"); addIterator.hasNext(); )
    {
