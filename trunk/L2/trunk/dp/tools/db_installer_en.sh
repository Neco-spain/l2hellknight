...
## DB Installer for *unix system          ##
############################################
## Writen by Avanege                      ##
## It is made for eonsw.ru                ##
## Me: http://vkontakte.ru/avanege        ##
############################################
trap finish 2
...
echo -ne "GAMESERVER DB install type: (f) full install, (s) skip or (q) quit? "
read INSTALLTYPE
case "$INSTALLTYPE" in
...
fulfinish(){
echo "Full database installed! Exit? (y/n) "
read LOGINPROMPT
case "$LOGINPROMPT" in
	"y"|"Y") finish;;
	"n"|"N") asklogin;;
