  ...
  implements IAdminCommandHandler
{
  ...
  public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
  {
    ...
    switch (1.$SwitchMap$l2p$gameserver$handler$admincommands$impl$AdminQuests$Commands[command.ordinal()])
    {
    case 1:
    ...
    case 2:
      ...
      if (_quest == null)
      {
        activeChar.sendMessage(new StringBuilder().append("Quest ").append(wordList[1]).append(" undefined").toString());
        return true;
      }
  ...
  private boolean cmd_Clear(Quest _quest, String[] wordList, Player activeChar)
  {
    ...
    if (qs == null)
    {
      activeChar.sendMessage(new StringBuilder().append("Player ").append(targetChar.getName()).append(" havn't Quest [").append(_quest.getName()).append("]").toString());
      return false;
    }
  ...
  private static boolean ShowQuestState(QuestState qs, Player activeChar)
  {
    ...
    replyMSG.append(fmtHEAD.sprintf(new Object[] { qs.getQuest().getClass().getSimpleName(), Integer.valueOf(id) }));
    replyMSG.append("<table width=260>");
    replyMSG.append(fmtRow.sprintf(new Object[] { "PLAYER: ", char_name, "" }));
    replyMSG.append(fmtRow.sprintf(new Object[] { "STATE: ", qs.getStateName(), fmtSetButton.sprintf(new Object[] { Integer.valueOf(id), "STATE", "$new_val", char_name, "" }) }));

    for (String key : vars.keySet()) {
      if (!key.equalsIgnoreCase("<state>")) {
        replyMSG.append(fmtRow.sprintf(new Object[] { new StringBuilder().append(key).append(": ").toString(), vars.get(key), fmtSetButton.sprintf(new Object[] { Integer.valueOf(id), "VAR", key, "$new_val", char_name }) }));
      }
    }

    replyMSG.append(fmtRow.sprintf(new Object[] { "<edit var=\"new_name\" width=50 height=12>", "~new var~", fmtSetButton.sprintf(new Object[] { Integer.valueOf(id), "VAR", "$new_name", "$new_val", char_name }) }));

    replyMSG.append("</table>");
    replyMSG.append(fmtFOOT.sprintf(new Object[] { Integer.valueOf(id), char_name, char_name }));
  ...
  private static boolean ShowQuestList(Player targetChar, Player activeChar)
  {
    ...
    for (QuestState qs : targetChar.getAllQuestsStates()) {
      if ((qs != null) && (qs.getQuest().getQuestIntId() != 255)) {
        replyMSG.append(fmtListRow.sprintf(new Object[] { Integer.valueOf(qs.getQuest().getQuestIntId()), targetChar.getName(), qs.getQuest().getName(), qs.getStateName() }));
      }
  ...
  private boolean cmd_Var(Quest _quest, String[] wordList, Player activeChar)
  {
    ...
    if (qs == null)
    {
      activeChar.sendMessage(new StringBuilder().append("Player ").append(targetChar.getName()).append(" havn't Quest [").append(_quest.getName()).append("], init quest by command:").toString());
  ...
  private boolean cmd_State(Quest _quest, String[] wordList, Player activeChar)
  {
    ...
    {
      activeChar.sendMessage(new StringBuilder().append("Wrong State ID: ").append(wordList[3]).toString());
      return false;
    }
    ...
    if (qs == null)
    {
      activeChar.sendMessage(new StringBuilder().append("Init Quest [").append(_quest.getName()).append("] for ").append(targetChar.getName()).toString());
  ...
  private Player getTargetChar(String[] wordList, int wordListIndex, Player activeChar)
  {
    if ((wordListIndex >= 0) && (wordList.length > wordListIndex))
    {
      ...
      if (player == null)
        activeChar.sendMessage(new StringBuilder().append("Can't find player: ").append(wordList[wordListIndex]).toString());
      return player;
    }
