...
public class ZakenManager extends GrandBossManager
{
  ...
  public void load() {
    ...
    case 0:
      ...
      break;
    case 1:
      _log.info("Zaken: live; farm delay: " + Config.ZAKEN_RESTART_DELAY / 60000L + "min.");
    }
  ...
  public void spawnBoss() {
    setState(1, 0L);
    self = ((Zaken)createOnePrivateEx(29022, Config.ZAKEN_SPAWN_LOC.x, Config.ZAKEN_SPAWN_LOC.y, Config.ZAKEN_SPAWN_LOC.z, 30000));
    self.setRunning();
  }
  ...
  public void notifyDie() {
    ...
    long offset = (Config.ZAKEN_MIN_RESPAWN + Config.ZAKEN_MAX_RESPAWN) / 2L;
    ...
    implements Runnable
  {
    ...
    {
      long delay = 0L;
      if (ZakenManager._status.respawn > 0L) {
        delay = ZakenManager._status.respawn - System.currentTimeMillis();
      }

      if (delay <= 0L)
        spawnBoss();
      else {
        ThreadPoolManager.getInstance().scheduleGeneral(new ZakenManager.SpawnBoss(ZakenManager.this), delay);
      }
      ZakenManager._status.wait = false;
    }
  }
