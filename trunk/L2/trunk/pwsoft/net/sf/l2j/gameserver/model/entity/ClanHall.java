...
public class ClanHall
{
  ...
  private void initialyzeTask(boolean forced)
  {
    ...
    if (_paidUntil > currentTime)
      ...
      if (System.currentTimeMillis() + 86400000L <= _paidUntil + 604800000L)
        ThreadPoolManager.getInstance().scheduleGeneral(new FeeTask(), System.currentTimeMillis() + 86400000L);
      else
        ThreadPoolManager.getInstance().scheduleGeneral(new FeeTask(), _paidUntil + 604800000L - System.currentTimeMillis());
    }
  ...
  private class FeeTask implements Runnable
  {
    ...
    public void run()
    {
      try {
        ...
        {
          if (_paidUntil != 0L) {
            while (_paidUntil < System.currentTimeMillis()) _paidUntil += 604800000L;
          }
          _paidUntil = (System.currentTimeMillis() + 604800000L);
        ...
        {
          _paid = false;
          if (System.currentTimeMillis() > _paidUntil + 604800000L)
          {
  ...
  public class ClanHallFunction
  {
    ...
    public void dbSave(boolean newFunction)
    {
      ...
      {
        ClanHall._log.log(Level.SEVERE, "Exception: ClanHall.updateFunctions(int type, int lvl, int lease, long rate, long time, boolean addNew): " + e.getMessage(), e);
      }
      finally
      {
        Close.CS(con, statement);
