...
{
	private final static Logger	_log	= Logger.getLogger(Network.class.getName());
	...
	public static boolean checkIfHostIsLoggedOn(L2PcInstance player)
	{
		...
		for (L2PcInstance loggedPlayersHosts : L2World.getInstance().getAllPlayers())
		{
			...
			if (clientsLoggedOn.equalsIgnoreCase(newClient))
				;
			{
				//_log.warning("Player: " + player.getName() + " logged with Host: " + newClient.toString() + " witch is already logged in !");
				loggedIn = true;
			}
		}
		return loggedIn;
