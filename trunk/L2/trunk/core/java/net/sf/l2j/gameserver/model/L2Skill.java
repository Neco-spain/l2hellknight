...
public abstract class L2Skill
{
	...
	static final HashMap<Integer, ArrayList<L2Skill>> _reuseGroups = new HashMap<Integer, ArrayList<L2Skill>>();
    ...
    protected L2Skill(StatsSet set)
    {
		...
		if(_reuseGroupId > 0)
		{
			if(_reuseGroups.get(_reuseGroupId) == null)
				_reuseGroups.put(_reuseGroupId, new ArrayList<L2Skill>());
			_reuseGroups.get(_reuseGroupId).add(this);
		}
        ...
        switch (targetType)
        {
            ...
            case TARGET_ONE:
            {
                ...
                switch(skillType)
                {
                    ...
                    case REFLECT: case UNBLEED: case UNPOISON:
                ...
                if ((target instanceof L2PcInstance && activeChar instanceof L2PcInstance) &&
                    (!target.isInsideZone(L2Character.ZONE_PVP) && !activeChar.isInsideZone(L2Character.ZONE_PVP)))
                {
                   if(((activeChar.getParty() != null && target.getParty() != null) &&
                        activeChar.getParty().getPartyLeaderOID() == target.getParty().getPartyLeaderOID()) ||
                        ((((L2PcInstance)activeChar).getClanId() != 0 && ((L2PcInstance)target).getClanId() != 0) &&
                        (((L2PcInstance)activeChar).getClanId() == ((L2PcInstance)target).getClanId())))
				        {
				      	if(isPvpSkill())
				     	{
				    		activeChar.sendPacket(new SystemMessage(SystemMessageId.TARGET_IS_INCORRECT));
                ...
                if (activeChar instanceof L2PcInstance)
                {
                    ...
                    if (clan != null)
                    {
                        ...
                        for (L2Object newTarget : activeChar.getKnownList().getKnownObjects().values())
                        {
                            ...
                            if (targetType == SkillTargetType.TARGET_ALLY && (((L2PcInstance) newTarget).isDead() || ((L2PcInstance) newTarget).getCurrentHp() == 0))
                            	continue;
