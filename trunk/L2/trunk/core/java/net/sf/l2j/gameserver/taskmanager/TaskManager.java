...
import static net.sf.l2j.gameserver.taskmanager.TaskTypes.TYPE_EVENT;
...
public final class TaskManager
{
    ...
    private boolean launchTask(ExecutedTask task)
    {
		...
		{
			long l3 = Long.valueOf(task.getParams()[0]).longValue() * 0x5265c00L;
			String as1[] = task.getParams()[1].split(":");
			if(as1.length != 3)
			{
				_log.warning((new StringBuilder()).append("Task ").append(task.getId()).append(" has incorrect parameters").toString());
				return false;
			}
			...
			{
				_log.warning((new StringBuilder()).append("Bad parameter on task ").append(task.getId()).append(": ").append(exception2.getMessage()).toString());
				return false;
			}
