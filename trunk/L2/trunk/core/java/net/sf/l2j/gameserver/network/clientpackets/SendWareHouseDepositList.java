...
public final class SendWareHouseDepositList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
    	...
    	if (!FloodProtector.getInstance().tryPerformAction(player.getObjectId(), FloodProtector.PROTECTED_WH))
    	{
    		return;
    	}
		...
		if (player.getActiveEnchantItem() != null)
        {
        	...
        	//Util.handleIllegalPlayerAction(player,"Player "+player.getName()+" tried to use Warehouse enchant Exploit!", Config.DEFAULT_PUNISH);
        	return;
        }
