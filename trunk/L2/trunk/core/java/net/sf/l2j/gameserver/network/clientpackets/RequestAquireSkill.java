...
public class RequestAquireSkill extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if (_skillType == 2) //pledgeskills TODO: Find appropriate system messages.
        {
    		...
    		{
    			L2LearnSkill[] shopskills = SkillTreeTable.getInstance().getAvailableShopSkills(player);
    			for (L2LearnSkill s : shopskills)
    			{
    				L2Skill sk = SkillTable.getInstance().getInfo(s.getId(), s.getLevel());
    			...
    			if (counts == 0 && !Config.ALT_GAME_SKILL_LEARN)
    			{
    				...
    				if (Config.SP_BOOK_NEEDED)
    				{
    					...
    					int spbId2 = 10001;
    					int costid = 4037;
    					int costcount = L2LearnSkill.getCostCount(costid);
    					
    					if (skill.getLevel() == 0 && spbId > -1 && spbId2 > -1)
    					{
    						L2ItemInstance spb = player.getInventory().getItemByItemId(spbId);
    						L2ItemInstance spb2 = player.getInventory().getItemByItemId(spbId2);
    						
    						if (spb == null || spb2 == null)
    						{
    							// Haven't spellbook
    							player.sendPacket(new SystemMessage(SystemMessageId.ITEM_MISSING_TO_LEARN_SKILL));
    							return;
    						...
    						// ok
    						player.destroyItem("Consume", spb.getObjectId(), 1, trainer, false);
    						player.destroyItem("Consume", spb2.getObjectId(), 1, trainer, false);
    						player.destroyItem("Consume", costid, costcount, trainer, false);
    					}
    				}
