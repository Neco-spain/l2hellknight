...
public class ScrollOfEscape implements IItemHandler
{
    ...
    public void useItem(L2PlayableInstance playable, L2ItemInstance item)
    {
		...
		if (Olympiad.getInstance().isRegisteredInComp(activeChar) || activeChar.getOlympiadGameId() > 0)
		{
		 ...
		 if (activeChar.isMovementDisabled() || /*activeChar.isMuted()*/  activeChar.isAlikeDead() || activeChar.isAllSkillsDisabled() || activeChar.isStunned() || activeChar.isSleeping() || activeChar.isParalyzed() || activeChar.isTeleporting())
		    return;
	...
	if (activeChar.isAfraid())
	{
        ...
        {
            activeChar.sendPacket(SystemMessage.sendString("«десь нельз€ использовать свиток телепорта!"));
            return;
        }

        if (activeChar.isInDuel())
        {
        	activeChar.sendPacket(SystemMessage.sendString("Ќельз€ использовать свиток телепорта по врем€ поединка."));
            return;
        }
