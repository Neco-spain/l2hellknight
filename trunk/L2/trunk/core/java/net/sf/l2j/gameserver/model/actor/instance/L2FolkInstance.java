...
public class L2FolkInstance extends L2NpcInstance
{
     ...
     * @param player
     */
	  public void showShopSkillList(L2PcInstance player, ClassId classId)
	  {
	    if (Config.DEBUG) 
	    {
	      _log.fine("ShopSkillList activated on: " + getObjectId());
	    }
	    
	    int npcId = getTemplate().npcId;
	    
	    L2LearnSkill[] skills = SkillTreeTable.getInstance().getAvailableShopSkills(player);
	    AquireSkillList asl = new AquireSkillList(AquireSkillList.skillType.Usual);
	    ...
	    for (L2LearnSkill s: skills)
	    {
	      ...
	      counts++;
	      asl.addSkill(s.getId(), s.getLevel(), s.getLevel(), s.getSpCost(), s.getCostId());
	    }
	    if (counts == 0)
	    {
	      int minlevel = SkillTreeTable.getInstance().getMinLevelForNewShopSkill(player);
	      if (minlevel > 0)
	      {
	        SystemMessage sm = new SystemMessage(SystemMessageId.DO_NOT_HAVE_FURTHER_SKILLS_TO_LEARN);
	        sm.addNumber(minlevel);
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
	    ...
	    else if (command.startsWith("ShopSkillList")) 
	    {
			showShopSkillList(player, player.getClassId());
	    }
