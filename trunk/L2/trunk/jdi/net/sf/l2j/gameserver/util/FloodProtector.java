...
public class FloodProtector
{
  ...
  private static final int[] REUSEDELAY = { 0, 42, 42, 16, 100, 5, Config.SUBCLASS_PROTECT * 10, Config.PVPPK_PROTECT * 10, 100, 50, Config.TRADE_FLOOD_TIME * 10, 15, 10, Config.SHOUT_FLOOD_TIME * 10, Config.WH_FLOOD_TIME * 10, Config.ENCHANT_FLOOD_TIME * 10, 100 };
  ...
  public void registerNewPlayer(int playerObjId)
  {
    ...
    _floodClient.put(Integer.valueOf(playerObjId), array);
  }

  public void removePlayer(int playerObjId)
  {
    _floodClient.remove(Integer.valueOf(playerObjId));
  }
  ...
  {
    FastMap.Entry entry = _floodClient.getEntry(Integer.valueOf(playerObjId));
    if (entry == null)
      return false;
    Integer[] value = (Integer[])entry.getValue();

    if (value[action].intValue() < GameTimeController.getGameTicks())
    {
      value[action] = Integer.valueOf(GameTimeController.getGameTicks() + REUSEDELAY[action]);
