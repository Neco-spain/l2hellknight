    ...
    public void run()
    {
      try
      {
        ...
        for (L2NpcInstance npcInst : spawnInst.getNPCInstanceList())
        {
          ...
          if (Config.DEBUG)
            AutoSpawnHandler._log.info("AutoSpawnHandler: Spawns removed for spawn instance (Object ID = " + _objectId + ").");
        }
      }
      catch (Exception e) {
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      try
      {
        ...
        {
          AutoSpawnHandler._log.info("AutoSpawnHandler: No location co-ords specified for spawn instance (Object ID = " + _objectId + ").");
        ...
        if (Config.DEBUG) {
          AutoSpawnHandler._log.info("AutoSpawnHandler: Spawned NPC ID " + spawnInst.getNpcId() + " at " + x + ", " + y + ", " + z + " (Near " + nearestTown + ") for " + spawnInst.getRespawnDelay() / 60000 + " minute(s).");
        }
