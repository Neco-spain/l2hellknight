  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar)
  {
    ...
    if (command.startsWith("admin_cw_info"))
    {
      if (!command.contains("menu"))
      {
        ...
        {
          activeChar.sendMessage(new StringBuilder().append("> ").append(cw.getName()).append(" (").append(cw.getItemId()).append(")").toString());
          if (cw.isActivated())
          {
            L2PcInstance pl = cw.getPlayer();
            activeChar.sendMessage(new StringBuilder().append("  Player holding: ").append(pl).toString() == null ? "null" : pl.getName());
            activeChar.sendMessage(new StringBuilder().append("    Player karma: ").append(cw.getPlayerKarma()).toString());
            activeChar.sendMessage(new StringBuilder().append("    Time Remaining: ").append(cw.getTimeLeft() / 60000L).append(" min.").toString());
            activeChar.sendMessage(new StringBuilder().append("    Kills : ").append(cw.getNbKills()).toString());
          }
