  ...
  implements SpawnListener
{
  ...
  protected void restoreFestivalData()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        query = query + "accumulated_bonus" + String.valueOf(i) + ", ";
      }query = query + "accumulated_bonus" + String.valueOf(4) + " ";
      query = query + "FROM seven_signs_status WHERE id=0";
    ...
    try
    {
      ...
      for (Map currCycleData : _festivalData.values())
      {
        for (StatsSet festivalDat : currCycleData.values())
        {
          ...
          statement.setLong(4, Long.valueOf(festivalDat.getString("date")).longValue());
  ...
  private class L2DarknessFestival
  {
    ...
    protected L2DarknessFestival(int cabal, int levelRange)
    {
      ...
      if (cabal == 2)
      {
        ...
        _witchSpawn = new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, SevenSignsFestival.FESTIVAL_DAWN_WITCH_SPAWNS[levelRange]);
        _startLocation = new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, SevenSignsFestival.FESTIVAL_DAWN_PLAYER_SPAWNS[levelRange]);
      }
      else
      {
        _participants = ((List)_duskFestivalParticipants.get(Integer.valueOf(levelRange)));
        _witchSpawn = new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, SevenSignsFestival.FESTIVAL_DUSK_WITCH_SPAWNS[levelRange]);
        _startLocation = new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, SevenSignsFestival.FESTIVAL_DUSK_PLAYER_SPAWNS[levelRange]);
      }
    ...
    protected void festivalInit()
    {
      if (_participants.size() > 0) {
        try
        {
          ...
          {
            _originalLocations.put(participant, new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, participant.getX(), participant.getY(), participant.getZ(), participant.getHeading()));
    ...
    protected void spawnFestivalMonsters(int respawnDelay, int spawnType) {
      ...
      {
        SevenSignsFestival.FestivalSpawn currSpawn = new SevenSignsFestival.FestivalSpawn(SevenSignsFestival.this, _npcSpawns[i]);
