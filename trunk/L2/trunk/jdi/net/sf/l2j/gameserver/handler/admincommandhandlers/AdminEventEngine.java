  ...
  implements IAdminCommandHandler
{
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    ...
    else if (command.startsWith("admin_event_name"))
    {
      ...
      try {
        ...
        showNewEventPage(activeChar); } catch (Exception e) {
        tempBuffer = "";
      }
    }
    ...
    else if (command.startsWith("admin_event_control_begin"))
    {
      try
      {
        ...
        for (int j = 0; j < L2Event.teamsNumber; j++) {
          ...
          L2Event.players.put(Integer.valueOf(j + 1), link);
        }
        ...
        while (L2Event.participatingPlayers.size() > 0) {
          ...
          if (!target.equals(""))
          {
            ((LinkedList)L2Event.players.get(Integer.valueOf(i + 1))).add(target);
    ...
    else
    {
      String type;
      String id;
      if (command.startsWith("admin_event_control_prize"))
      {
      ...
      else if (command.startsWith("admin_event_announce"))
      {
        ...
        while (st.hasMoreElements()) {
          ...
          if (!temp2.equals(" ")) {
            ...
            type = i; id = i = Integer.valueOf(i.intValue() + 1);
          }
        }
  ...
  void muestraNpcConInfoAPlayers(L2PcInstance activeChar, int id) {
    ...
    temp.clear();
    for (Iterator i$ = L2World.getInstance().getAllPlayers().iterator(); i$.hasNext(); ) { player = (L2PcInstance)i$.next();
  ...
  String getMaxLeveledPlayer()
  {
    ...
    while (it.hasNext()) try {
        ...
        try { L2Event.participatingPlayers.remove(tempName2); } catch (Exception e2) {
        }
  ...
  void destroyEventNpcs()
  {
    while (L2Event.npcs.size() > 0) {
      try {
        L2NpcInstance npc = (L2NpcInstance)L2World.getInstance().findObject(Integer.parseInt((String)L2Event.npcs.getFirst()));
        ...
        npc.deleteMe();
        L2Event.npcs.removeFirst(); } catch (Exception e) {  }
      L2Event.npcs.removeFirst();
    }
  }

  void teleportTeam(L2PcInstance activeChar, int team) {
    LinkedList linked = (LinkedList)L2Event.players.get(Integer.valueOf(team));
    Iterator it = linked.iterator();
    while (it.hasNext()) try {
        L2PcInstance pc = L2World.getInstance().getPlayer(it.next().toString());
        pc.setTitle((String)L2Event.names.get(Integer.valueOf(team)));
  ...
  void sitTeam(int team) {
    ...
    while (it.hasNext()) try {
        ...
        if (pc.eventSitForced) pc.sitDown(); else
          pc.standUp();
      } catch (Exception e) {
      } 
  }
  ...
  void regardTeam(L2PcInstance activeChar, int team, int n, int id, String type) {
    ...
    while (it.hasNext()) try {
        ...
        else if (type.equalsIgnoreCase("kills")) temp = n * target.kills.size(); else
          temp = n;
