...
public class Announcements
{
  ...
  public void showAnnouncements(L2PcInstance activeChar)
  {
    ...
    {
      CreatureSay cs = new CreatureSay(0, 10, activeChar.getName(), (String)_announcements.get(i));
      activeChar.sendPacket(cs);
    }

    for (int i = 0; i < _eventAnnouncements.size(); i++)
    {
      List entry = (List)_eventAnnouncements.get(i);

      DateRange validDateRange = (DateRange)entry.get(0);
      String[] msg = (String[])(String[])entry.get(1);
  ...
  public void listAnnouncements(L2PcInstance activeChar)
  {
    ...
    for (int i = 0; i < _announcements.size(); i++)
    {
      replyMSG.append("<table width=260><tr><td width=220>" + (String)_announcements.get(i) + "</td><td width=40>");
  ...
  private void saveToDisk()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < _announcements.size(); i++)
      {
        save.write((String)_announcements.get(i));
        save.write("\r\n");
      }
      save.flush();
      save.close();
