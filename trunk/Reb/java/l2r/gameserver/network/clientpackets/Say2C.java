...
public class Say2C extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(_text.startsWith(".") && !Config.NOT_USE_USER_VOICED) // Если доступны войс комманды по конфигу, обрабатываем их
		{
		...
		/*
		else if(_text.startsWith(".") && _text.endsWith("hellbound") && Config.NOT_USE_USER_VOICED) // Если войс комманды не доступны, обрабатываем только .hellbound
		{
			String fullcmd = _text.substring(1).trim();
			String command = fullcmd.split("\\s+")[0];
			String args = fullcmd.substring(command.length()).trim();
		...
		*/
		...
		else if(_text.startsWith(".") && _text.endsWith("offline") && Config.SERVICES_OFFLINE_TRADE_ALLOW && Config.NOT_USE_USER_VOICED) // Если войс комманды не доступны, но включен оффлайн трей, обрабатываем его
		{
			String fullcmd = _text.substring(1).trim();
			String command = fullcmd.split("\\s+")[0];
			String args = fullcmd.substring(command.length()).trim();
