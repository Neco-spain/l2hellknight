	...
	protected void runImpl()
	{
		...
		try
		{
			loop: for(int i = 0; i < _count; i++)
			{
				...
				{
					buyer.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.SetPrivateStoreBuyList.TooLowPrice", buyer).addItemName(item).addNumber(item.getReferencePrice() / 2));
					continue;
				}
