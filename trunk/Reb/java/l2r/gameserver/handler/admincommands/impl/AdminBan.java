				...
				case admin_chatban:
				{
					try
					{
						...
						if(AutoBan.ChatBan(player, period, msg, activeChar.getName()))
						{
							...
							Announcements.getInstance().announceToAll("На персонажа " + player + " наложен кляп длительностью " + period + " минут.");

							Player p = World.getPlayer(player);
							if(p != null) // чар есть в мире
							{
								p.sendPacket(new Say2(activeChar.getObjectId(), ChatType.TELL, "->" + p.getName(), "На вам наложен кляп длительностью " + TimeUtils.minutesToFullString(period) + ", причина: " + msg));
							}
						}
						else
						{
