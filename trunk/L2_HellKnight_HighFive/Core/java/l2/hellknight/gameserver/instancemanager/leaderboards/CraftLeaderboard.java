...
public class CraftLeaderboard
{
	...
	protected int					TASK_DELAY			= ExternalConfig.RANK_CRAFT_INTERVAL;
	...
	public void formRank()
	{
		...
		Announcements.getInstance().announceToAll("Attention Players: " + arTop.name + " is the winner for this time with " + arTop.sucess + "/" + arTop.fail + ". Next calculation in " + ExternalConfig.RANK_CRAFT_INTERVAL + " min(s).");
		if (winner != null && ExternalConfig.RANK_CRAFT_REWARD_ID > 0 && ExternalConfig.RANK_CRAFT_REWARD_COUNT > 0)
		{
			winner.getInventory().addItem("CraftManager", ExternalConfig.RANK_CRAFT_REWARD_ID, ExternalConfig.RANK_CRAFT_REWARD_COUNT, winner, null);
			if (ExternalConfig.RANK_CRAFT_REWARD_COUNT > 1) //You have earned $s1.
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(ExternalConfig.RANK_CRAFT_REWARD_ID).addNumber(
						ExternalConfig.RANK_CRAFT_REWARD_COUNT));
			else
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1).addItemName(ExternalConfig.RANK_CRAFT_REWARD_ID));
	...
	public String showHtm(int owner)
	{
		...
		pt += "</table>";
		pt += "<br><br>";
		if (ExternalConfig.RANK_CRAFT_REWARD_ID > 0 && ExternalConfig.RANK_CRAFT_REWARD_COUNT > 0)
		{
			pt += "Next Reward Time in <font color=\"LEVEL\">" + calcMinTo() + " min(s)</font><br1>";
			pt += "<font color=\"aadd77\">" + ExternalConfig.RANK_CRAFT_REWARD_COUNT + " &#" + ExternalConfig.RANK_CRAFT_REWARD_ID + ";</font>";
		}
