...
import l2.hellknight.gameserver.Territory;
...
public class L2Spawn
{
	...
	/** The X random position from the spawn point */
	private int _randomX;
	
	/** The Y random position from the spawn point */
	...
	public L2Spawn(L2NpcTemplate mobTemplate) throws SecurityException, ClassNotFoundException, NoSuchMethodException
	{
		...
		Class<?>[] parameters = {int.class, Class.forName("l2.hellknight.gameserver.model.actor.templates.L2NpcTemplate")};
		_constructor = Class.forName("l2.hellknight.gameserver.model.actor.instance." + _template.getType() + "Instance").getConstructor(parameters);
	}
	...
	/**
	 * Set the random X spawn from the spawn point.<BR><BR>
	 */
	...
	/**
	 * Set the random Y spawn from the spawn point.<BR><BR>
	 */
	...
	private L2Npc initializeNpcInstance(L2Npc mob)
	{
		...
		{
			if (ExternalConfig.ENABLE_RANDOM_SPAWN_MOB && ExternalConfig.USE_RANDOMXY_FROM_DATABASE && (mob instanceof L2MonsterInstance) && (!getTemplate().isQuestMonster()) && (!mob.isRaid()) && (!((L2MonsterInstance)mob).isRaidMinion()))
			{
				newlocx = getLocx() + Rnd.get(-getRandomX(), getRandomX());
				newlocy = getLocy() + Rnd.get(-getRandomY(), getRandomY());
			}
			else if (ExternalConfig.ENABLE_RANDOM_SPAWN_MOB && (mob instanceof L2MonsterInstance) && (!getTemplate().isQuestMonster()) && (!mob.isRaid()) && (!((L2MonsterInstance)mob).isRaidMinion()))
			{
				newlocx = getLocx() + Rnd.get(ExternalConfig.MIN_SPAWN_MOB,ExternalConfig.MAX_SPAWN_MOB);
				newlocy = getLocy() + Rnd.get(ExternalConfig.MIN_SPAWN_MOB,ExternalConfig.MAX_SPAWN_MOB);
			}
			else
			{
				newlocx = getLocx();
