...
import l2.hellknight.gameserver.custom.AutoVoteRewardHandler;
import l2.hellknight.gameserver.custom.DatabaseBackupManager;
import l2.hellknight.gameserver.custom.DonateEngine;
import l2.hellknight.gameserver.datatables.AdminTable;
import l2.hellknight.gameserver.datatables.ArmorSetsData;
...
import l2.hellknight.gameserver.datatables.EnchantItemData;
...
import l2.hellknight.gameserver.datatables.FishingMonstersData;
...
import l2.hellknight.gameserver.datatables.InitialEquipmentData;
...
import l2.hellknight.gameserver.datatables.NpcWalkerRoutesData;
...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		{
			_log.info("Server Version:    " + Config.SERVER_VERSION);
		}
		...
		if (ExternalConfig.VOTESYSTEMENABLE)
		{
		...
		if (ExternalConfig.DATABASE_BACKUP_MAKE_BACKUP_ON_STARTUP)
		...
		if (ExternalConfig.RANK_ARENA_ENABLED)
			ArenaLeaderboard.getInstance();
		
		if (ExternalConfig.RANK_FISHERMAN_ENABLED)
			FishermanLeaderboard.getInstance();
		
		if (ExternalConfig.RANK_CRAFT_ENABLED)
			CraftLeaderboard.getInstance();
		
		if (ExternalConfig.RANK_TVT_ENABLED)
		...
		printSection("DonateEngine");
		DonateEngine.getInstance();
