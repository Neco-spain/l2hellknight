...
import static l2.hellknight.gameserver.util.Util.formatAdena;
...
import l2.hellknight.gameserver.model.actor.stat.SummonStat;
import l2.hellknight.gameserver.model.actor.status.PcStatus;
import l2.hellknight.gameserver.model.actor.status.SummonStatus;
...
import l2.hellknight.gameserver.network.serverpackets.SetSummonRemainTime;
...
public class Buffer extends Quest
{
    ...
    private static final boolean SCRIPT_RELOAD = ExternalConfig.NpcBuffer_Reload;
    private static final boolean SMART_WINDOW = ExternalConfig.NpcBuffer_SmartWindow;
    private static final boolean ENABLE_VIP_BUFFER = ExternalConfig.NpcBuffer_VIP;
    private static final int VIP_ACCESS_LEVEL = ExternalConfig.NpcBuffer_VIP_ALV;
    private static final boolean ENABLE_BUFF_SECTION = ExternalConfig.NpcBuffer_EnableBuff;
    private static final boolean ENABLE_SCHEME_SYSTEM = ExternalConfig.NpcBuffer_EnableScheme;
    private static final boolean ENABLE_HEAL = ExternalConfig.NpcBuffer_EnableHeal;
    private static final boolean ENABLE_BUFFS = ExternalConfig.NpcBuffer_EnableBuffs;
    private static final boolean ENABLE_RESIST = ExternalConfig.NpcBuffer_EnableResist;
    private static final boolean ENABLE_SONGS = ExternalConfig.NpcBuffer_EnableSong;
    private static final boolean ENABLE_DANCES = ExternalConfig.NpcBuffer_EnableDance;
    private static final boolean ENABLE_CHANTS = ExternalConfig.NpcBuffer_EnableChant;
    private static final boolean ENABLE_OTHERS = ExternalConfig.NpcBuffer_EnableOther;
    private static final boolean ENABLE_SPECIAL = ExternalConfig.NpcBuffer_EnableSpecial;
    private static final boolean ENABLE_CUBIC = ExternalConfig.NpcBuffer_EnableCubic;
    private static final boolean ENABLE_BUFF_REMOVE = ExternalConfig.NpcBuffer_EnableCancel;
    private static final boolean ENABLE_BUFF_SET = ExternalConfig.NpcBuffer_EnableBuffSet;
    private static final boolean BUFF_WITH_KARMA = ExternalConfig.NpcBuffer_EnableBuffPK;
    private static final boolean FREE_BUFFS = ExternalConfig.NpcBuffer_EnableFreeBuffs;
    private static final boolean TIME_OUT = ExternalConfig.NpcBuffer_EnableTimeOut;
    private static final int TIME_OUT_TIME = ExternalConfig.NpcBuffer_TimeOutTime;
    private static final int MIN_LEVEL = ExternalConfig.NpcBuffer_MinLevel;
    private static final int BUFF_REMOVE_PRICE = ExternalConfig.NpcBuffer_PriceCancel;
    private static final int HEAL_PRICE = ExternalConfig.NpcBuffer_PriceHeal;
    private static final int BUFF_PRICE = ExternalConfig.NpcBuffer_PriceBuffs;
    private static final int RESIST_PRICE = ExternalConfig.NpcBuffer_PriceResist;
    private static final int SONG_PRICE = ExternalConfig.NpcBuffer_PriceSong;
    private static final int DANCE_PRICE = ExternalConfig.NpcBuffer_PriceDance;
    private static final int CHANT_PRICE = ExternalConfig.NpcBuffer_PriceChant;
    private static final int OTHERS_PRICE = ExternalConfig.NpcBuffer_PriceOther;
    private static final int SPECIAL_PRICE = ExternalConfig.NpcBuffer_PriceSpecial;
    private static final int CUBIC_PRICE = ExternalConfig.NpcBuffer_PriceCubic;
    private static final int BUFF_SET_PRICE = ExternalConfig.NpcBuffer_PriceSet;
    private static final int SCHEME_BUFF_PRICE = ExternalConfig.NpcBuffer_PriceScheme;
    private static final int SCHEMES_PER_PLAYER = ExternalConfig.NpcBuffer_MaxScheme;
    private static final int CONSUMABLE_ID = ExternalConfig.NpcBuffer_consumableID;
    ...
    private String rebuildMainHtml(QuestState st)
    {
        ...
        {
            bottonA = "Auto Buff"; bottonB = "Heal Me"; bottonC = "Remove Buffs";
        ...
        if (MESSAGE.length() > 0)
        {
            MAIN_HTML_MESSAGE += "<BR1><table width=100% border=0 cellspacing=0 cellpadding=1 bgcolor=444444><tr>"
                              +  "<td><font color=00FFFF>Buffs:</font></td><td align=right><font color=LEVEL>" + formatAdena(BUFF_PRICE) + "</font> adena</td></tr></table>"
    ...
    private String generateScheme(QuestState st)
    {
        ...
        if (schemeName.size() > 0)
            HTML += "<td><button value=\"Edit\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " edit_1 x x x\" width=85 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>"
                 + "<td><button value=\"Delete\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " delete_1 x x x\" width=85 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>";
        else
            HTML += "</tr></table>";
        return HTML;
    }
    ...
    private Buffer(int id, String name, String descr)
    {
        super(id, name, descr);
        addStartNpc(NPC_ID);
    ...
    private String viewAllBuffs(String type, String typeName, String page)
    {
      ...
      //}
        ...
        typeName = typeName.replace(" ", "_");
        for (int ii = 1; ii <= pc; ++ii)
            if (ii == Integer.parseInt(page)) HTML_MESSAGE += "<td width=" + width + " align=center><font color=LEVEL>" + pageName + ii + "</font></td>";
            else
            ...
            case "removeBuffs":
                if ((int)(System.currentTimeMillis()/1000) > st.getInt("blockUntilTime"))
                {
                    ...
                    else
                    {
                        ...
                        if (getpetbuff)
                        {
                            if (player.getPet() != null) player.getPet().stopAllEffects();
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
    {
        ...
        {
            if (!ENABLE_VIP_BUFFER && player.getAccessLevel().getLevel() != VIP_ACCESS_LEVEL) 
            {
    ...
    {
        new Buffer(-1, QUEST_LOADING_INFO, "custom");
    }
}
