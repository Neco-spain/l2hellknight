			...
			if (!((L2Npc) target).getTemplate().getDropData().isEmpty())
			{
				...
				for (L2DropCategory cat : ((L2Npc)target).getTemplate().getDropData())
				{
					for (L2DropData drop : cat.getAllDrops())
					{
						...
						double chance = (((double)drop.getChance()/10000)*Config.RATE_DROP_ITEMS); //get chance of drop in shift+click on mob
						StringUtil.append(html1,
								"<tr>",
								...
								"<td>", String.valueOf(chance),  "%" + "</td>", //show chance of drop in shift+click on mob
								"</tr>"
						);
					}
				}
