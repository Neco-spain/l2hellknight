...
public class StatManager implements ScriptFile, ICommunityBoardHandler {
    ...
    private void showClan(Player player) {
        ...
        try {
            con = DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement("SELECT clan_subpledges.name,clan_data.clan_level,clan_data.reputation_score,clan_data.hasCastle,ally_data.ally_name FROM clan_data LEFT JOIN ally_data ON clan_data.ally_id = ally_data.ally_id, clan_subpledges WHERE clan_data.clan_level>0 AND clan_subpledges.clan_id=clan_data.clan_id AND clan_subpledges.type='0' order by clan_data.clan_level desc limit 10;");
            rs = statement.executeQuery();
            ...
            while (rs.next()) {
                ...
                if (tp.AllyName != null)
                    ...
                    html.append("<td width=140>Нет альянса</td>");
