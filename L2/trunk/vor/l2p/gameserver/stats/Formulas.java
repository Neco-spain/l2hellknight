...
public class Formulas
{
  ...
  public static double calcMagicDam(Creature attacker, Creature target, Skill skill, int sps)
  {
    ...
    if ((shield) && (Config.MAGIC_SHIELD_BLOCK))
    {
      if (Rnd.chance(Config.MAGIC_FULL_SHIELD_BLOCK))
      {
        damage = 0.0D;
        target.sendPacket(SystemMsg.YOUR_EXCELLENT_SHIELD_DEFENSE_WAS_A_SUCCESS);
    ...
    if ((Rnd.chance(failChance)) && (Config.MAGICFAILURES))
    {
      if (levelDiff > Config.MAGICFAILURES_DIFF)
      {
        damage = 0.0D;
  ...
  public static boolean calcBlow(Creature activeChar, Creature target, Skill skill)
  {
    ...
    if ((Config.SKILLS_SHOW_BLOW_CHANCE) && (chance > 0.0D)) {
      activeChar.sendMessage(new StringBuilder().append("Blow chance is: ").append(chance).toString());
    }
  ...
  public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
  {
    ...
    if (Config.SKILLS_SHOW_CHANCE) {
      caster.getPlayer().sendMessage(new StringBuilder().append(env.skill.toString()).append(" chance is: ").append(value).append("%").toString());
    }
    if (debugGlobal)
    {
