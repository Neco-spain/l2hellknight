...
public class FuncEnchant extends Func
{
  ...
  public void calc(Env env)
  {
    ...
    Player p = (Player)env.character;
    if ((p.isInOlympiadMode()) && (Config.OLY_ENCH_LIMIT_ENABLE))
    {
      if (env.item.isWeapon())
        overenchant = Math.max(0, Config.OLY_ENCHANT_LIMIT_WEAPON - 3);
      else if (env.item.isArmor())
        overenchant = Math.max(0, Config.OLY_ENCHANT_LIMIT_ARMOR - 3);
      else if (env.item.isAccessory())
        overenchant = Math.max(0, Config.OLY_ENCHANT_LIMIT_JEWEL - 3);
    }
