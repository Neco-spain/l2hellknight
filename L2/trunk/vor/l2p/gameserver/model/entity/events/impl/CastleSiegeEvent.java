...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
  ...
  public void stopEvent(boolean step)
  {
    ...
    if (ownerClan != null)
    {
      ...
      {
        ((Castle)getResidence()).setRewardCount(((Castle)getResidence()).getRewardCount() + Config.CLAN_BLOOD_ALLIANCE_REWARD);
        ownerClan.broadcastToOnlineMembers(new L2GameServerPacket[] { new SystemMessage2(SystemMsg.SINCE_YOUR_CLAN_EMERGED_VICTORIOUS_FROM_THE_SIEGE_S1_POINTS_HAVE_BEEN_ADDED_TO_YOUR_CLANS_REPUTATION_SCORE).addInteger(ownerClan.incReputation(Config.CLAN_REP_FOR_DEFEND_CASTLE, false, toString())) });
      }
      else
      {
        ...
        ownerClan.broadcastToOnlineMembers(new L2GameServerPacket[] { new SystemMessage2(SystemMsg.SINCE_YOUR_CLAN_EMERGED_VICTORIOUS_FROM_THE_SIEGE_S1_POINTS_HAVE_BEEN_ADDED_TO_YOUR_CLANS_REPUTATION_SCORE).addInteger(ownerClan.incReputation(Config.CLAN_REP_FOR_TAKE_CASTLE, false, toString())) });

        if (_oldOwner != null) {
          _oldOwner.broadcastToOnlineMembers(new L2GameServerPacket[] { new SystemMessage2(SystemMsg.YOUR_CLAN_HAS_FAILED_TO_DEFEND_THE_CASTLE_S1_POINTS_HAVE_BEEN_DEDUCTED_FROM_YOU_CLAN_REPUTATION_SCORE_AND_ADDED_TO_YOUR_OPPONENTS).addInteger(-_oldOwner.incReputation(-Config.CLAN_REP_FOR_LOSE_CASTLE, false, toString())) });
        }
  ...
  private void setNextSiegeTime()
  {
    ...
    calendar.set(11, Config.CASTLE_WAR_HOUR);
    if (calendar.before(Config.CASTLE_VALIDATION_DATE))
      calendar.add(3, 1);
    validateSiegeDate(calendar, 2);
  ...
  public Location getRestartLoc(Player player, RestartType type)
  {
    ...
    {
    case 1:
      if (SevenSigns.getInstance().getSealOwner(3) != 2) break;
