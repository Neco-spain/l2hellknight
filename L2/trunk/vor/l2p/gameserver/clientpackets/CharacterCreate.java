...
public class CharacterCreate extends L2GameClientPacket
{
  ...
  private void initNewChar(GameClient client, Player newChar)
  {
    ...
    if (Config.STARTING_SP > 0) {
      newChar.setSp(Config.STARTING_SP);
    }
    if (Config.STARTING_LEVEL > 1)
    {
      Long exp_add = Long.valueOf(l2p.gameserver.model.base.Experience.LEVEL[Config.STARTING_LEVEL] - newChar.getExp());
      newChar.addExpAndSp(exp_add.longValue(), 0L);
    }

    if (Config.ENABLE_STARTING_ITEM)
    {
      for (int i = 0; i < Config.STARTING_ITEM_ID.length; i++)
      {
        ItemInstance itemCr = ItemFunctions.createItem(Config.STARTING_ITEM_ID[i]);
        itemCr.setCount(Config.STARTING_ITEM_COUNT[i]);
        newChar.getInventory().addItem(itemCr);
      }
    }
