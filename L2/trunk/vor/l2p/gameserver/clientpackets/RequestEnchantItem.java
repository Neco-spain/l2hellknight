...
public class RequestEnchantItem extends L2GameClientPacket
{
  ...
  private static void doEnchantOld(Player player, ItemInstance item, ItemInstance scroll, ItemInstance catalyst) {
    ...
    case 21580:
      ...
      fail = true; break;
    default:
      if (item.getEnchantLevel() < Config.ENCHANT_MAX) break;
      fail = true;
    }
      ...
      if (itemType == 0)
      {
        ...
        else
        {
          ...
          if (ItemFunctions.isBlessedEnchantScroll(scrollId))
            chance = Config.ADD_BLESSED_ENCHANT_CHANCE_WEAPON;
      ...
      else
      {
        ...
        if (itemType == 1)
        {
          ...
          else
          {
            double chance;
            if (ItemFunctions.isBlessedEnchantScroll(scrollId))
              chance = Config.ADD_BLESSED_ENCHANT_CHANCE_ARMOR;
        ...
        else
        {
          ...
          if (itemType == 2)
          {
            ...
            else
            {
              double chance;
              if (ItemFunctions.isBlessedEnchantScroll(scrollId))
                chance = Config.ADD_BLESSED_ENCHANT_CHANCE_JEWEL;
              else chance = Config.ENCHANT_CHANCE_ACCESSORY;
            }
          }
          else
          {
    ...
    {
      item.setEnchantLevel(Config.ADD_FAIL_ENCHANT_VALUE);
      item.setJdbcState(JdbcEntityState.UPDATED);
      item.update();
