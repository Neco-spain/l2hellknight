...
public class Lottery
{
  ...
  public int[] checkTicket(int id, int enchant, int type2)
  {
    ...
    {
      con = L2DatabaseFactory.getInstance().getConnection();
      PreparedStatement statement = con.prepareStatement("SELECT number1, number2, prize1, prize2, prize3 FROM games WHERE id = 1 and idnr = ?");
    ...
    public void run()
    {
      ...
      for (int i = 0; i < 5; i++)
      {
        if (luckynums[i] < 17) enchant = (int)(enchant + Math.pow(2.0D, luckynums[i] - 1)); else {
          type2 = (int)(type2 + Math.pow(2.0D, luckynums[i] - 17)); }  } int count1 = 0;
      int count2 = 0;
      int count3 = 0;
      int count4 = 0;
      ...
      try { con = L2DatabaseFactory.getInstance().getConnection();
        statement = con.prepareStatement("SELECT enchant_level, custom_type2 FROM items WHERE item_id = 4442 AND custom_type1 = ?");
      ...
      if (count1 > 0)
      {
        SystemMessage sm = new SystemMessage(SystemMessageId.AMOUNT_FOR_WINNER_S1_IS_S2_ADENA_WE_HAVE_S3_PRIZE_WINNER);
        sm.addNumber(getId());
      ...
      {
        SystemMessage sm = new SystemMessage(SystemMessageId.AMOUNT_FOR_LOTTERY_S1_IS_S2_ADENA_NO_WINNER);
        sm.addNumber(getId());
        sm.addNumber(getPrize());
        Announcements.getInstance().announceToAll(sm);
      }
