...
public class WildBeastFarmSiege extends ClanHallSiege
{
  ...
  private final ExclusiveTask _mobControlTask = new ExclusiveTask()
  {
    ...
    {
      int mobCount = 0;
      for (WildBeastFarmSiege.clanPlayersInfo cl : _clansInfo.values())
        if (cl._mob.isDead())
        {
      ...
      if (mobCount < 2) {
        ...
        else
        {
          ...
          ThreadPoolManager.getInstance().scheduleGeneral(new WildBeastFarmSiege.midSiegeStep(WildBeastFarmSiege.this, null), 5000L);
        }
      }
      else schedule(3000L);
    }
  ...
  public void gateControl(int val)
  {
    if (val == 1)
    {
      DoorTable.getInstance().getDoor(Integer.valueOf(21150003)).openMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150004)).openMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150001)).closeMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150002)).closeMe();
    }
    else if (val == 2)
    {
      DoorTable.getInstance().getDoor(Integer.valueOf(21150001)).closeMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150002)).closeMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150003)).closeMe();
      DoorTable.getInstance().getDoor(Integer.valueOf(21150004)).closeMe();
    }
  }
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      {
        L2Clan clan = ClanTable.getInstance().getClanByName(clanName);
        L2NpcTemplate template = NpcTable.getInstance().getTemplate(35617 + mobCounter);
        ...
        WildBeastFarmSiege.clanPlayersInfo regPlayers = (WildBeastFarmSiege.clanPlayersInfo)_clansInfo.get(Integer.valueOf(clan.getClanId()));
