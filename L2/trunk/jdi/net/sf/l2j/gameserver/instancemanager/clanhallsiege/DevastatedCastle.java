...
public class DevastatedCastle
{
  ...
  private DevastatedCastle()
  {
    ...
    synchronized (this)
    {
      ...
      _log.info("Devastated Castle: " + milliToSiege / 1000L + " sec. until siege begin");
      rms = null;
    }
  }
  ...
  private void setCalendarForNextSiege()
  {
    ...
    if (daysToChange == 7) {
      if (_siegetime.get(11) < HOUR)
        daysToChange = 0;
      else if ((_siegetime.get(11) == HOUR) && (_siegetime.get(12) < MINUTES))
        daysToChange = 0;
    }
    if (daysToChange > 0) {
      _siegetime.add(5, daysToChange);
    }
    _siegetime.set(11, HOUR);
    _siegetime.set(12, MINUTES);
  }

  private int getDaysToSiege()
  {
    int numDays = _siegetime.get(7) - START_DAY;
  ...
  public void MessengerSpawn()
  {
    ...
    try
    {
      L2NpcTemplate template = NpcTable.getInstance().getTemplate(35420);
    ...
    ThreadPoolManager.getInstance().scheduleGeneral(rs, 14400000L);

    ThreadPoolManager.getInstance().scheduleGeneral(new DeSpawnTimer(result), 7200000L);
    ThreadPoolManager.getInstance().scheduleGeneral(new AnnounceInfo("Siege registration of Devastated castle is over!"), 7200000L);
    ThreadPoolManager.getInstance().scheduleGeneral(new AnnounceInfo("2 hours until siege begin."), 7200000L);
  ...
  public void Siege()
  {
    ...
    try
    {
      ...
      spawn.stopRespawn();
      result = spawn.spawnOne(false);
      _gustav = ThreadPoolManager.getInstance().scheduleGeneral(new DeSpawnTimer(result), 3600000L);
      ...
      spawn.setLocz(-2195);
      spawn.stopRespawn();
      _minion1 = spawn.spawnOne(false);
      _dietrich = ThreadPoolManager.getInstance().scheduleGeneral(new DeSpawnTimer(_minion1), 3600000L);
      ...
      spawn.setLocz(-2194);
      spawn.stopRespawn();
      _minion2 = spawn.spawnOne(false);
      _mikhail = ThreadPoolManager.getInstance().scheduleGeneral(new DeSpawnTimer(_minion2), 3600000L);
  ...
  {
    _monsters.add(new MonsterLocation(id, x, y, z, heading, null));
  }
  ...
  public void spawnMonsters()
  {
    ...
    _monsterdespawn = ThreadPoolManager.getInstance().scheduleGeneral(new DeSpawnMonsters(), 3600000L);
  }
  ...
  protected class DeSpawnMonsters
    implements Runnable
  {
    protected DeSpawnMonsters()
    {
    }
