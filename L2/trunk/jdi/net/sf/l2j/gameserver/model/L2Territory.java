  ...
  implements Serializable
{
  ...
  public boolean isIntersect(int x, int y, Point p1, Point p2)
  {
    ...
    if ((dx1 >= 0.0D) && (dx2 >= 0.0D)) return true;

    if ((dx1 < 0.0D) && (dx2 < 0.0D)) return false;
  ...
  public boolean isInside(int x, int y)
  {
    ...
    {
      Point p1 = (Point)_points.get(i > 0 ? i - 1 : _points.size() - 1);
      Point p2 = (Point)_points.get(i);

      if (!isIntersect(x, y, p1, p2)) continue; intersect_count++;
    }
  ...
  public int[] getRandomPoint()
  {
    ...
    for (int i = 0; i < 100; i++)
    {
      ...
      for (i = 0; i < _points.size(); i++)
      {
        ...
        if ((curdistance != 0.0D) && (distance >= curdistance))
          continue;
