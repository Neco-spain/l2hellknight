...
public class L2DropCategory
{
  ...
  public void addDropData(L2DropData drop)
  {
    if (!drop.isQuestDrop())
    {
      ...
      _categoryBalancedChance = (int)(_categoryBalancedChance + Math.min(drop.getChance() * Config.RATE_DROP_ITEMS, 1000000.0F));
    }
  }
  ...
  public synchronized L2DropData dropOne()
  {
    ...
    for (L2DropData drop : getAllDrops())
    {
      sum = (int)(sum + Math.min(drop.getChance() * Config.RATE_DROP_ITEMS, 1000000.0F));
