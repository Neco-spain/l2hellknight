...
public class Gordon extends L2AttackableAIScript
{
  ...
  public Gordon(int id, String name, String descr)
  {
    ...
    startQuestTimer("check_ai", 120000L, null, null, true);
  ...
  public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
  {
    ...
    else if (event == "check_ai")
    {
      ...
      if (!_isSpawned)
      {
        L2NpcInstance gordon_ai = findTemplate(29095);
        if (gordon_ai != null)
        {
          _isSpawned = true;
          startQuestTimer("Start", 1000L, gordon_ai, null, true);
    ...
    else if (event == "Start")
    {
      if ((npc != null) && (_isSpawned == true))
      {
          ...
          if ((chars != null) && (chars.size() > 0))
          {
            for (L2PcInstance pc : chars)
            {
              if ((pc.isCursedWeaponEquiped()) && (pc.isInsideRadius(npc, 5000, false, false)))
              {
                ...
                startQuestTimer("time_isAttacked", 180000L, npc, null);
                return super.onAdvEvent(event, npc, player);
              }
            }
          }
        ...
        if (_isAttacked == true)
          return super.onAdvEvent(event, npc, player);
        if ((npc.getNpcId() == 29095) && (npc.getX() - 50 <= X) && (npc.getX() + 50 >= X) && (npc.getY() - 50 <= Y) && (npc.getY() + 50 >= Y))
        {
          _isWalkTo += 1;
          if (_isWalkTo > 55)
            _isWalkTo = 1;
  ...
  {
    if ((npc.getNpcId() == 29095) && (_npcBlock == 0))
    {
      _isSpawned = true;
      _isWalkTo = 1;
      startQuestTimer("Start", 1000L, npc, null, true);
    }
