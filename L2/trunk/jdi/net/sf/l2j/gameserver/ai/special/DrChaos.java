...
public class DrChaos extends Quest
{
  ...
  public L2NpcInstance FindTemplate(int npcId)
  {
    ...
    Map values = SpawnTable.getInstance().getSpawnTable();
    for (int i = 0; i < values.size(); i++)
    {
      L2Spawn spawn = (L2Spawn)values.get(Integer.valueOf(i));
  ...
  public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
  {
    if (event.equalsIgnoreCase("Chat"))
    {
      ...
      startQuestTimer("1", 3000L, npc, player);
    }
    else if (event.equalsIgnoreCase("1"))
    {
      L2NpcInstance machine_instance = FindTemplate(32032);
      ...
      {
        startQuestTimer("2", 2000L, npc, player);
      }startQuestTimer("3", 10000L, npc, player);
    }
    ...
    else if (event.equalsIgnoreCase("5"))
    {
      ...
      if (!_IsGolemSpawned)
      {
        L2NpcInstance golem = addSpawn(25512, 94640, -112496, -3336, 0, false, 0);
        _IsGolemSpawned = true;
        startQuestTimer("6", 1000L, golem, player);
  ...
  public String onFirstTalk(L2NpcInstance npc, L2PcInstance player)
  {
    if (npc.getNpcId() == 32033)
    {
      ...
      startQuestTimer("Chat", 3000L, npc, player);
    }
    return null;
  }
