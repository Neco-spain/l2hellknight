  ...
  implements IAdminCommandHandler
{
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar)
  {
    try
    {
      ...
      {
        if (!CTF.checkMinLevel(Integer.valueOf(command.substring(17)).intValue()))
          return false;
        CTF._minlvl = Integer.valueOf(command.substring(17)).intValue();
        showMainPage(activeChar);
      }
      ...
      else if (command.startsWith("admin_ctf_tele_team "))
      {
        for (String t : CTF._teams)
          if (t.equals(command.substring(20)))
          {
            ...
            activeChar.teleToLocation(((Integer)CTF._teamsX.get(index)).intValue(), ((Integer)CTF._teamsY.get(index)).intValue(), ((Integer)CTF._teamsZ.get(index)).intValue());
          }
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_tele_flag "))
      {
        for (String t : CTF._teams)
          ...
          {
            int index = CTF._teams.indexOf(t);
            activeChar.teleToLocation(((Integer)CTF._flagsX.get(index)).intValue(), ((Integer)CTF._flagsY.get(index)).intValue(), ((Integer)CTF._flagsZ.get(index)).intValue());
          }
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_maxlvl "))
      {
        if (!CTF.checkMaxLevel(Integer.valueOf(command.substring(17)).intValue()))
          return false;
        CTF._maxlvl = Integer.valueOf(command.substring(17)).intValue();
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_minplayers "))
      {
        CTF._minPlayers = Integer.valueOf(command.substring(21)).intValue();
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_maxplayers "))
      {
        CTF._maxPlayers = Integer.valueOf(command.substring(21)).intValue();
        showMainPage(activeChar);
      }
      ...
      {
        CTF._npcId = Integer.valueOf(command.substring(14)).intValue();
        showMainPage(activeChar);
      }
      ...
      {
        CTF._rewardId = Integer.valueOf(command.substring(17)).intValue();
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_reward_amount "))
      {
        CTF._rewardAmount = Integer.valueOf(command.substring(24)).intValue();
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_jointime "))
      {
        CTF._joinTime = Integer.valueOf(command.substring(19)).intValue();
        showMainPage(activeChar);
      }
      else if (command.startsWith("admin_ctf_eventtime "))
      {
        CTF._eventTime = Integer.valueOf(command.substring(20)).intValue();
        showMainPage(activeChar);
      }
      ...
      else if (command.startsWith("admin_ctf_team_color "))
      {
        ...
        CTF.setTeamColor(command.substring(params[0].length() + params[1].length() + 2), Integer.decode("0x" + params[1]).intValue());
        showMainPage(activeChar);
      }
  ...
  public void showMainPage(L2PcInstance activeChar)
  {
    ...
    for (String team : CTF._teams)
    {
      ...
      String c = Integer.toHexString(((Integer)CTF._teamColors.get(CTF._teams.indexOf(team))).intValue());
      while (c.length() < 6)
        c = "0" + c;
