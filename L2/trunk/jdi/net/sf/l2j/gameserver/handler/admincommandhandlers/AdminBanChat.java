    ...
    if (command.startsWith("admin_banchat"))
    {
      try
      {
        ...
        if (playerObj != null)
        {
          ...
          activeChar.sendMessage(new StringBuilder().append("Character ").append(player).append(" banchat for ").append(delay > 0 ? new StringBuilder().append(delay).append(" minutes.").toString() : "ever!").toString());
        }
        else {
    ...
    else if (command.startsWith("admin_unbanchat"))
    {
      try
      {
        ...
        if (playerObj != null)
        {
          ...
          activeChar.sendMessage(new StringBuilder().append("Character ").append(player).append(" unbanchat").toString());
        }
        else {
  ...
  private void chatBanOfflinePlayer(L2PcInstance activeChar, String name, int delay)
  {
    ...
    try
    {
      ...
      if (count == 0)
        ...
        activeChar.sendMessage(new StringBuilder().append("Character ").append(name).append(" banchat for ").append(delay > 0 ? new StringBuilder().append(delay).append(" minutes.").toString() : "ever!").toString());
    }
  ...
  private void chatUnBanOfflinePlayer(L2PcInstance activeChar, String name) {
    ...
    try
    {
      ...
      if (count == 0)
        ...
        activeChar.sendMessage(new StringBuilder().append("Character ").append(name).append(" unbanchat").toString());
    }
    catch (SQLException e)
    {
