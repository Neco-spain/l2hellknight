...
import scripts.communitybbs.BB.Topic.ConstructorType;
...
public class TopicBBSManager extends BaseBBSManager
{
  ...
  {
    _maxId.remove(f);
    _maxId.put(f, Integer.valueOf(id));
  }

  public int getMaxID(Forum f)
  {
    Integer i = (Integer)_maxId.get(f);
    if (i == null)
    {
      return 0;
    }
  ...
  public void parsewrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar)
  {
    if (ar1.equals("crea"))
    {
      ...
      {
        f.vload();
        Topic t = new Topic(Topic.ConstructorType.CREATE, getInstance().getMaxID(f) + 1, Integer.parseInt(ar2), ar5, Calendar.getInstance().getTimeInMillis(), activeChar.getName(), activeChar.getObjectId(), 1, 0);
  ...
  private void showMemoTopics(Forum forum, L2PcInstance activeChar, int index)
  {
    ...
    int i = 0; for (int j = getMaxID(forum) + 1; i < 12 * index; j--)
    {
      if (j < 0)
      {
        break;
