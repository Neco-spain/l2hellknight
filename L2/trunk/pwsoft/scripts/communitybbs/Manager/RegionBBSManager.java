...
public class RegionBBSManager extends BaseBBSManager
{
  ...
  private static FastMap<Integer, FastList<L2PcInstance>> _onlinePlayers = new FastMap().shared();
  private static FastMap<Integer, FastMap<String, String>> _communityPages = new FastMap().shared();
  ...
  private void showOldCommunityPI(L2PcInstance activeChar, String name)
  {
    ...
    if (player != null)
    {
      ...
      if ((activeChar != null) && ((activeChar.isGM()) || (player.getObjectId() == activeChar.getObjectId()) || (Config.SHOW_LEVEL_COMMUNITYBOARD)))
      {
        ...
        long nextLevelExpNeeded = 0L;
        if (player.getLevel() < 80)
        {
          nextLevelExp = net.sf.l2j.gameserver.model.base.Experience.LEVEL[(player.getLevel() + 1)];
  ...
  private void addOnlinePlayer(L2PcInstance player)
  {
    ...
    for (FastList page : _onlinePlayers.values())
    {
    ...
    if (!added)
    {
      ...
      if (temp.add(player))
      {
        _onlinePlayers.put(Integer.valueOf(page), temp);
        if (!player.isInvisible())
          _onlineCount += 1;
        _onlineCountGm += 1;
      }
  ...
  {
    for (Iterator i$ = _onlinePlayers.keySet().iterator(); i$.hasNext(); ) { int page = ((Integer)i$.next()).intValue();

      FastMap communityPage = new FastMap();
      TextBuilder htmlCode = new TextBuilder("<html><body><br>");
      String tdClose = "</td>";
      ...
      if (getOnlineCount("gm") > Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
      {
        ...
        htmlCode.append("<tr>");
        if (page == 1) htmlCode.append("<td align=right width=190><button value=\"Prev\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>"); else {
        ...
        htmlCode.append("<td FIXWIDTH=10></td>");
        if (getOnlineCount("gm") <= page * Config.NAME_PAGE_SIZE_COMMUNITYBOARD) htmlCode.append("<td width=190><button value=\"Next\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>"); else {
      ...
      if (getOnlineCount("pl") > Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
      {
        ...
        htmlCode.append("<td FIXWIDTH=10></td>");
        if (getOnlineCount("pl") <= page * Config.NAME_PAGE_SIZE_COMMUNITYBOARD) htmlCode.append("<td width=190><button value=\"Next\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>"); else {
          htmlCode.append("<td width=190><button value=\"Next\" action=\"bypass _bbsloc;page;" + (page + 1) + "\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
        }
      ...
      _communityPages.put(Integer.valueOf(page), communityPage);
    }
  }
  ...
  {
    return (FastList)_onlinePlayers.get(Integer.valueOf(page));
  }

  public String getCommunityPage(int page, String type)
  {
    if (_communityPages.get(Integer.valueOf(page)) != null)
      return (String)((FastMap)_communityPages.get(Integer.valueOf(page))).get(type);
    return null;
  }
}