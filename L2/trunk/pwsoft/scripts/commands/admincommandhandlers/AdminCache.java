  ...
  implements IAdminCommandHandler
{
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar)
  {
    ...
    else if ((command.startsWith("admin_cache_crest_rebuild")) || (command.startsWith("admin_cache_crest_reload")))
    {
      ...
      activeChar.sendAdmResultMessage("Cache[Crest]: " + String.format("%.3f", new Object[] { Float.valueOf(CrestCache.getInstance().getMemoryUsage()) }) + " megabytes on " + CrestCache.getInstance().getLoadedFiles() + " files loaded");
    }
