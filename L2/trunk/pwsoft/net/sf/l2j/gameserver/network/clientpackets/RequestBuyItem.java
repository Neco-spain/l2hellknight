...
public final class RequestBuyItem extends L2GameClientPacket
{
  ...
  protected void runImpl()
  {
    ...
    if (System.currentTimeMillis() - player.gCPAX() < 100L)
    {
    ...
    for (int i = 0; i < _count; i++)
    {
      ...
      if ((!Config.LOG_ITEMS) || (item == null))
        continue;
      String act = new StringBuilder().append("BUY ").append(item.getItemName()).append("(").append(count).append(")(").append(item.getObjectId()).append(")(npc:").append(merchant == null ? "null" : Integer.valueOf(merchant.getTemplate().npcId)).append(") #(player ").append(player.getName()).append(", account: ").append(player.getAccountName()).append(", ip: ").append(player.getIP()).append(")").toString();
    ...
    if (merchant != null)
    {
      String html = HtmCache.getInstance().getHtm(new StringBuilder().append("data/html/").append(htmlFolder).append("/").append(merchant.getNpcId()).append("-bought.htm").toString());
