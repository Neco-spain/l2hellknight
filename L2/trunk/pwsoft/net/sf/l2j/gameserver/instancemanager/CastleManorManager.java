  ...
  private void load()
  {
    ...
    try
    {
      ...
      for (Castle castle : CastleManager.getInstance().getCastles()) {
        ...
        st = con.prepareStatement("SELECT * FROM castle_manor_production WHERE castle_id=?");
        ...
        st = con.prepareStatement("SELECT * FROM castle_manor_procure WHERE castle_id=?");
        st.setInt(1, castle.getCastleId());
        rs = st.executeQuery();
        rs.setFetchSize(50);
        while (rs.next()) {
    ...
    _scheduledManorRefresh = ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
    {
      public void run() {
        if (!isDisabled()) {
          ...
          CastleManorManager._log.info("CastleManorManager: Under maintenance mode started");

          _scheduledMaintenanceEnd = ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
          {
            public void run() {
              CastleManorManager._log.info("CastleManorManager: Next period started");
              setNextPeriod();
              try {
                save();
              } catch (Exception e) {
                CastleManorManager._log.info("CastleManorManager: Failed to save manor data: " + e);
              }
  ...
  public void updatePeriodApprove()
  {
    ...
    _scheduledNextPeriodapprove = ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
    {
      public void run() {
        if (!isDisabled()) {
          approveNextPeriod();
          CastleManorManager._log.info("CastleManorManager: Next period approved");
        }
        updatePeriodApprove();
      }
    }
