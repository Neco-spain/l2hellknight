...
public class BookMarkList
{
  ...
  {
    return (BookMark[])elementData.toArray(new BookMark[elementData.size()]);
  }
  ...
  public BookMark get(int slot)
  {
    if ((slot < 1) || (slot > elementData.size()))
      return null;
    return (BookMark)elementData.get(slot - 1);
  }
  ...
  public boolean tryTeleport(int slot)
  {
    ...
    if ((slot < 1) || (slot > elementData.size()))
      return false;
    BookMark bookmark = (BookMark)elementData.get(slot - 1);
    if (!checkTeleportLocation(owner, bookmark.x, bookmark.y, bookmark.z)) {
      return false;
    }

    if (Functions.removeItem(owner, 13016, 1L) != 1L)
    {
  ...
  public boolean add(Location loc, String aname, String aacronym, int aiconId, boolean takeFlag)
  {
    ...
    if ((takeFlag) && 
      (Functions.removeItem(owner, 20033, 1L) != 1L))
    {
  ...
  public void store()
  {
    ...
    try
    {
      ...
      {
        statement.setInt(1, owner.getObjectId());
        slotId++; statement.setInt(2, slotId);
