...
import l2m.gameserver.skills.conditions.ConditionPlayerResidence;
...
public abstract class StatParser<H extends AbstractHolder> extends AbstractDirParser<H>
{
  ...
  {
    Iterator i$ = n.elements().iterator(); if (i$.hasNext()) { Object element = (Element)i$.next();
  ...
  protected Condition parseTargetCondition(Element element)
  {
    ...
    for (Iterator iterator = element.attributeIterator(); iterator.hasNext(); )
    {
      ...
      if (name.equalsIgnoreCase("pvp")) {
        cond = joinAnd(cond, new ConditionTargetPlayable(Boolean.valueOf(value).booleanValue()));
      }
    }
    return cond;
  }
  ...
  protected Condition parsePlayerCondition(Element element)
  {
    ...
    for (Iterator iterator = element.attributeIterator(); iterator.hasNext(); )
    {
      ...
      if (name.equalsIgnoreCase("residence"))
      {
        ...
        cond = joinAnd(cond, new ConditionPlayerOlympiad(Boolean.valueOf(value).booleanValue()));
    ...
    for (Iterator iterator = element.attributeIterator(); iterator.hasNext(); )
    {
      ...
      else if ((name.equalsIgnoreCase("kind")) || (name.equalsIgnoreCase("weapon")))
      {
        ...
        while (st.hasMoreTokens())
        {
          ...
          WeaponTemplate.WeaponType[] arr$ = WeaponTemplate.WeaponType.VALUES; int len$ = arr$.length; for (int i$ = 0; ; i$++) { if (i$ >= len$) break label255; WeaponTemplate.WeaponType wt = arr$[i$];
            if (!wt.toString().equalsIgnoreCase(item))
              continue;
            mask |= wt.mask();
            break;
          }
          ArmorTemplate.ArmorType[] arr$ = ArmorTemplate.ArmorType.VALUES; int len$ = arr$.length; for (int i$ = 0; ; i$++) { if (i$ >= len$) break label314; ArmorTemplate.ArmorType at = arr$[i$];
  ...
  protected void parseTriggers(Element f, StatTemplate triggerable)
  {
    for (Iterator iterator = f.elementIterator(); iterator.hasNext(); )
    {
      ...
      for (subIterator = element.elementIterator(); subIterator.hasNext(); )
      {
