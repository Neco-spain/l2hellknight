package l2m.gameserver.handler.bbs;
...
public class CommunityBoardManager
{
  ...
  public void registerHandler(ICommunityBoardHandler commHandler)
  {
    for (String bypass : commHandler.getBypassCommands())
    {
      if (_handlers.containsKey(bypass)) {
        _log.warn("CommunityBoard: dublicate bypass registered! First handler: " + ((ICommunityBoardHandler)_handlers.get(bypass)).getClass().getSimpleName() + " second: " + commHandler.getClass().getSimpleName());
      }
  ...
  public ICommunityBoardHandler getCommunityHandler(String bypass)
  {
    ...
    for (Map.Entry entry : _handlers.entrySet()) {
      if (bypass.contains((CharSequence)entry.getKey()))
