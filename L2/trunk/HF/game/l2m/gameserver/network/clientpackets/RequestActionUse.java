...
import l2m.gameserver.network.serverpackets.ExAirShipTeleportList;
import l2m.gameserver.network.serverpackets.ExAskCoupleAction;
...
import l2m.gameserver.network.serverpackets.PrivateStoreManageListSell;
import l2m.gameserver.network.serverpackets.RecipeShopManageList;
...
public class RequestActionUse extends L2GameClientPacket
{
  ...
  protected void runImpl()
  {
    ...
    if (action.type == 3)
    {
      ...
      activeChar.broadcastPacket(new L2GameServerPacket[] { new SocialAction(activeChar.getObjectId(), action.value) });
    ...
    case 0:
      ...
      {
        if ((target != null) && ((target instanceof StaticObjectInstance)) && (((StaticObjectInstance)target).getType() == 1) && (activeChar.getDistance3D(target) <= 200.0D))
    ...
    case 21:
      if (pet == null) break;
      pet.setFollowMode(!pet.isFollowMode()); break;
    case 16:
    case 22:
    ...
    case 19:
      ...
      else if ((!PetDataTable.isVitaminPet(pet.getNpcId())) && (pet.isPet()) && (pet.getCurrentFed() < 0.55D * pet.getMaxFed()))
      {
    ...
    case 54:
      ...
      pet.moveToLocation(target.getLoc(), 100, true); break;
    case 1070:
      if (!(pet instanceof PetBabyInstance)) break;
      ((PetBabyInstance)pet).triggerBuff(); break;
    case 67:
    ...
    case 68:
      ...
      break;
    case 69:
      if ((!activeChar.isClanAirShipDriver()) || (!activeChar.getBoat().isDocked())) break;
      activeChar.sendPacket(new ExAirShipTeleportList((ClanAirShip)activeChar.getBoat())); break;
    case 70:
      if ((!activeChar.isInBoat()) || (!activeChar.getBoat().isAirShip()) || (!activeChar.getBoat().isDocked())) break;
      activeChar.getBoat().oustPlayer(activeChar, activeChar.getBoat().getReturnLoc(), true); break;
    case 1001:
      break;
    default:
      _log.warn("unhandled action type " + _actionId + " by player " + activeChar.getName());
  ...
  public static enum Action
  {
    ...
    private Action(int id, int type, int value, int transform) { this.id = id;
      this.type = type;
      this.value = value;
