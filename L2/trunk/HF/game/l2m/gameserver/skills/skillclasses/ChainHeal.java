...
import l2m.gameserver.skills.conditions.ConditionTargetRelation;
import l2m.gameserver.skills.conditions.ConditionTargetRelation.Relation;
...
public class ChainHeal extends Skill
{
  ...
  public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse)
  {
    ...
    for (Creature target : targets)
    {
      if ((target == null) || (target.isHealBlocked()) || (target.isCursedWeaponEquipped()) || (ConditionTargetRelation.getRelation(activeChar, target) != ConditionTargetRelation.Relation.Friend) || (target.isAutoAttackable(activeChar))) {
        continue;
      }
