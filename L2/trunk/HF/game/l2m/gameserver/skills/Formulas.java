...
public class Formulas
{
  ...
  public static boolean calcBlow(Creature activeChar, Creature target, Skill skill)
  {
    ...
    {
      activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", (Player)activeChar, new Object[0]).addString(skill.getName()).addNumber(0L));
    }
  ...
  public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
  {
    ...
    if (aConfig.get("AltDebugEnabled", false))
    {
      debugCaster = (caster.getPlayer() != null) && (caster.getPlayer().isDebug());

      debugTarget = (target.getPlayer() != null) && (target.getPlayer().isDebug());

      boolean debugPvP = (aConfig.get("AltDebugPvPEnabled", false)) && (debugCaster) && (debugTarget) && ((!aConfig.get("AltDebugPvPDuelOnly", true)) || ((caster.getPlayer().isInDuel()) && (target.getPlayer().isInDuel())));

      debugGlobal = (debugPvP) || ((aConfig.get("AltDebugPvEEnabled", false)) && (((debugCaster) && (target.isMonster())) || ((debugTarget) && (caster.isMonster()))));
    }
    ...
    double mLevel = skill.getMagicLevel();

    mLevel = (mLevel - target.getLevel() + 3.0D) * skill.getLevelModifier();

    value += (mLevel >= 0.0D ? 0.0D : mLevel);
    ...
    if ((Config.SKILLS_CHANCE_SHOW) || (caster.getPlayer().isGM()))
    {
      Player player = caster.getPlayer();
      if ((player != null) && (player.getVarB("SkillsHideChance")))
      {
        player.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", player, new Object[0]).addString(skill.getName()).addNumber(Math.round(0.0F)));
      }
    }

    if (caster.isMonster())
    {
      value *= aConfig.get("SkillsMonsterChanceMod", 1.0D);
    }
    ...
    if (debugGlobal)
    {
      ...
      stat.append(") lvl:");
      stat.append(String.format("%1.1f", new Object[] { Double.valueOf(mLevel) }));
      stat.append(" elem:");
