...
import l2m.gameserver.model.instances.ControlKeyInstance;
import l2m.gameserver.model.pledge.Clan;
import l2m.gameserver.network.serverpackets.DeleteObject;
...
import l2m.gameserver.templates.AirshipDock.AirshipPlatform;
import l2m.gameserver.utils.Location;

public class ClanAirShip extends AirShip
{
  private static final long serialVersionUID = 1L;
  private static final long MAINTENANCE_DELAY = 60000L;
  ...
  public void startDepartTask()
  {
    ...
    {
      arrivalWay.addObject("boat_points", p);
    }

    for (BoatPoint p : _platform.getDepartPoints())
    {
      departWay.addObject("boat_points", p);
    }

    arrivalWay.addOnTimeAction(0, new StartStopAction("event", true));
    departWay.addOnTimeAction(300, new StartStopAction("event", true));
  ...
  public void addTeleportPoint(Player player, int id)
  {
    ...
    {
      BoatPoint point = (BoatPoint)getDock().getTeleportList().get(id);
      ...
      getCurrentWay().clearActions();
      getCurrentWay().addObject("boat_points", point);
      getCurrentWay().startEvent();
    }
  }
  ...
  public void trajetEnded(boolean oust)
  {
    ...
    {
      _customMove = true;
      _deleteTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new FuelAndDeleteTask(null), 60000L, 60000L);
    }
  }
  ...
  public void setDriver(Player player)
  {
    if (player != null)
    {
      ...
      int x = player.getInBoatPosition().x - 366;
      int y = player.getInBoatPosition().y;
      int z = player.getInBoatPosition().z - 107;
      if (x * x + y * y + z * z > 2500)
      {
