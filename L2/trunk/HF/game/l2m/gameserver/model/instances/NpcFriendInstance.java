  ...
  public void onAction(Player player, boolean shift)
  {
    if (this != player.getTarget())
    {
      ...
      if (isAutoAttackable(player))
        player.sendPacket(makeStatusUpdate(new int[] { 9, 10 }));
  ...
  public void onBypassFeedback(Player player, String command)
  {
    ...
    if (actualCommand.equalsIgnoreCase("Buff"))
    {
      ...
      if ((skill != 0) && (player.getInventory().destroyItemByItemId(item, count)))
        ...
        showChatWindow(player, "npc_friend/" + getNpcId() + "-havenotitems.htm", new Object[0]);
    }
    else if (command.startsWith("Chat"))
    {
      ...
      if (!fname.equals(""))
        showChatWindow(player, fname, new Object[0]);
    }
