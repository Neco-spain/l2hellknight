package l2m.gameserver.model.actor.recorder;
...
public final class PlayerStatsChangeRecorder extends CharStatsChangeRecorder<Player>
{
  ...
  protected void onSendChanges()
  {
    ...
    if ((_changes & 0x40) == 64) {
      ((Player)_activeChar).sendStatusUpdate(false, false, new int[] { 14 });
    }
    if ((_changes & 0x20) == 32) {
      ((Player)_activeChar).sendStatusUpdate(false, false, new int[] { 15 });
    }
    if ((_changes & 0x8) == 8) {
      ((Player)_activeChar).sendStatusUpdate(true, false, new int[] { 27 });
    }
    if ((_changes & 0x10) == 16)
