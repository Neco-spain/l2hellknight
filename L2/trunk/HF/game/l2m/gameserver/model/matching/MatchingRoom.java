package l2m.gameserver.model.matching;
...
import l2m.gameserver.model.PlayerGroup;
  ...
  implements PlayerGroup
{
  ...
  private final PartyListenerImpl _listener = new PartyListenerImpl(null);
  ...
  private boolean addMember0(Player player, L2GameServerPacket p)
  {
    ...
    for (Player $member : this) {
      if ($member != player)
        $member.sendPacket(new IStaticPacket[] { p, addMemberPacket($member, player) });
    }
    MatchingRoomManager.getInstance().removeFromWaitingList(player);
    player.sendPacket(new IStaticPacket[] { infoRoomPacket(), membersPacket(player) });
    player.sendChanges();
    return true;
  }
  ...
  public void removeMember(Player member, boolean oust)
  {
    ...
    L2GameServerPacket infoPacket;
    L2GameServerPacket exitMessage;
    if (_members.isEmpty()) {
      disband();
    }
    else {
      infoPacket = infoRoomPacket();
      SystemMsg exitMessage0 = exitMessage(true, oust);
      exitMessage = exitMessage0 != null ? (SystemMessage2)new SystemMessage2(exitMessage0).addName(member) : null;
      for (Player player : this) {
        player.sendPacket(new IStaticPacket[] { infoPacket, removeMemberPacket(player, member), exitMessage });
      }
    }
    member.sendPacket(new IStaticPacket[] { closeRoomPacket(), exitMessage(false, oust) });
  ...
  public abstract SystemMsg exitMessage(boolean paramBoolean1, boolean paramBoolean2);
  ...
  public abstract L2GameServerPacket addMemberPacket(Player paramPlayer1, Player paramPlayer2);

  public abstract L2GameServerPacket removeMemberPacket(Player paramPlayer1, Player paramPlayer2);

  public abstract L2GameServerPacket updateMemberPacket(Player paramPlayer1, Player paramPlayer2);

  public abstract L2GameServerPacket membersPacket(Player paramPlayer);

  public abstract int getType();

  public abstract int getMemberType(Player paramPlayer);

  public void broadCast(IStaticPacket[] arg) { for (Player player : this)
  ...
  private class PartyListenerImpl
    implements OnPlayerPartyInviteListener, OnPlayerPartyLeaveListener
  {
