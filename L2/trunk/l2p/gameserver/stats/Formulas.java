...
public class Formulas
{
  ...
  public static boolean calcBlow(Creature activeChar, Creature target, Skill skill)
  {
    ...
    if ((Config.SKILLS_CHANCE_SHOW) && (activeChar.isPlayer()) && (((Player)activeChar).getVarB("SkillsHideChance"))) {
      activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", (Player)activeChar, new Object[0]).addString(skill.getName()).addNumber(()chance));
    }
  ...
  public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
  {
    ...
    if ((Config.SKILLS_CHANCE_SHOW) || (caster.getPlayer().isGM()))
    {
      ...
      if ((player != null) && (player.getVarB("SkillsHideChance"))) {
        player.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", player, new Object[0]).addString(skill.getName()).addNumber(Math.round(value)));
      }
    }
