...
import l2p.gameserver.serverpackets.ExNavitAdventEffect;
import l2p.gameserver.serverpackets.ExNavitAdventPointInfo;
import l2p.gameserver.serverpackets.ExNavitAdventTimeChange;
...
public class NevitSystem
{
  ...
  private void startNevitEffect(int time)
  {
    if (getEffectTime() > 0)
    {
      _player.setVitality(_player.getVarD("vitality", 0.0D));
      _player.unsetVar("vitality");
    ...
    if (time > 0)
    {
      ...
      _player.setVar("vitality", _player.getVitality(), -1L);
      _player.sendPacket(new ExNavitAdventEffect(time));
      _player.setVitality(l2p.gameserver.Config.VITALITY_LEVELS[4]);
