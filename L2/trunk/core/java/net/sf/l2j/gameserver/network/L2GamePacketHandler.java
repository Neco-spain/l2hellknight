...
import net.sf.protection.Network;
...
public final class L2GamePacketHandler extends TCPHeaderHandler<L2GameClient> implements IPacketHandler<L2GameClient>, IClientFactory<L2GameClient>, IMMOExecutor<L2GameClient>
{
     ...
     */
	private boolean fixEnterWorld = false;
	private int state = 0;
	...
	private boolean my_fix(int opcode)
	{
		if(opcode==0xd0 && state==1)
		{
			state=2;
			return true;
		}
		else if(opcode==0x03 && state==2)
		{
			state=0;
			fixEnterWorld=false;
			return true;
	...
	public ReceivablePacket<L2GameClient> handlePacket(ByteBuffer buf, L2GameClient client)
	{
		...
		if(Config.ENTER_WORLD_FIX)
		{
			if(fixEnterWorld)
			{
				if(!my_fix(opcode))
				{
					_log.severe("Cheating: replaced EnterWorld");
...
//						break;
	...
	private void printDebug(int opcode, ByteBuffer buf, GameClientState state, L2GameClient client)
	{
		...
		if (Config.ENABLE_PACKET_PROTECTION)
			Network.handlePacketProtection(client);
	}
