	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if(command.startsWith("admin_sethero"))
        {
            ...
            if(cha != null)
            {
                ...
                activeChar.sendMessage((new StringBuilder("Персонаж ")).append(cha.getName()).append(" получил статус героя на ").append(days).append(" дней.").toString());
            } else
            {
                activeChar.sendMessage((new StringBuilder("Персонаж ")).append(name).append(" не найден или оффлайн.").toString());
            }
        }
		else if(command.startsWith("admin_delhero"))
        {
            ...
            if(cha != null)
            {
                ...
                activeChar.sendMessage((new StringBuilder("У персонажа ")).append(cha.getName()).append(" удален статус героя.").toString());
            } else
            {
                activeChar.sendMessage((new StringBuilder("Персонаж ")).append(name).append(" не найден или оффлайн.").toString());
            }
		...
		else if (command.startsWith("admin_setname"))
		{
			try
			{
				...
				//L2World.getInstance().removeVisibleObject(player, player.getWorldRegion());
