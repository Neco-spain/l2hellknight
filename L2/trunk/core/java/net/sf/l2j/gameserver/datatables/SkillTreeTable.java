...
import net.sf.l2j.gameserver.model.L2LearnSkill;
...
public class SkillTreeTable
{
	...
	private List<L2LearnSkill> _shopSkillTrees;
	...
	private SkillTreeTable()
	{
        ...
        try
        {
          _shopSkillTrees = new FastList<L2LearnSkill>();
          
          PreparedStatement statement = con.prepareStatement("SELECT skill_id, level, name, sp, min_level, costid, cost, bookid, listid FROM shop_skill_trees ORDER BY skill_id, level");
          ...
          while (skilltree5.next())
          {
            ...
            int spbId = skilltree5.getInt("bookid");
            int spbId2 = skilltree5.getInt("listid");
            if (prevSkillId != id)
            	prevSkillId = id;
            
            L2LearnSkill skill = new L2LearnSkill(id, lvl, minLvl, name, cost, costId, costCount, spbId, spbId2);
            _shopSkillTrees.add(skill);
          }
          skilltree5.close();
          statement.close();
          count6 = _shopSkillTrees.size();
        }
        catch (Exception e)
        {
          _log.severe("Error while creating shop skills table: " + e);
        }
        try
          {
            con.close(); 
        ...
        _log.config("ShopSkillTreeTable: Loaded " + count6 + " shop skills");
    }
	...
	@SuppressWarnings("unused")
	public L2LearnSkill[] getAvailableShopSkills(L2PcInstance cha)
	{
	    List<L2LearnSkill> result = new FastList<L2LearnSkill>();
	    List<L2LearnSkill> skills = new FastList<L2LearnSkill>();
	    skills.addAll(_shopSkillTrees);
	    if (skills == null)
	    {
	        // the skilltree for this class is undefined, so we give an empty list
	        _log.warning("Skilltree for fishing is not defined !");
	        return new L2LearnSkill[0];
	    }

        L2Skill[] oldSkills = cha.getAllSkills();

        for (L2LearnSkill temp : skills)
        {
            if (temp.getMinLevel() <= cha.getLevel())
            {
                boolean knownSkill = false;
        ...
        return result.toArray(new L2LearnSkill[result.size()]);
	}
    ...
    @SuppressWarnings("unused")
	public int getMinLevelForNewShopSkill(L2PcInstance cha)
    {
        int minLevel = 0;
        List<L2LearnSkill> skills = new FastList<L2LearnSkill>();
