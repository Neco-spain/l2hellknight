    	...
    	if (player != null)
        {
          ...
          if (player.getParty() != null)
          {
            for (L2PcInstance member : player.getParty().getPartyMembers())
            {
            	if (Config.NOBLESSE_BY_KILLING_RB)
                {
                  if (getNpcId() == 25325)
                  {
                    if (!member.isNoble() /*&& member.getSubClasses() != null*/)
                    {
                      member.setNoble(true);
                      member.addItem("Tiara", 7694, 1, null, true);
                      member.sendMessage("¬ы получили статус двор€нина за убийство Barakiel'€");
                    }
                  }
                }
          ...
          else 
          {
        	  if (Config.NOBLESSE_BY_KILLING_RB)
              {
                if (getNpcId() == 25325)
                {
                  if (!player.isNoble() /*&& player.getSubClasses() != null*/)
                  {
                    player.setNoble(true);
                    player.addItem("Tiara", 7694, 1, null, true);
                    player.sendMessage("¬ы получили статус двор€нина за убийство Barakiel'€");
                  }
                }
              }
