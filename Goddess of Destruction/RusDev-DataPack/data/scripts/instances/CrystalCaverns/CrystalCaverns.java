...
import com.l2js.gameserver.instancemanager.InstanceManager.InstanceWorld;
import com.l2js.gameserver.model.*;
import com.l2js.gameserver.model.L2Skill.SkillTargetType;
import com.l2js.gameserver.model.actor.*;
...
public class CrystalCaverns extends Quest
{
	...
	{
			18378, 143682, 142492, -11886, 16384
	};
	...
	{
			22279, 22280, 22281, 22282, 22283, 22285, 22286, 22287, 22288, 22289, 22293, 22294, 22295, 22296, 22297,
			22305, 22306, 22307, 22416, 22418, 22419, 22420
	};
	 ...
	 * 32767},
	 * {22308, 151697, 152621, -12167, 31423},{22309, 151061, 152581, -12165,
	 * 6228},
	 * {22309, 150653, 152253, -12132, 31343},{22309, 150628, 152431, -12165,
	 * 33022},
	 * {22309, 151620, 152487, -12165, 30114},{22309, 151672, 152544, -12165,
	 * 31846},
	 * {22309, 150488, 152350, -12165, 29072},{22310, 151139, 152238, -12132,
	 * 1069}
	 */
	};

	private static int[][] STEAM4_SPAWNS =
	{// {22308, 151707, 150199, -12165, 32859},{22308, 152091, 150140, -12165,
		// 32938},
	...
	private void Throw(L2Character effector, L2Character effected)
	{
		...
		if (Config.GEODATA > 0)
		{
			Location destiny = GeoData.getInstance().moveCheck(effected.getX(), effected.getY(), effected.getZ(), _x,
					_y, _z, effected.getInstanceId());
			_x = destiny.getX();
			_y = destiny.getY();
		}
	...
	protected int enterInstance(L2PcInstance player, String template, teleCoord teleto)
	{
		...
		else
		{
			...
			_log.info("Crystal Caverns started " + template + " Instance: " + instanceId + " created by player: "
					+ player.getName());
	...
	protected void runHall(CCWorld world)
	{
		...
		{
			L2Npc mob = addSpawn(CGMOBS[Rnd.get(CGMOBS.length)], spawn[0], spawn[1], spawn[2], spawn[3], false, 0,
	...
	protected void runSteamOracles(CCWorld world, int[][] oracleOrder)
	{
		...
		{
			world.oracles.put(
					addSpawn(oracle[0], oracle[1], oracle[2], oracle[3], oracle[4], false, 0, false, world.instanceId),
					null);
		}
	}
	 ...
	 * );//up power
	 * addSpawn(29101,151951,142078,-12706,65203,false,0,false,world.instanceId,0
	 * );//up power
	 * addSpawn(29101,154396,140667,-12706,22197,false,0,false,world.instanceId,0
	 * );//up power
	 * addSpawn(29102,152162,141249,-12706,5511,false,0,false,world.instanceId,0)
	 * ;//down power
	 * addSpawn(29102,153571,140458,-12706,16699,false,0,false,world.instanceId,0
	 * );//down power
	 * addSpawn(29102,154976,141265,-12706,26908,false,0,false,world.instanceId,0
	 * );//down power
	 * addSpawn(29102,155203,142071,-12706,31560,false,0,false,world.instanceId,0
	 * );//down power
	 * addSpawn(29102,154380,143468,-12708,43943,false,0,false,world.instanceId,0
	 * );//down power
				...
				else if (!world.copys.isEmpty())
				{
					...
					if (skill != null
							&& (skill.getTargetType() == SkillTargetType.TARGET_AREA
									...
									|| skill.getTargetType() == SkillTargetType.TARGET_FRONT_AURA || skill
									.getTargetType() == SkillTargetType.TARGET_BEHIND_AURA))
						notAOE = false;
					if (notAOE)
					{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			else if (event.equalsIgnoreCase("spawn_oracle"))
			{
				...
				addSpawn((Rnd.get(10) < 5 ? 29116 : 29117), npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(),
						false, 0, false, world.instanceId); // Baylor's Chest
			...
			else if (event.equalsIgnoreCase("baylor_alarm"))
			{
				if (world._alarm == null)
				{
					...
					world._alarm
							.broadcastPacket(new CreatureSay(world._alarm.getObjectId(), 1, world._alarm.getName(),
			...
			{
				CrystalGolem cryGolem = world.crystalGolems.get(npc);
				L2CharPosition newpos = new L2CharPosition(cryGolem.foodItem.getX(), cryGolem.foodItem.getY(),
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			if (world.status == 2 && world.npcList1.containsKey(npc))
			{
				...
				world.crystalGolems.put(
						addSpawn(CRYSTAL_GOLEM, 140547, 151670, -11813, 32767, false, 0, false, world.instanceId),
						crygolem1);
				world.crystalGolems.put(
						addSpawn(CRYSTAL_GOLEM, 141941, 151684, -11813, 63371, false, 0, false, world.instanceId),
						crygolem2);
				...
				if (checkKillProgress(0, npc, world))
				{
					...
					switch (world.status)
					{
						...
						case 25:
							...
							L2Npc kechi = addSpawn(KECHI, 154069, 149525, -12158, 51165, false, 0, false,
									world.instanceId);
							startQuestTimer("checkKechiAttack", 1000, kechi, null);
							return "";
						default:
							_log.warning("CrystalCavern-SteamCorridor: status " + world.status
			...
			else if (npc.getNpcId() == ORACLE_GUIDE_3)
			{
				if (world.status < 30 && checkBaylorConditions(player))
				{
					...
					else
					{
						...
						{
							int rnd = Rnd.get(100);
							partyMember.destroyItemByItemId("Quest", (rnd < 33 ? BOSS_CRYSTAL_1
									: (rnd < 67 ? BOSS_CRYSTAL_2 : BOSS_CRYSTAL_3)), 1, partyMember, true);
