...
public class Base64
{
	...
	static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset)
	{
		...
		else
		{
			...
			{
				System.out.println(StringUtil.concat(String.valueOf(source[srcOffset]), ": ",
						String.valueOf(DECODABET[source[srcOffset]])));
	...
	public static byte[] decode(byte[] source, int off, int len)
	{
		...
		for (i = off; i < off + len; i++)
		{
			...
			{
				System.err.println(StringUtil.concat("Bad Base64 input character at ", String.valueOf(i), ": ",
						String.valueOf(source[i]), "(decimal)"));
				return null;
			} // end else:
