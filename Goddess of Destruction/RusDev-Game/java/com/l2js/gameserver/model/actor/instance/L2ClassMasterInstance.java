...
public final class L2ClassMasterInstance extends L2MerchantInstance
{
				...
				else
				{
					...
					if (player.getLevel() >= minLevel || Config.ALLOW_ENTIRE_TREE)
					{
						...
						for (ClassId cid : ClassId.values())
						{
							...
							if (validateClassId(currentClassId, cid) && cid.level() == level)
							{
								StringUtil.append(menu, "<a action=\"bypass -h npc_%objectId%_change_class ", String
										.valueOf(cid.getId()), "\">",
										CharTemplateTable.getInstance().getClassNameById(cid.getId()), "</a><br>");
							}
						}
						...
						if (menu.length() > 0)
						{
							...
							html.replace("%name%",
									CharTemplateTable.getInstance().getClassNameById(currentClassId.getId()));
	...
	private static final void showTutorialHtml(L2PcInstance player)
	{
		...
		for (ClassId cid : ClassId.values())
		{
			...
			{
				StringUtil.append(menu, "<a action=\"link CO", String.valueOf(cid.getId()), "\">", CharTemplateTable
						.getInstance().getClassNameById(cid.getId()), "</a><br>");
			}
		}
	...
	private static final boolean checkAndChangeClass(L2PcInstance player, int val)
	{
		...
		if (!Config.CLASS_MASTER_SETTINGS.getRewardItems(newJobLevel).isEmpty())
		{
			if (player.getWeightPenalty() >= 3
					|| (player.getInventoryLimit() * 0.8 <= player.getInventory().getSize(false)))
			{
		...
		if (Config.CLASS_MASTER_SETTINGS.isAllowed(player.getClassId().level() + 1)
				...
				&& ((player.getClassId().level() == 1 && player.getLevel() >= 40) || (player.getClassId().level() == 2 && player
						.getLevel() >= 76)))
