 ...
 */
package com.l2js.gameserver.model.actor.status;
...
public class PcStatus extends PlayableStatus
{
	...
	public final void reduceHp(double value, L2Character attacker, boolean awake, boolean isDOT,
			boolean isHPConsumption, boolean ignoreCP)
	{
		...
		if (attacker != null && attacker != getActiveChar())
		{
			...
			mpDam = (int) value * (int) getActiveChar().getStat().calcStat(Stats.MANA_SHIELD_PERCENT, 0, null, null)
					/ 100;
			...
			if (fullValue > 0 && !isDOT)
			{
				...
				if (tDmg > 0)
				{
					...
					{
						smsg = SystemMessage
								.getSystemMessage(SystemMessageId.GIVEN_S1_DAMAGE_TO_YOUR_TARGET_AND_S2_DAMAGE_TO_SERVITOR);
						smsg.addNumber(fullValue);
						smsg.addNumber(tDmg);
