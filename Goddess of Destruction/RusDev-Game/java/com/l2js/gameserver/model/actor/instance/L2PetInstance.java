...
import com.l2js.gameserver.datatables.CharSummonTable;
import com.l2js.gameserver.datatables.ItemTable;
import com.l2js.gameserver.datatables.PetDataTable;
import com.l2js.gameserver.datatables.SkillTable;
import com.l2js.gameserver.datatables.SummonEffectsTable;
import com.l2js.gameserver.datatables.SummonEffectsTable.SummonEffect;
...
import com.l2js.gameserver.instancemanager.CursedWeaponsManager;
import com.l2js.gameserver.instancemanager.ItemsOnGroundManager;
...
import com.l2js.gameserver.model.L2PetData;
import com.l2js.gameserver.model.L2PetLevelData;
...
import com.l2js.gameserver.model.actor.stat.PetStat;
...
import com.l2js.gameserver.network.serverpackets.PetInventoryUpdate;
import com.l2js.gameserver.network.serverpackets.PetItemList;
import com.l2js.gameserver.network.serverpackets.StatusUpdate;
import com.l2js.gameserver.network.serverpackets.StopMove;
...
public class L2PetInstance extends L2Summon
{
			...
			try
			{
				...
				if (foodIds.length == 0)
				{
					if (getCurrentFed() == 0)
					{
						...
						getOwner().sendPacket(
								SystemMessage.getSystemMessage(SystemMessageId.THERE_NOT_MUCH_TIME_REMAINING_UNTIL_HELPER_LEAVES));
					return;
				}
				...
				else
				{
					if (getCurrentFed() == 0)
					{
						...
						if (Rnd.get(100) < 30)
						{
							...
							getOwner().sendPacket(sm);
							_log.info("Hungry pet [" + getTemplate().getName() + "][" + getLevel()
					...
					else if (getCurrentFed() < (0.10 * getPetLevelData().getPetMaxFeed()))
					{
						SystemMessage sm = SystemMessage.getSystemMessage(
								SystemMessageId.PET_CAN_RUN_AWAY_WHEN_HUNGER_BELOW_10_PERCENT);
						getOwner().sendPacket(sm);
						if (Rnd.get(100) < 3)
						{
							stopFeed();
	...
	{
		this(objectId, template, owner, control, (byte) (template.idTemplate == 12564 ? owner.getLevel()
				: template.level));
	}
	...
	public void deleteMe(L2PcInstance owner)
	{
		...
		destroyControlItem(owner, false); // this should also delete the pet
											// from the db
	}
	...
	private static L2PetInstance restore(L2ItemInstance control, L2NpcTemplate template, L2PcInstance owner)
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT item_obj_id, name, level, curHp, curMp, exp, sp, fed FROM pets WHERE item_obj_id=?");
	...
	private void deathPenalty()
	{
		...
		long lostExp = Math.round((getStat().getExpForLevel(lvl + 1) - getStat().getExpForLevel(lvl)) * percentLost
				/ 100);
