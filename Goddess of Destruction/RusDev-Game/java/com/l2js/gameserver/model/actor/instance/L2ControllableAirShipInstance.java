...
import com.l2js.gameserver.model.actor.stat.ControllableAirShipStat;
import com.l2js.gameserver.network.SystemMessageId;
import com.l2js.gameserver.network.serverpackets.DeleteObject;
...
public class L2ControllableAirShipInstance extends L2AirShipInstance
{
			...
			if (_captain == null && player.getAirShip() == this)
			{
				...
				else if (player.isSitting())
				{
					player.sendPacket(SystemMessage.getSystemMessage(
							SystemMessageId.YOU_CANNOT_CONTROL_THE_HELM_WHILE_IN_A_SITTING_POSITION));
					return false;
				}
				else if (player.isParalyzed())
				{
					player.sendPacket(SystemMessage.getSystemMessage(
							SystemMessageId.YOU_CANNOT_CONTROL_THE_HELM_WHILE_YOU_ARE_PETRIFIED));
					return false;
				}
				...
				{
					player.sendPacket(SystemMessage.getSystemMessage(
							SystemMessageId.YOU_CANNOT_CONTROL_THE_HELM_WHILE_HOLDING_A_FLAG));
					return false;
				}
	...
	public void onSpawn()
	{
		...
		_consumeFuelTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new ConsumeFuelTask(), 60000,
				60000);
	}
