...
public final class L2SkillLearn
{
	...
	private FastList<UsablePacketSkill> prequisiteskills = new FastList<UsablePacketSkill>();
	private FastList<UsablePacketItem> requireditems = new FastList<UsablePacketItem>();

	/**
	 * Constructor for L2SkillLearn.
	 * 
	 * @param set
	 *            the set with the L2SkillLearn data.
	 */
	public L2SkillLearn(StatsSet set)
	{
		...
		if (!set.getString("preReqSkillIdLvl", "").isEmpty())
		{
			...
			{
				_log.severe(getClass().getSimpleName() + ": Malformed preReqSkillIdLvl for Learn Skill Id " + _skillId
						+ " and level " + _skillLvl + "!");
			}
		}
		else
    ...
    {
	   return prequisiteskills;
    }
	
	
	public FastList<UsablePacketItem> getRequiredItems()
    {
