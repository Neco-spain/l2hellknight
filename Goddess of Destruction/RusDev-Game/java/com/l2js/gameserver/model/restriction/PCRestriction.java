...
public class PCRestriction extends AbstractRestriction
{
	...
	private static void checkingEvent(L2PcInstance activeChar)
	{
		...
		if (Config.DM_EVENT_ENABLED) DMEvent.onLogin(activeChar);
		if (Config.LM_EVENT_ENABLED) LMEvent.onLogin(activeChar);
		if (Config.HITMAN_ENABLE_EVENT) Hitman.getInstance().onEnterWorld(activeChar);
	}
	
	private static void getCustomWelcome(L2PcInstance activeChar)
	{
		if (Config.SERVER_WELCOME_MESSAGE_ENABLE) activeChar.sendMessage(Config.SERVER_WELCOME_MESSAGE);
		if (Config.ONLINE_PLAYERS_AT_STARTUP) activeChar.sendMessage("Online Players: " + L2World.getInstance().getAllPlayersCount(activeChar.isGM()));
		if (Config.SCREEN_WELCOME_MESSAGE_ENABLE) activeChar.sendPacket(new ExShowScreenMessage(Config.SCREEN_WELCOME_MESSAGE, Config.SCREEN_WELCOME_MESSAGE_TIME));
	}
	...
	private static void sendMsg(L2PcInstance activeChar)
	{
		...
		if (Config.HERO_ANNOUNCE_LOGIN && activeChar.isHero()) Announcements.getInstance().announceToAll(Config.HERO_MSG_LOGIN.replaceAll("\\$player", nameChar));
		
		if (Config.CASTLE_LORDS_ANNOUNCE && clan != null && clan.getHasCastle() > 0)
		{
			Castle castle = CastleManager.getInstance().getCastleById(clan.getHasCastle());
			if ((castle != null) && (activeChar.getObjectId() == clan.getLeaderId())) Announcements.getInstance().announceToAll(Config.CASTLE_LORDS_MSG.replaceAll("\\$player", nameChar).replaceAll("\\$castle", castle.getName()));	
		}
	}
	...
	private void setSystemPvPColor(L2PcInstance activeChar)
	{
		...
		if (!activeChar.isGM() && Config.ALLOW_PVP_COLOR_SYSTEM)
		{
			...
			if (Config.ALLOW_PVP_COLOR_NAME)
			{
				...
				if (indexColor != null) appearance.setNameColor(colorName.get(indexColor));
			}
			...
			if (Config.ALLOW_PVP_COLOR_TITLE)
			{
				...
				if (indexColor == null) indexColor = colorTitle.lowerKey(pvpAmount);
				
				if (indexColor != null) appearance.setTitleColor(colorTitle.get(indexColor));
			}
		}
	}
	...
	private void checkIllegalEnchant(L2PcInstance activeChar)
	{
		if (!activeChar.isGM() && Config.ALLOW_VALID_ENCHANT)
		{
			...
			{
				if (i.isArmor() && i.getEnchantLevel() > Config.ENCHANT_MAX_ARMOR) actionIllegalEnchant(activeChar, i, "Armor");
				else if (i.isWeapon() && i.getEnchantLevel() > Config.ENCHANT_MAX_WEAPON) actionIllegalEnchant(activeChar, i, "Weapon");
			}
		}
	}
	...
	public static void actionIllegalEnchant(L2PcInstance activeChar, L2ItemInstance i, String cat)
	{
		if (Config.DESTROY_ENCHANT_ITEM)
		{
			...
			_log.log(Level.WARNING, "Player: " + activeChar.getName() + " use illegal item (" + cat + "). Item: " + i + " was deleted!");
		}
		else _log.log(Level.WARNING, "Player: " + activeChar.getName() + " use illegal item (" + cat + "). Item: " + i + "!");
		activeChar.sendMessage("[Server]: You have Items over enchanted!");
		if (Config.PUNISH_PLAYER) Util.handleIllegalPlayerAction(activeChar, "Player: " + activeChar.getName() + " use illegal item (" + cat + "). Item: " + i + "!", Config.DEFAULT_PUNISH);
	}
}
