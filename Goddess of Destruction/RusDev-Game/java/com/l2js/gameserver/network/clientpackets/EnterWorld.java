...
import com.l2js.Base64;
import com.l2js.Config;
import com.l2js.gameserver.*;
import com.l2js.gameserver.cache.HtmCache;
import com.l2js.gameserver.communitybbs.Manager.RegionBBSManager;
import com.l2js.gameserver.datatables.AdminCommandAccessRights;
...
import com.l2js.gameserver.model.actor.instance.L2ClassMasterInstance;
import com.l2js.gameserver.model.actor.instance.L2PcInstance;
import com.l2js.gameserver.model.entity.*;
...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		activeChar.setAdress(adress);
		
		// Multibox Protection Engine
		if (Config.MULTIBOX_PROTECTION_ENABLED)
			IPManager.getInstance().validBox(activeChar, Config.MULTIBOX_PROTECTION_CLIENTS_PER_PC,
					L2World.getInstance().getAllPlayers().values(), true);
		...
		{
			if (Config.DEBUG)
				_log.warning("User already exists in Object ID map! User " + activeChar.getName()
			...
			if (Config.ENABLE_SAFE_ADMIN_PROTECTION)
			{
				if (Config.SAFE_ADMIN_NAMES.contains(activeChar.getName()))
				{
					...
					if (Config.SAFE_ADMIN_SHOW_ADMIN_ENTER)
						_log.info("Safe Admin: " + activeChar.getName() + "(" + activeChar.getObjectId()
								+ ") has been logged in.");
				}
				else
				{
					activeChar.getPcAdmin().punishUnSafeAdmin();
					_log.warning("WARNING: Unsafe Admin: " + activeChar.getName() + "(" + activeChar.getObjectId()
							+ ") as been logged in.");
				...
				if (!clanHall.getPaid())
					activeChar
							.sendPacket(SystemMessage
									.getSystemMessage(SystemMessageId.PAYMENT_FOR_YOUR_CLAN_HALL_HAS_NOT_BEEN_MADE_PLEASE_MAKE_PAYMENT_TO_YOUR_CLAN_WAREHOUSE_BY_S1_TOMORROW));
			}
		...
		{
			float points = Config.RATE_RECOVERY_ON_RECONNECT
					* (System.currentTimeMillis() - activeChar.getLastAccess()) / 60000;
			if (points > 0)
		...
		if (Config.DISPLAY_SERVER_DEV)
		{
			activeChar.sendMessage(getText("U2VydmVyIGJ5IFJ1c0Rldg==\n"));
			activeChar.sendMessage(getText("U2l0ZTogcnVzZGV2LnBwLnVh\n"));

			if (Config.DISPLAY_SERVER_VERSION)
			{
				if (Config.SERVER_VERSION != null)
					activeChar.sendMessage(getText("UnVzRGV2IENvcmUgVmVyc2lvbjo=") + " " + Config.SERVER_VERSION);

				if (Config.DATAPACK_VERSION != null)
					activeChar.sendMessage(getText("UnVzRGV2IERQIFZlcnNpb246") + " " + Config.DATAPACK_VERSION);
			}
			activeChar.sendMessage(getText("Q29weXJpZ2h0IMKpIDIwMTE=\n"));
		}
		...
		// Attacker or spectator logging in to a siege zone. Actually should be
		// checked for inside castle only?
		if (!activeChar.isGM()
		// inside siege zone
		...
		CommunityServerThread.getInstance().sendPacket(
				new WorldInfo(activeChar, null, WorldInfo.TYPE_UPDATE_PLAYER_STATUS));
	...
	private void notifyClanMembers(L2PcInstance activeChar)
	{
		...
		// This null check may not be needed anymore since notifyClanMembers is
		// called from within a null check already. Please remove if we're
