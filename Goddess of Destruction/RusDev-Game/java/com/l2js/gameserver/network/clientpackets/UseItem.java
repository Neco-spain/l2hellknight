...
import com.l2js.gameserver.network.serverpackets.ShowCalculator;
...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.isStunned() || activeChar.isParalyzed() || activeChar.isSleeping() || activeChar.isAfraid()
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT && activeChar.getKarma() > 0)
		{
			...
			if (sHolders != null)
			{
				for (SkillHolder sHolder : sHolders)
				{
					...
					if (skill != null
							&& (skill.getSkillType() == L2SkillType.TELEPORT || skill.getSkillType() == L2SkillType.RECALL))
						return;
				}
			}
		...
		if (item.isEquipable())
		{
			...
			if (activeChar.isAttackingNow())
			{
				ThreadPoolManager.getInstance().scheduleGeneral(
						new WeaponEquipTask(item, activeChar),
						(activeChar.getAttackEndTime() - GameTimeController.getGameTicks())
