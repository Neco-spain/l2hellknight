...
import static com.l2js.gameserver.ai.CtrlIntention.AI_INTENTION_ATTACK;
import static com.l2js.gameserver.ai.CtrlIntention.AI_INTENTION_IDLE;
...
import com.l2js.gameserver.Territory;
...
import com.l2js.gameserver.model.actor.instance.*;
import com.l2js.gameserver.model.quest.Quest;
import com.l2js.gameserver.templates.chars.L2NpcTemplate;
import com.l2js.gameserver.templates.chars.L2NpcTemplate.AIType;
...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	 ...
	 * <BR>
	 * <li>The target isn't a Folk, a Door or another L2Npc</li> <li>The target
	 * isn't dead, isn't invulnerable, isn't in silent moving mode AND too far
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		{ // the actor is a L2FriendlyMobInstance
			...
			// Check if the L2PcInstance target has karma (=PK)
			if (target instanceof L2PcInstance && ((L2PcInstance) target).getKarma() > 0)
				return GeoData.getInstance().canSeeTarget(me, target); // Los
																		// Check
			else
				return false;
		}
	...
	private void thinkAttack()
	{
			...
			try
			{
				for (L2Object obj : objs)
				{
					if (obj instanceof L2Npc)
					{
						...
						// Catacomb mobs should assist lilim and nephilim other
						// than dungeon
						...
						// Lilim mobs should assist other Lilim and catacomb
						// mobs
						else if ("c_dungeon_lilim".equals(faction_id) && "c_dungeon_clan".equals(npcfaction))
							sevenSignFaction = true;
						// Nephilim mobs should assist other Nephilim and
						// catacomb mobs
						...
						if (npc.isInsideRadius(called, factionRange, true, false) && called.hasAI())
						{
							...
							// && GeoData.getInstance().canSeeTarget(called,
							// npc))
							{
								if (originalAttackTarget instanceof L2Playable)
								{
									Quest[] quests = called.getTemplate().getEventQuests(
								...
								{
									((L2Attackable) called).addDamageHate(getAttackTarget(), 0,
											npc.getHating(getAttackTarget()));
		...
		// Note from Gnacik:
		// On l2js because of that sometimes mobs don't attack player only
		// running
			...
			for (L2Object nearby : npc.getKnownList().getKnownObjects().values())
			{
						...
						&& nearby != mostHate)
				{
					...
					if (!npc.isInsideRadius(newX, newY, collision, false))
					{
						...
						if (Config.GEODATA == 0
								|| GeoData.getInstance().canMoveFromToTarget(npc.getX(), npc.getY(), npc.getZ(), newX,
										newY, newZ, npc.getInstanceId()))
		...
		if (!npc.isMovementDisabled() && npc.getCanDodge() > 0)
			if (Rnd.get(100) <= npc.getCanDodge())
			{
				...
				if (Math.sqrt(distance2) <= 60 + combinedCollision)
				{
					...
					if (Config.GEODATA == 0
							|| GeoData.getInstance().canMoveFromToTarget(npc.getX(), npc.getY(), npc.getZ(), posX,
									posY, posZ, npc.getInstanceId()))
			...
			if (_skillrender.hasHealSkill() && _skillrender._healskills != null)
			{
					...
					if (leader != null && !leader.isDead()
						...
						for (L2Skill sk : _skillrender._healskills)
						{
							...
							if (!Util.checkIfInRange(
									(sk.getCastRange() + collision + leader.getTemplate().collisionRadius), npc,
									leader, false)
				...
				for (L2Skill sk : _skillrender._healskills)
				{
					...
					if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_ONE)
						for (L2Character obj : npc.getKnownList().getKnownCharactersInRadius(
								sk.getCastRange() + collision))
						{
		...
		if ((npc.isMovementDisabled() && (dist > range || mostHate.isMoving()))
				|| (dist > range && mostHate.isMoving()))
		{
			movementDisable();
			return;
		}
	...
	private boolean cast(L2Skill sk)
	{
		...
		double range = caster.getPhysicalAttackRange() + caster.getTemplate().collisionRadius
		...
		switch (sk.getSkillType())
		{
			...
			case BUFF:
			{
				...
				// ----------------------------------------
				// If actor already have buff, start looking at others same
				// faction mob to cast
				if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_SELF)
					return false;
				if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_ONE)
			...
			case BALANCE_LIFE:
			{
				...
				if (caster.isMinion() && sk.getTargetType() != L2Skill.SkillTargetType.TARGET_SELF)
				{
					...
					{
						if (!Util
								.checkIfInRange((sk.getCastRange() + caster.getTemplate().collisionRadius + leader
										.getTemplate().collisionRadius), caster, leader, false)
								&& !isParty(sk) && !caster.isMovementDisabled())
						{
							moveToPawn(leader,
									sk.getCastRange() + caster.getTemplate().collisionRadius
				...
				if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_ONE)
					for (L2Character obj : caster.getKnownList().getKnownCharactersInRadius(
							sk.getCastRange() + caster.getTemplate().collisionRadius))
					{
						if (!(obj instanceof L2Attackable) || obj.isDead())
							continue;
				...
				{
					for (L2Character obj : caster.getKnownList().getKnownCharactersInRadius(
							sk.getSkillRadius() + caster.getTemplate().collisionRadius))
					{
			...
			case BLEED:
			{
				...
				else if (canAOE(sk))
				{
					...
					if ((sk.getTargetType() == SkillTargetType.TARGET_AREA
							|| sk.getTargetType() == SkillTargetType.TARGET_BEHIND_AREA || sk.getTargetType() == SkillTargetType.TARGET_FRONT_AREA)
	...
	private void movementDisable()
	{
		...
		try
		{
			...
			if (_skillrender.hasSkill())
			{
					...
					for (L2Skill sk : _skillrender._immobiliseskills)
					{
						if (sk.getMpConsume() >= npc.getCurrentMp()
								...
								|| (sk.getCastRange() + npc.getTemplate().collisionRadius
										+ getAttackTarget().getTemplate().collisionRadius <= dist2 && !canAura(sk))
								|| (sk.isMagic() && npc.isMuted()) || (!sk.isMagic() && npc.isPhysicalMuted()))
						{
							continue;
						}
				 ...
				 * ||(sk.getCastRange()+ _actor.getTemplate().collisionRadius +
				 * getAttackTarget().getTemplate().collisionRadius <= dist2 &&
				 * !canAura(sk))
