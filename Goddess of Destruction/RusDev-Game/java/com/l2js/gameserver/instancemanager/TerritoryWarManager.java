...
import com.l2js.gameserver.datatables.ClanTable;
import com.l2js.gameserver.datatables.NpcTable;
...
import com.l2js.gameserver.model.L2SiegeClan;
...
import com.l2js.gameserver.model.TerritoryWard;
...
import com.l2js.gameserver.model.entity.Siegable;
import com.l2js.gameserver.model.quest.Quest;
import com.l2js.gameserver.network.SystemMessageId;
import com.l2js.gameserver.network.serverpackets.L2GameServerPacket;
...
public class TerritoryWarManager implements Siegable
{
	...
	public void registerMerc(int castleId, L2PcInstance player)
	{
		if (player == null
				...
				|| (_registeredMercenaries.get(castleId) != null && _registeredMercenaries.get(castleId).contains(
						player.getObjectId())))
			return;
	...
	public void removeMerc(int castleId, L2PcInstance player)
	{
		if (player == null)
			return;
				...
				&& _registeredMercenaries.get(castleId).contains(player.getObjectId()))
		{
			_registeredMercenaries.get(castleId).remove(
					_registeredMercenaries.get(castleId).indexOf(player.getObjectId()));
		...
		if (_territoryList.get(newOwnerId) != null)
		{
			...
			if (ward != null)
			{
				...
				if (terNew.getOwnerClan() != null && terNew.getOwnedWardIds().contains(newOwnerId + 80))
				{
					...
					{
						final FastList<L2SkillLearn> residentialSkills = SkillTreesData.getInstance()
								.getAvailableResidentialSkills(wardId);
			...
			if (_territoryList.containsKey(oldOwnerId))
			{
				...
				if (broadcastMessage)
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.CLAN_S1_HAS_SUCCEDED_IN_CAPTURING_S2_TERRITORY_WARD);
				...
				{
					final FastList<L2SkillLearn> territorySkills = SkillTreesData.getInstance()
							.getAvailableResidentialSkills(territoryId);
					...
					if (!terOld.getOwnedWardIds().isEmpty() && !terOld.getOwnedWardIds().contains(oldOwnerId + 80))
					{
						...
						{
							final FastList<L2SkillLearn> wardSkills = SkillTreesData.getInstance()
									.getAvailableResidentialSkills(wardId);
	...
	public void dropCombatFlag(L2PcInstance player, boolean isKilled, boolean isSpawnBack)
	{
		for (TerritoryWard twWard : _territoryWards)
			if (twWard.playerId == player.getObjectId())
			{
				...
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.THE_CHAR_THAT_ACQUIRED_S1_WARD_HAS_BEEN_KILLED);
	...
	public void giveTWPoint(L2PcInstance killer, int victimSide, int type)
	{
		...
		if (killer.getParty() != null && type < 5)
			...
			{
				if (pl.getSiegeSide() == victimSide || pl.getSiegeSide() == 0
	...
	public int[] calcReward(L2PcInstance player)
	{
		if (_participantPoints.containsKey(player.getObjectId()))
		{
			...
			// than he cant get rewards(also this will handle that player
			// already get his/her rewards)
			if (temp[6] < 10)
				return reward;
	...
	private void changeRegistration(int castleId, int objId, boolean delete)
	{
		...
		try
		{
			...
			if (delete)
				statement = con
						.prepareStatement("DELETE FROM territory_registrations WHERE castleId=? and registeredId=?");
			else
				statement = con
						.prepareStatement("INSERT INTO territory_registrations (castleId, registeredId) values (?,?)");
	...
	private final void load()
	{
		...
		try
		{
			...
			PLAYER_WITH_WARD_CAN_BE_KILLED_IN_PEACEZONE = Boolean.parseBoolean(territoryWarSettings.getProperty(
					"PlayerWithWardCanBeKilledInPeaceZone", "False"));
			SPAWN_WARDS_WHEN_TW_IS_NOT_IN_PROGRESS = Boolean.parseBoolean(territoryWarSettings.getProperty(
					"SpawnWardsWhenTWIsNotInProgress", "False"));
			RETURN_WARDS_WHEN_TW_STARTS = Boolean.parseBoolean(territoryWarSettings.getProperty(
					"ReturnWardsWhenTWStarts", "False"));
			MINTWBADGEFORNOBLESS = Integer.decode(territoryWarSettings
					.getProperty("MinTerritoryBadgeForNobless", "100"));
			MINTWBADGEFORSTRIDERS = Integer.decode(territoryWarSettings.getProperty("MinTerritoryBadgeForStriders",
					"50"));
			MINTWBADGEFORBIGSTRIDER = Integer.decode(territoryWarSettings.getProperty("MinTerritoryBadgeForBigStrider",
					"80"));
			...
			try
			{
				...
				while (rs.next())
				{
					...
					switch (spawnType)
					{
						...
						case 2: // castle npcs
							_territoryList.get(castleId).getSpawnList()
									.add(new TerritoryNPCSpawn(castleId, loc, npcId, spawnType, null));
							break;
						case 3: // ward spawns
			...
			try
			{
				...
				while (rs.next())
				{
					...
					if (t != null)
					{
						...
						{
							t.setOwnerClan(ClanTable.getInstance().getClan(
									CastleManager.getInstance().getCastleById(castleId).getOwnerId()));
	...
	private void startTerritoryWar()
	{
		...
		// MapRegionTable.TeleportWhereType.Town); // Teleport to the closest
		// town
		for (Territory t : activeTerritoryList)
		{
			...
			for (TerritoryNPCSpawn ward : t.getOwnedWard())
				if (ward.getNpc() != null && t.getOwnerClan() != null)
				{
					...
					_territoryWards.add(new TerritoryWard(ward.getNpcId(), ward.getLocation().getX(), ward
							.getLocation().getY(), ward.getLocation().getZ(), 0, ward.getNpcId() + 13479, t
							.getCastleId(), ward.getNpc()));
				}
		...
		if (RETURN_WARDS_WHEN_TW_STARTS)
			for (TerritoryWard ward : _territoryWards)
				...
				{
					ward.unSpawnMe();
					ward.setNpc(addTerritoryWard(ward.getTerritoryId(), ward.getTerritoryId() - 80,
							ward.getOwnerCastleId(), false));
		...
		public void run()
		{
			...
			try
			{
				...
				if (timeRemaining > 7200000)
				{
					...
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining - 7200000); // Prepare task for 2h
														// before TW start to
														// end registration
				}
				...
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.THE_TERRITORY_WAR_REGISTERING_PERIOD_ENDED);
					Announcements.getInstance().announceToAll(sm);
					_isRegistrationOver = true;
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining - 1200000); // Prepare task for 20
														// mins left before TW
														// start.
				}
				...
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.TERRITORY_WAR_BEGINS_IN_20_MINUTES);
					Announcements.getInstance().announceToAll(sm);
					_isTWChannelOpen = true;
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining - 600000); // Prepare task for 10 mins
														// left before TW start.
				}
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.TERRITORY_WAR_BEGINS_IN_10_MINUTES);
					Announcements.getInstance().announceToAll(sm);
					_isTWChannelOpen = true;
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining - 300000); // Prepare task for 5 mins
														// left before TW start.
				}
				else if ((timeRemaining <= 300000) && (timeRemaining > 60000))
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.TERRITORY_WAR_BEGINS_IN_5_MINUTES);
					Announcements.getInstance().announceToAll(sm);
					_isTWChannelOpen = true;
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining - 60000); // Prepare task for 1 min
													// left before TW start.
				}
				else if ((timeRemaining <= 60000) && (timeRemaining > 0))
				{
					...
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(),
							timeRemaining); // Prepare task for TW start.
				}
				else if ((timeRemaining + WARLENGTH) > 0)
				{
					...
					_isRegistrationOver = true;
					startTerritoryWar();
					_scheduledEndTWTask = ThreadPoolManager.getInstance()
							.scheduleGeneral(new ScheduleEndTWTask(), 1000); // Prepare
																				// task
																				// for
																				// TW
																				// end.
					_scheduledRewardOnlineTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(
							new RewardOnlineParticipants(), 60000, 60000);
				}
			}
			catch (Exception e)
			{
	...
	private class ScheduleEndTWTask implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_scheduledEndTWTask.cancel(false);
				long timeRemaining = _startTWDate.getTimeInMillis() + WARLENGTH
						- Calendar.getInstance().getTimeInMillis();
				if (timeRemaining > 3600000)
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.THE_TERRITORY_WAR_WILL_END_IN_S1_HOURS);
					sm.addNumber(2);
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(),
				...
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.THE_TERRITORY_WAR_WILL_END_IN_S1_MINUTES);
				...
				{
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.S1_SECONDS_TO_THE_END_OF_TERRITORY_WAR);
					sm.addNumber(Math.round(timeRemaining / 1000));
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(),
							timeRemaining); // Prepare task for second count
											// down
				}
				else
				{
					endTerritoryWar();
					// _scheduledStartTWTask =
					// ThreadPoolManager.getInstance().scheduleGeneral(new
					// ScheduleStartTWTask(), 1000);
	...
	public class Territory
	{
		...
		private TerritoryNPCSpawn getFreeWardSpawnPlace()
		{
			...
			for (int i = 0; i < _territoryWardSpawnPlaces.length; i++)
				if (_territoryWardSpawnPlaces[i] == null)
					...
					_log.log(Level.WARNING, "TerritoryWarManager: territory ward spawn place " + i + " has npc name: "
							+ _territoryWardSpawnPlaces[i].getNpc().getName());
				else
		...
		private void removeWard(int wardId)
		{
			...
			_log.log(Level.WARNING, "TerritoryWarManager: cant delete wardId: " + wardId + " for territory: "
					+ _territoryId);
		}
