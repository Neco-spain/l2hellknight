...
public class ChatTrade implements IChatHandler
{
		...
		if (isValid(activeChar) || activeChar.isGM())
		{
			...
			if (!isValidPunished(activeChar) && !activeChar.isGM())
			{
				activeChar.sendMessage("You were punished for using this chat. (-"
						+ String.valueOf(Config.PUNISHED_AMOUNT_PVP_CHAT_TRADE) + ")");
	...
	{
		if (Config.ENABLE_PVP_CHAT_TRADE_BLOCK && Config.PUNISHED_AMOUNT_PVP_CHAT_TRADE > 0
				&& Config.AMOUNT_PVP_CHAT_TRADE > Config.PUNISHED_AMOUNT_PVP_CHAT_TRADE)
			return false;
		return true;
	}
