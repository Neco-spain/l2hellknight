...
import com.l2js.gameserver.Item;
...
import com.l2js.gameserver.model.actor.instance.L2EventMonsterInstance;
import com.l2js.gameserver.model.actor.instance.L2PcInstance;
import com.l2js.gameserver.skills.SkillsEngine;
import com.l2js.gameserver.templates.item.*;
import com.l2js.gameserver.util.GMAudit;
...
public class ItemTable
{
	 ...
	 * @param reference
	 *            : Object Object referencing current action like NPC selling
	...
	public L2ItemInstance createItem(String process, int itemId, long count, L2PcInstance actor, Object reference)
	{
		...
		if (process.equalsIgnoreCase("loot"))
		{
			...
			if (reference instanceof L2Attackable && ((L2Attackable) reference).isRaid()) // loot
																							// privilege
																							// for
																							// raids
			{
				...
				if (raid.getFirstCommandChannelAttacked() != null && !Config.AUTO_LOOT_RAIDS)
				{
					...
					itemLootShedule = ThreadPoolManager.getInstance().scheduleGeneral(new ResetOwner(item),
							Config.LOOT_RAIDS_PRIVILEGE_INTERVAL);
					item.setItemLootShedule(itemLootShedule);
				}
			}
			else if (!Config.AUTO_LOOT
					|| (reference instanceof L2EventMonsterInstance && ((L2EventMonsterInstance) reference)
							.eventDropOnGround()))
			{
		...
		if (actor != null)
		{
			if (actor.isGM())
			{
				...
				{
					referenceName = (((L2Object) reference).getName() != null ? ((L2Object) reference).getName()
							: "no-name");
				}
		...
		synchronized (item)
		{
			...
			if (actor != null)
			{
				if (actor.isGM())
				{
					...
					String targetName = (actor.getTarget() != null ? actor.getTarget().getName() : "no-target");
					if (Config.GMAUDIT)
						GMAudit.auditGMAction(actor.getName() + " [" + actor.getObjectId() + "]", process + "(id: "
								+ item.getItemId() + " count: " + item.getCount() + " itemObjId: " + item.getObjectId()
