...
import com.l2js.gameserver.model.L2SummonItem;
...
public class CharSummonTable
{
	...
	public void saveSummon(L2SummonInstance summon)
	{
		...
		{
			_log.log(Level.SEVERE, "Failed to store summon [SummonId: " + summon.getNpcId() + "] from Char [CharId: "
					+ summon.getOwner().getObjectId() + "] data", e);
		}
		finally
		{
			L2DatabaseFactory.close(con);
	...
	public void restoreServitor(L2PcInstance activeChar)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				 ...
				 * summon = new L2MerchantSummonInstance(IdFactory.getInstance().getNextId(), summonTemplate,
				 * activeChar, skill);
				 */
				else
					summon = new L2SummonInstance(IdFactory.getInstance().getNextId(), summonTemplate, activeChar,
							skill);
				...
				if (summon.getLevel() >= Experience.PET_MAX_LEVEL)
				{
					...
					_log.warning("Summon (" + summon.getName() + ") NpcID: " + summon.getNpcId()
							+ " has a level above 86. Please rectify.");
				}
				else
				{
