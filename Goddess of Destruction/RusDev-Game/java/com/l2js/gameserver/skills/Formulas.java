...
public final class Formulas
{
			...
			boolean crit, boolean dual, boolean ss)
	{
		...
		if (crit)
		{
			...
			damage = 2 * attacker.calcStat(Stats.CRITICAL_DAMAGE, 1, target, skill)
					* target.calcStat(Stats.CRIT_VULN, target.getTemplate().baseCritVuln, target, null)
					* (70 * damage / defence);
	...
	{
		return activeChar.calcStat(Stats.BLOW_RATE, chance * (1.0 + (activeChar.getDEX() - 20) / 100), target, null) > Rnd
				.get(100);
	}
				...
				&& !(target instanceof L2Npc && ((L2Npc) target).getNpcId() == 35062))
		{
					...
					&& Rnd.get(1000) < calcLethal(activeChar, target, skill.getLethalChance2(), skill.getMagicLevel()))
			{
															...
															// and CP to 1
				{
					...
					if (!player.isInvul())
					{
						if (!(activeChar instanceof L2PcInstance && (((L2PcInstance) activeChar).isGM() && !((L2PcInstance) activeChar)
								.getAccessLevel().canGiveDamage())))
						{
					...
					if (!player.isInvul())
					{
								...
								.getAccessLevel().canGiveDamage())))
						{
							...
							player.sendPacket(SystemMessage.getSystemMessage(
									SystemMessageId.CP_DISAPPEARS_WHEN_HIT_WITH_A_HALF_KILL_SKILL));
	...
	public static double calcSkillTypeProficiency(double multiplier, L2Character attacker, L2Character target,
			L2SkillType type)
	{
		if (type != null)
		{
	...
	public static boolean calcEffectSuccess(L2Character attacker, L2Character target, EffectTemplate effect,
			L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		if (attacker.isDebug() || Config.DEVELOPER)
		{
			...
			StringUtil.append(stat, skill.getName(), " eff.type:", type.toString(), " power:", String.valueOf(value),
					" stat:", String.format("%1.2f", statModifier), " res:", String.format("%1.2f", resMod), "(",
					String.format("%1.2f", profModifier), "/", String.format("%1.2f", vulnModifier), ") elem:",
					String.valueOf(elementModifier), " mAtk:", String.format("%1.2f", mAtkModifier), " ss:",
					String.valueOf(ssModifier), " lvl:", String.valueOf(deltamod), " total:", String.valueOf(rate));
			...
			boolean ss, boolean sps, boolean bss)
	{
		...
		if (attacker.isDebug() || Config.DEVELOPER)
		{
			...
			StringUtil.append(stat, skill.getName(), " type:", skill.getSkillType().toString(), " power:",
					...
					String.format("%1.2f", vulnModifier), ") elem:", String.valueOf(elementModifier), " mAtk:",
					String.format("%1.2f", mAtkModifier), " ss:", String.valueOf(ssModifier), " lvl:",
	...
	public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, L2Character target, L2Skill skill, byte shld)
	{
		...
		if (attacker.getOwner().isDebug() || Config.DEVELOPER)
		{
			...
			StringUtil.append(stat, skill.getName(), " type:", skill.getSkillType().toString(), " power:",
					...
					String.format("%1.2f", resMod), "(", String.format("%1.2f", profModifier), "/",
					String.format("%1.2f", vulnModifier), ") elem:", String.valueOf(elementModifier), " mAtk:",
					String.format("%1.2f", mAtkModifier), " lvl:", String.valueOf(deltamod), " total:",
					String.valueOf(rate));
			final String result = stat.toString();
	...
	public static double calcElemental(L2Character attacker, L2Character target, L2Skill skill)
	{
		...
		else
		{
			...
			if (element >= 0)
			{
				calcTotal = Math.max(attacker.getAttackElementValue(element) - target.getDefenseElementValue(element),
						0);
	...
	public static byte calcSkillReflect(L2Character target, L2Skill skill)
	{
		...
		switch (skill.getSkillType())
		{
			...
			case MANADAM:
			case CPDAMPERCENT:
				final Stats stat = skill.isMagic() ? Stats.VENGEANCE_SKILL_MAGIC_DAMAGE
						: Stats.VENGEANCE_SKILL_PHYSICAL_DAMAGE;
		...
		final double reflectChance = target.calcStat(skill.isMagic() ? Stats.REFLECT_SKILL_MAGIC
				: Stats.REFLECT_SKILL_PHYSIC, 0, null, skill);
