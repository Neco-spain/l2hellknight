...
import l2.hellknight.gameserver.network.serverpackets.ExSendUIEvent;
import l2.hellknight.gameserver.network.serverpackets.ExShowScreenMessage2;
...
public class IceQueenCastle2 extends Quest
{
        ...
        */
        ...
        private boolean checkConditions(L2PcInstance player)
        {
                ...
                if (_isHard)
                {
                        if (player.getParty().getCommandChannel().getMemberCount() < ExternalConfig.MIN_FREYA_HC_PLAYERS)
                        {
                                player.getParty().getCommandChannel().broadcastPacket(SystemMessage.getSystemMessage(2793).addNumber(10));
                                return false;
                        }
                        
                        if (player.getParty().getCommandChannel().getMemberCount() > ExternalConfig.MAX_FREYA_HC_PLAYERS)
                        {
                ...
                {
                        if (player.getParty().getCommandChannel().getMemberCount() < ExternalConfig.MIN_FREYA_PLAYERS)
                        {
                                player.getParty().getCommandChannel().broadcastPacket(SystemMessage.getSystemMessage(2793).addNumber(10));
                                return false;
                        }
                        
                        if (player.getParty().getCommandChannel().getMemberCount() > ExternalConfig.MAX_FREYA_PLAYERS)
                        {
                                player.getParty().getCommandChannel().broadcastPacket(SystemMessage.getSystemMessage(2102));
                                return false;
                        }
                ...
                for (L2PcInstance partyMember : player.getParty().getCommandChannel().getMembers())
                {
                        ...
                        if(_isHard)
                        {
                                if (partyMember.getLevel() < ExternalConfig.MIN_LEVEL_HC_PLAYERS)
                                {
                        ...
                        {
                                if (partyMember.getLevel() < ExternalConfig.MIN_LEVEL_PLAYERS)
                                {
                                        SystemMessage sm = SystemMessage.getSystemMessage(2097);
                                        sm.addPcName(partyMember);
                                        player.getParty().getCommandChannel().broadcastPacket(sm);
