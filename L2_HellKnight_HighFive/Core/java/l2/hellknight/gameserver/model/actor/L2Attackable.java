...
import l2.hellknight.gameserver.datatables.EventDroplist.DateDrop;
...
import l2.hellknight.gameserver.model.actor.knownlist.AttackableKnownList;
import l2.hellknight.gameserver.model.actor.status.AttackableStatus;
...
public class L2Attackable extends L2Npc
{
				...
				for (RewardInfo reward : rewards.values())
				{
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							tmp = calculateExpAndSp(levelDiff, damage, attacker.isPremium());
							exp = tmp[0];
							exp *= 1 - penalty;
							sp = tmp[1];
					...
					else
					{
						...
						// Calculate Exp and SP rewards
						tmp = calculateExpAndSp(levelDiff, partyDmg, attacker.isPremium());
						exp_premium = tmp[0];
						sp_premium = tmp[1];
						exp = tmp[0];
						sp = tmp[1];
	...
	/**
	 * 
	 * @see l2.hellknight.gameserver.model.actor.L2Character#addAttackerToAttackByList(l2.hellknight.gameserver.model.actor.L2Character)
	 */
	@Override
	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if (lastAttacker.isPremium())
			dropChance *= ExternalConfig.PR_RATE_DROP_ITEMS_ID.get(drop.getItemId());
		else
			dropChance *= Config.RATE_DROP_ITEMS_ID.get(drop.getItemId());
		else if (isSweep)
		if (lastAttacker.isPremium())
			dropChance *= ExternalConfig.PREMIUM_RATE_DROP_SPOIL;
		else
			dropChance *= Config.RATE_DROP_SPOIL;
		else
		if (lastAttacker.isPremium())
			dropChance *= isRaid() && !isRaidMinion() ? ExternalConfig.PREMIUM_RATE_DROP_ITEMS_BY_RAID : ExternalConfig.PREMIUM_RATE_DROP_ITEMS;
		else
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (lastAttacker.getPremiumService()==1)
			categoryDropChance *= isRaid() && !isRaidMinion() ? ExternalConfig.PREMIUM_RATE_DROP_ITEMS_BY_RAID : ExternalConfig.PREMIUM_RATE_DROP_ITEMS;
		else
