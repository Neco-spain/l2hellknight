...
import l2.hellknight.gameserver.model.L2ExtractableProductItem;
import l2.hellknight.gameserver.model.L2ExtractableSkill;
	...
	*/
	...
	public static final boolean checkForAreaFriendlySkills(L2PcInstance activeChar, L2Character target, L2Skill skill)
	{
		...
		if (target instanceof L2Playable)
		{
			...
			if (activeChar.isInOlympiadMode() && pTarget.isInOlympiadMode()
					&& activeChar.getOlympiadSide() != pTarget.getOlympiadSide()
					&& activeChar.isOlympiadStart())
				return false;
			
			if (activeChar.getSiegeState() > 0 && activeChar.isInsideZone(L2Character.ZONE_SIEGE)
					&& activeChar.getSiegeState() != pTarget.getSiegeState()
					&& pTarget.getSiegeSide() != activeChar.getSiegeSide())
				return false;

			
			if (!TvTEvent.checkForTvTSkill(activeChar, pTarget, skill))
				return false;
			...
			if (pTarget.isAutoAttackable(activeChar) || pTarget.isInsideZone(L2Character.ZONE_PVP) || activeChar.isInSameClanWar(pTarget) || pTarget.getKarma() != 0 || pTarget.getPvpFlag() != 0)
				return false;
