...
public class TvTRoundEvent
{
	...
	public static void init()
	{
		...
		_teams[0] = new TvTRoundEventTeam(ExternalConfig.TVT_ROUND_EVENT_TEAM_1_NAME, ExternalConfig.TVT_ROUND_EVENT_TEAM_1_COORDINATES);
		_teams[1] = new TvTRoundEventTeam(ExternalConfig.TVT_ROUND_EVENT_TEAM_2_NAME, ExternalConfig.TVT_ROUND_EVENT_TEAM_2_COORDINATES);
	}
	
	/**
	 * Starts the participation of the TvTRoundEvent<br>
	 * 1. Get L2NpcTemplate by ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_ID<br>
	...
	{
		L2NpcTemplate tmpl = NpcTable.getInstance().getTemplate(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_ID);
		
		if (tmpl == null)
		{
			_log.warning("TvTRoundEventEngine[TvTRoundEvent.startParticipation()]: L2NpcTemplate is a NullPointer -> Invalid npc id in ExternalConfigs?");
			return false;
		}
		...
		try
		{
			...
			_npcSpawn.setLocx(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[0]);
			_npcSpawn.setLocy(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[1]);
			_npcSpawn.setLocz(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[2]);
			_npcSpawn.setAmount(1);
			_npcSpawn.setHeading(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	 ...
	 * 3. Close doors specified in ExternalConfigs<br>
	 * 4. Open the anteroom doors specified in ExternalConfigs<br>
	...
	public static boolean startEvent()
	{
		...
		if (_teams[0].getParticipatedPlayerCount() < ExternalConfig.TVT_ROUND_EVENT_MIN_PLAYERS_IN_TEAMS || _teams[1].getParticipatedPlayerCount() < ExternalConfig.TVT_ROUND_EVENT_MIN_PLAYERS_IN_TEAMS)
		{
			// Set state INACTIVE
		...
		if (ExternalConfig.TVT_ROUND_EVENT_IN_INSTANCE)
		{
			try
			{
				_TvTRoundEventInstance = InstanceManager.getInstance().createDynamicInstance(ExternalConfig.TVT_ROUND_EVENT_INSTANCE_FILE);
				...
				InstanceManager.getInstance().getInstance(_TvTRoundEventInstance).setEmptyDestroyTime(ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY * 1000 + 60000L);
			}
			catch (Exception e)
			{
		...
		// Opens all doors specified in ExternalConfigs for tvt round
		openDoors(ExternalConfig.TVT_ROUND_DOORS_IDS_TO_OPEN);
		// Closes all doors specified in ExternalConfigs for tvt round
		closeDoors(ExternalConfig.TVT_ROUND_DOORS_IDS_TO_CLOSE);
		// Open the doors after a ExternalConfigurable time
		openAnteroomDoors();
	 ...
	 * 2. Open the anteroom doors specified in ExternalConfigs<br>
	...
	{
		int TvTRoundWaitOpenAnteroomDoors = (ExternalConfig.TVT_ROUND_EVENT_WAIT_OPEN_ANTEROOM_DOORS + ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY);
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
		{
			@Override
			public void run()
			{
				// Open all doors specified in the ExternalConfig
				openDoors(ExternalConfig.TVT_ROUND_ANTEROOM_DOORS_IDS_TO_OPEN_CLOSE);
				closeAnteroomDoors();
			}
	...
	{
		int TvTRoundWaitCloseAnteroomDoors = (ExternalConfig.TVT_ROUND_EVENT_WAIT_CLOSE_ANTEROOM_DOORS + ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY);
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
		{
			@Override
			public void run()
			{
				// Close all doors specified in the ExternalConfig
				closeDoors(ExternalConfig.TVT_ROUND_ANTEROOM_DOORS_IDS_TO_OPEN_CLOSE);
			}
	...
	public static String calculatePoints()
	{
		if (_teams[0].getPoints() == _teams[1].getPoints())
		{
			...
			if (ExternalConfig.TVT_ROUND_GIVE_POINT_TEAM_TIE)
			{
				_teams[0].increaseRoundPoints();
				_teams[1].increaseRoundPoints();
				if (ExternalConfig.TVT_ROUND_EVENT_STOP_ON_TIE)
					addRoundTie();
	...
	public static String calculateRewards()
	{
		if (_teams[0].getRoundPoints() == _teams[1].getRoundPoints())
		{
			...
			if (ExternalConfig.TVT_ROUND_REWARD_TEAM_TIE)
			{
				rewardTeam(_teams[0]);
				rewardTeam(_teams[1]);
	...
	private static void rewardTeam(TvTRoundEventTeam team)
	{
		...
		for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
		{
			...
			for (int[] reward : ExternalConfig.TVT_ROUND_EVENT_REWARDS)
			{
	 ...
	 * 3. Open doors specified in ExternalConfigs<br>
	...
	public static void stopEvent()
	{
		...
		unSpawnNpc();
		// Opens all doors specified in ExternalConfigs for tvt round
		openDoors(ExternalConfig.TVT_ROUND_DOORS_IDS_TO_CLOSE);
		// Closes all doors specified in ExternalConfigs for tvt round
		closeDoors(ExternalConfig.TVT_ROUND_DOORS_IDS_TO_OPEN);
		...
		for (TvTRoundEventTeam team : _teams)
		{
			for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
			{
				...
				if (playerInstance != null)
				{
					new TvTRoundEventTeleporter(playerInstance, ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
				}
			}
		}
	 ...
	 *
	 * @param playerObjectId as int<br>
	 * @return boolean: true if success, otherwise false<br>
	 */
	...
	{
		return ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[0] != 0 && ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[1] != 0;
	}
	
	public static boolean hasParticipationFee(L2PcInstance playerInstance)
	{
		return playerInstance.getInventory().getInventoryItemCount(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[0], -1) >= ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[1];
	}
	
	public static boolean payParticipationFee(L2PcInstance playerInstance)
	{
		return playerInstance.destroyItemByItemId("TvT Participation Fee", ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[0], ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[1], _lastNpcSpawn, true);
	}
	
	public static String getParticipationFee()
	{
		int itemId = ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[0];
		int itemNum = ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_FEE[1];
	...
	/**
	 * Close doors specified in ExternalConfigs
	 * @param doors 
	 */
	...
	/**
	 * Open doors specified in ExternalConfigs
	 * @param doors 
	 */
	...
	public static void onLogout(L2PcInstance playerInstance)
	{
		if (playerInstance != null && (isStarting() || isStarted() || isParticipating()))
		{
			if (removeParticipant(playerInstance.getObjectId()))
				playerInstance.setXYZInvisible(ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101)-50,
						ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101)-50,
						ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_NPC_COORDINATES[2]);
		}
	}
	...
	public static synchronized void onBypass(String command, L2PcInstance playerInstance)
	{
		...
		if (command.equals("tvt_round_event_participation"))
		{
			...
			else if (playerLevel < ExternalConfig.TVT_ROUND_EVENT_MIN_LVL || playerLevel > ExternalConfig.TVT_ROUND_EVENT_MAX_LVL)
			{
				htmContent = HtmCache.getInstance().getHtm(playerInstance.getHtmlPrefix(), htmlPath+"Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%roundmin%", String.valueOf(ExternalConfig.TVT_ROUND_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%roundmax%", String.valueOf(ExternalConfig.TVT_ROUND_EVENT_MAX_LVL));
				}
			}
			else if (_teams[0].getParticipatedPlayerCount() == ExternalConfig.TVT_ROUND_EVENT_MAX_PLAYERS_IN_TEAMS && _teams[1].getParticipatedPlayerCount() == ExternalConfig.TVT_ROUND_EVENT_MAX_PLAYERS_IN_TEAMS)
			{
				htmContent = HtmCache.getInstance().getHtm(playerInstance.getHtmlPrefix(), htmlPath+"TeamsFull.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%roundmax%", String.valueOf(ExternalConfig.TVT_ROUND_EVENT_MAX_PLAYERS_IN_TEAMS));
				}
			}
			else if (ExternalConfig.TVT_ROUND_EVENT_MAX_PARTICIPANTS_PER_IP > 0
					&& !AntiFeedManager.getInstance().tryAddPlayer(AntiFeedManager.TVT_ROUND_ID, playerInstance, ExternalConfig.TVT_ROUND_EVENT_MAX_PARTICIPANTS_PER_IP))
			{
				htmContent = HtmCache.getInstance().getHtm(playerInstance.getHtmlPrefix(), htmlPath+"IPRestriction.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%roundmax%", String.valueOf(AntiFeedManager.getInstance().getLimit(playerInstance, ExternalConfig.TVT_ROUND_EVENT_MAX_PARTICIPANTS_PER_IP)));
				}
			}
		...
		else if (command.equals("tvt_round_event_remove_participation"))
		{
			...
			if (ExternalConfig.TVT_ROUND_EVENT_MAX_PARTICIPANTS_PER_IP > 0)
	 ...
	 *
	 * @param playerInstance as L2PcInstance<br>
	 * @param targetedPlayerObjectId as int<br>
	...
	public static boolean onAction(L2PcInstance playerInstance, int targetedPlayerObjectId)
	{
		...
		if (playerTeamId != -1 && targetedPlayerTeamId != -1 && playerTeamId == targetedPlayerTeamId && playerInstance.getObjectId() != targetedPlayerObjectId && !ExternalConfig.TVT_ROUND_EVENT_TARGET_TEAM_MEMBERS_ALLOWED)
		{
			return false;
		}
	...
	public static boolean onScrollUse(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_ROUND_EVENT_SCROLL_ALLOWED)
			return false;
	...
	public static boolean onPotionUse(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_ROUND_EVENT_POTIONS_ALLOWED)
			return false;
	...
	public static boolean onItemSummon(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_ROUND_EVENT_SUMMON_BY_ITEM_ALLOWED)
		{
			return false;
		}
	...
	public static void onKill(L2Character killerCharacter, L2PcInstance killedPlayerInstance)
	{
		...
		if (ExternalConfig.TVT_ROUND_EVENT_ON_DIE)
	...
	public static void onTeleported(L2PcInstance playerInstance)
	{
		...
		{
			if (ExternalConfig.TVT_ROUND_EVENT_MAGE_BUFFS != null && !ExternalConfig.TVT_ROUND_EVENT_MAGE_BUFFS.isEmpty())
			{
				for (int i : ExternalConfig.TVT_ROUND_EVENT_MAGE_BUFFS.keys())
				{
					L2Skill skill = SkillTable.getInstance().getInfo(i, ExternalConfig.TVT_ROUND_EVENT_MAGE_BUFFS.get(i));
					if (skill != null)
		...
		{
			if (ExternalConfig.TVT_ROUND_EVENT_FIGHTER_BUFFS != null && !ExternalConfig.TVT_ROUND_EVENT_FIGHTER_BUFFS.isEmpty())
			{
				for (int i : ExternalConfig.TVT_ROUND_EVENT_FIGHTER_BUFFS.keys())
				{
					L2Skill skill = SkillTable.getInstance().getInfo(i, ExternalConfig.TVT_ROUND_EVENT_FIGHTER_BUFFS.get(i));
					if (skill != null)
						skill.getEffects(playerInstance, playerInstance);
				}
			}
	 ...
	 *
	 * @param playerObjectId as Integer<br>
