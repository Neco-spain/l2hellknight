...
public class TvTRoundEventTeleporter implements Runnable
{
	...
	public TvTRoundEventTeleporter(L2PcInstance playerInstance, int[] coordinates, boolean fastSchedule, boolean adminRemove)
	{
		...
		long delay = ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY * 1000;
	...
	public void run()
	{
		...
		if (ExternalConfig.TVT_ROUND_EVENT_EFFECTS_REMOVAL == 0
				|| (ExternalConfig.TVT_ROUND_EVENT_EFFECTS_REMOVAL == 1 && (_playerInstance.getTeam() == 0 || (_playerInstance.isInDuel() && _playerInstance.getDuelState() != Duel.DUELSTATE_INTERRUPTED))))
