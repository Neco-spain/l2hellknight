...
public class TvTRoundManager
{
	...
	{
		if (ExternalConfig.TVT_ROUND_EVENT_ENABLED)
		{
			TvTRoundEvent.init();
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ExternalConfig.TVT_ROUND_EVENT_INTERVAL)
			{
		...
		{
			_log.warning("TvTRoundEventEngine[TvTRoundManager.scheduleEventStart()]: Error figuring out a start time. Check TvTRoundEventInterval in ExternalConfig file.");
		}
	}
	...
	public void startReg()
	{
		...
		{
			Announcements.getInstance().announceToAll("TvT Round Event: Registration opened for " + ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_TIME
					+ " minute(s).");
			
			// schedule registration end
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_ROUND_EVENT_PARTICIPATION_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void startFirstRound()
	{
		...
		else
		{
			...
			TvTRoundEvent.sysMsgToAllParticipants("TvT Round Event: Teleporting participants to the anteroom in "
					+ ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_ROUND_EVENT_FIRST_FIGHT_RUNNING_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void startSecondRound()
	{
		...
		else
		{
			...
			TvTRoundEvent.sysMsgToAllParticipants("TvT Round Event: Teleporting participants to the anteroom in "
					+ ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_ROUND_EVENT_SECOND_FIGHT_RUNNING_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void startThirdRound()
	{
		...
		else
		{
			...
			TvTRoundEvent.sysMsgToAllParticipants("TvT Round Event: Teleporting participants to the anteroom in "
					+ ExternalConfig.TVT_ROUND_EVENT_START_RESPAWN_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_ROUND_EVENT_THIRD_FIGHT_RUNNING_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void endFirstRound()
	{
		...
		if (ExternalConfig.TVT_ROUND_EVENT_STOP_ON_TIE && TvTRoundEvent.getRoundTie() >= ExternalConfig.TVT_ROUND_EVENT_MINIMUM_TIE)
		{
	...
	public void endSecondRound()
	{
		...
		else if (ExternalConfig.TVT_ROUND_EVENT_REWARD_ON_SECOND_FIGHT_END && TvTRoundEvent.checkForPossibleWinner())
		{
