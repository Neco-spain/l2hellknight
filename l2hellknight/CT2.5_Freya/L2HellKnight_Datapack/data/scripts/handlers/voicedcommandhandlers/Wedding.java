...
public class Wedding implements IVoicedCommandHandler
{
	...
	public boolean goToLove(L2PcInstance activeChar)
	{
		...
		else if (!DM.onEscapeUse(activeChar.getObjectId()))
		{
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		else if (!LMEvent.onEscapeUse(activeChar.getObjectId()))
		{
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		...
		else if (!DM.onEscapeUse(partner.getObjectId()))
		{
			activeChar.sendMessage("Your partner is in an event.");
			return false;
		}
		else if (!LMEvent.onEscapeUse(partner.getObjectId()))
		{
			activeChar.sendMessage("Your partner is in an event.");
			return false;
		}
		...
		Scenkos.toSelfAndKnownPlayersInRadius(activeChar, msk, 810000/*900*/);
