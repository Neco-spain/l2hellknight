 ...
 */
package l2.hellknight.gameserver.model.olympiad;
...
public class OlympiadManager
{
	...
	private final boolean checkNoble(L2PcInstance noble, L2PcInstance player)
	{
		...
		if (LMEvent.isPlayerParticipant(noble.getObjectId()))
		{
			player.sendMessage("You can't join olympiad while participating on LM Event.");
			return false;
		}
		
		if (DM.isPlayerParticipant(noble.getObjectId()))
		{
			player.sendMessage("You can't join olympiad while participating on DM Event.");
			return false;
		}
		
		if (noble.isInEventDM())
		{
			noble.sendMessage("You can't join olympiad while participating on DM Event.");
			return false;
		}
