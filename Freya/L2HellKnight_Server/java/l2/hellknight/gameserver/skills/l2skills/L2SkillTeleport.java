...
public class L2SkillTeleport extends L2Skill
{
	...
	public void useSkill(L2Character activeChar, L2Object[] targets)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			if (!LMEvent.onEscapeUse(((L2PcInstance) activeChar).getObjectId()))
			{
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
			...
			if (!DM.onEscapeUse(((L2PcInstance) activeChar).getObjectId()))
			{
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		...
		try
		{
			for (L2Character target: (L2Character[]) targets)
			{
				if (target instanceof L2PcInstance)
				{
					...
					// Check to see if player is in DM event
					...
					//Kratei's Cube
					if (((L2PcInstance) targetChar).getIsInKrateisCube())
					{
						targetChar.sendMessage("You may not escape from Kratei's Cube!");
						continue;
					}
					...
					if (targetChar != activeChar)
					{
						...
						if (!DM.onEscapeUse(targetChar.getObjectId()))
							continue;
						
						if (!LMEvent.onEscapeUse(targetChar.getObjectId()))
							continue;
