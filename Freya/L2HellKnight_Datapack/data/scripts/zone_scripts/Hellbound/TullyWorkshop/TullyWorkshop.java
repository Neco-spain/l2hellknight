...
package zone_scripts.Hellbound.TullyWorkshop;
...
import l2.hellknight.gameserver.instancemanager.RaidBossSpawnManager.StatusEnum;
...
import l2.hellknight.gameserver.model.zone.type.L2DamageZone;
...
public class TullyWorkshop extends Quest
{
	...
	private static final int[] SIN_WARDENS = { 22423, 22431 };
	...
	//Items
	private static final int[] REWARDS = { 10427, 10428, 10429, 10430, 10431 };
	
	private static final int[] DEATH_COUNTS = { 7, 10 }; //7 for 6th floor and 10 for 8'th floor
	...
	private static final int[] TELEPORTING_MONSTERS = { 22377, 22378, 22379,  22383 };
	...
	private List<L2MonsterInstance> spawnedFollowers = new FastList<L2MonsterInstance>();
	private List<L2MonsterInstance> spawnedFollowerMinions = new FastList<L2MonsterInstance>();
	private L2Npc spawnedAgent = null;
	private L2Spawn pillarSpawn = null;

	private int[][] deathCount = new int[2][4];
	...
	{
		{200012, 200013, 200014, 200015},	//6'th floor
		{200016, 200017, 200018, 200019}	//8'th floor
	};

	private static final int[][] AGENT_COORDINATES = 
	{
		{ -13312, 279172, -13599, -20300}, //6'th floor room 1
		{ -11696, 280208, -13599, 13244}, //6'th floor room 2
		{ -13008, 280496, -13599, 27480}, //6'th floor room 3
		{ -11984, 278880, -13599, -4472}, //6'th floor room 4
		{ -13312, 279172, -10492, -20300}, //8'th floor room 1
		{ -11696, 280208, -10492, 13244}, //8'th floor room 2
		{ -13008, 280496, -10492, 27480}, //8'th floor room 3
		{ -11984, 278880, -10492, -4472} //8'th floor room 4
	};

	private static final int[][] SERVANT_COORDINATES = 
	{
		{ -13214, 278493, -13601, 0}, //6'th floor room 1
		{ -11727, 280711, -13601, 0}, //6'th floor room 2
		{ -13562, 280175, -13601, 0}, //6'th floor room 3
		{ -11514, 278592, -13601, 0}, //6'th floor room 4
		{ -13370, 278459, -10497, 0}, //8'th floor room 1
		{ -11984, 280894, -10497, 0}, //8'th floor room 2
		{ -14050, 280312, -10497, 0}, //8'th floor room 3
		{ -11559, 278725, -10495, 0} //8'th floor room 4
	};

	private static final int[][] CUBE_68_TELEPORTS = 
	{
		{ -12176, 279696, -13596 }, //to 6'th floor
		{ -12176, 279696, -10492 }, //to 8'th floor
	...
	static
	{
		...
		TULLY_DOORLIST.put(99999, new int[] { 19260019 }); //18455, second instance with x == -14610
	...
	public final String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if (npcId == AGENT)
		{
			...
			if (roomData[0] < 0 || roomData[1] < 0) //npc is located on floor 7 (or more exactly - not located on floors 6th or 8th)
				return "32372-02.htm";
			else
				return "32372-01.htm";
		}
		...
		{
			if (npc.isInsideRadius(-12752, 279696, -13596, 100, true, false)) //6th floor - "mode == 1"
				return "32467-01.htm";
			
			else if (npc.isInsideRadius(-12752, 279696, -10492, 100, true, false)) //8th floor - "mode == 2"
				return "32467-02.htm";
		...
		else if (npcId == TOMBSTONE)
		{
			...
			return haveItem ? "32344-01.htm" : "32344-01a.htm"; 
		}
  ...
  public String onTalk ( L2Npc npc, L2PcInstance player)
	{
		if (npc.getNpcId() == TOMBSTONE)
		{
			...
			boolean[] haveItems = { false, false, false, false, false };
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			if (npc.getAI().getIntention() != CtrlIntention.AI_INTENTION_ATTACK && npc.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST &&
		...
		else if (event.equalsIgnoreCase("enter") && npcId == DORIAN)
		{
				L2Party party = player.getParty();
		...
		else if ((event.equalsIgnoreCase("up") || event.equalsIgnoreCase("down")) && TELE_COORDS.containsKey(npcId))
		{
			...
			if (party == null)
				...
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.TOO_FAR_FROM_NPC));
	...
	/**
	 * Spawns Crystall Pillar (we can't track initial spawn if it done from spawntable)
	 * and opens / closes doors according to Darion's state
	**/	 	 
	...
	private void handleDoorsOnDeath()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] { 20250006, 20250007 }, STATE_OPEN), 2000);
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] { 20250778 }, STATE_CLOSE), 3000);
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] { 20250777 }, STATE_CLOSE), 6000);
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] { 20250009, 20250008 }, STATE_OPEN), 11000);
	}
	...
	private void handleDoorsOnRespawn()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] {20250777, 20250778 }, STATE_OPEN), 1000);
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[] {20250005, 20250004, 20250006,  20250007}, STATE_CLOSE), 4000);			
	}
	...
	{
		new TullyWorkshop(-1, TullyWorkshop.class.getSimpleName(), "zone_scripts/Hellbound");
		if (Config.ENABLE_LOADING_INFO_FOR_SCRIPTS)
			_log.info("Loaded Hellbound: Tully Workshop");
	}
}
