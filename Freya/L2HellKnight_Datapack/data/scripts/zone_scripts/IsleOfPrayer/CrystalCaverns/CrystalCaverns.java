...
//		public           Map<L2DoorInstance, L2PcInstance> openedDoors    = new FastMap<L2DoorInstance, L2PcInstance>();
	...
	private static final int[][] ordreOracle1 = {
		{147090, 152505, -12169, 31613},
		{147090, 152575, -12169, 31613},
		{147090, 152645, -12169, 31613},
		{147090, 152715, -12169, 31613}
	};

	private static final int[][] ordreOracle2 = {
		{149783, 152505, -12169, 31613},
		{149783, 152575, -12169, 31613},
		{149783, 152645, -12169, 31613},
		{149783, 152715, -12169, 31613}
	};

	private static final int[][] ordreOracle3 = {
		{152461, 152505, -12169, 31613},
		{152461, 152575, -12169, 31613},
		{152461, 152645, -12169, 31613},
		{152461, 152715, -12169, 31613}
	};
	...
	//private static final int INSTANCEPENALTY = 1; //add for test Alergy
	...
	protected void runFirst(CCWorld world)
	{
		...
		for (L2Npc npc : world.keyKeepers)
		{
	...
	protected void runSteamOracles(CCWorld world, int[][] oracleOrder, int oracleId)
	{
		int rndOracle = Rnd.get(1, 4);
		int i = 1;
		int oracleI;
		world.oracles.clear();
		for(int[] oracle : oracleOrder)
		{
			if (i == rndOracle)
				...
				oracleI = 32274;
			world.oracles.put(addSpawn(oracleI, oracle[0], oracle[1], oracle[2], oracle[3], false,0,false,world.instanceId), null);
			i++;
		}		
	}
	...
	public String onAdvEvent (String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			else if (event.equalsIgnoreCase("auto1Food"))
			{
				if (!world.crystalGolems.containsKey(npc))
					world.crystalGolems.put(npc, new CrystalGolem());
				if (world.status != 3  || !world.crystalGolems.containsKey(npc)
				...
				if (minDist != 300000)
					startQuestTimer("get1Food",2000,npc,null);
				else
				{
					if (Rnd.get(100) < 5)
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(),1,npc.getName(),"ah ... I am so hungry ..."));
					startQuestTimer("auto1Food",2000,npc,null);
				}
				return "";
			}			
			...
			else if (event.equalsIgnoreCase("back1Food"))
			{
				if (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)
				{
					cancelQuestTimers("back1Food");
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE, null);
					world.crystalGolems.get(npc).foodItem = null;
				}
			}			
			...
			else if (event.equalsIgnoreCase("reach1Food"))
			{
				CrystalGolem cryGolem = world.crystalGolems.get(npc);
				int dx;
				int dy;
				...
				{
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, cryGolem.oldpos);
					cancelQuestTimers("reach1Food");
					startQuestTimer("back1Food", 2000, npc, null, true);
					return "";
				}
				else if (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)
				{
			...
			else if (event.equalsIgnoreCase("get1Food"))
			{
				CrystalGolem cryGolem = world.crystalGolems.get(npc);
				L2CharPosition newpos = new L2CharPosition(cryGolem.foodItem.getX(), cryGolem.foodItem.getY(), cryGolem.foodItem.getZ(), 0);
				cryGolem.oldpos = new L2CharPosition(npc.getX(), npc.getY(), npc.getZ(), npc.getHeading());
				npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, newpos);
				startQuestTimer("reach1Food", 2000, npc, null, true);
				cancelQuestTimers("get1Food");
			}		
		}
		return "";
	}
	...
	public String onKill( L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			if (world.status == 2 && world.npcList1.containsKey(npc))
			{
				...
				L2Npc cGolem1 = addSpawn(CRYSTAL_GOLEM,140547,151670,-11813,32767,false,0,false,world.instanceId);
				L2Npc cGolem2 = addSpawn(CRYSTAL_GOLEM,141941,151684,-11813,63371,false,0,false,world.instanceId);
				world.crystalGolems.put(cGolem1,new CrystalGolem());
				world.crystalGolems.put(cGolem2,new CrystalGolem());
				startQuestTimer("autoFood",2000,cGolem1,null);
				startQuestTimer("auto1Food",2000,cGolem2,null);
			}
			...
			else if (world.status == 2 && world.keyKeepers.contains(npc))
			{
				if (npc.getNpcId() == GK1)
				{	
					player.addItem("CrystalCaverns", 9698, 1, null, true);
					//giveRewards(player, npc.getInstanceId(), 9698, false);
					//((L2MonsterInstance)npc).dropItem(player, 9698, 1);
					runEmeraldSquare(world);
				}
				else if (npc.getNpcId() == GK2)
				{
					player.addItem("CrystalCaverns", 9699, 1, null, true);
					//giveRewards(player, npc.getInstanceId(), 9699, false);
			...
			else if (world.status >= 22 && world.status <= 25)
			{
				...
				if (checkKillProgress(0, npc, world))
				{
					...
					runSteamOracles(world, oracleOrder, oracleId);
				}
			}
		...
		if (character instanceof L2PcInstance)
		{
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				if (world.status == 8)
				{
					int[][] spawns;
					int room = 0;
					L2PcInstance acticeCharacter = (L2PcInstance)character;
					switch(zone.getId())
					{
					case 20105:
						spawns = ROOM2_SPAWNS;
					...
					if (room == 0)
						return "";					
					QuestState st = acticeCharacter.getQuestState(qn);
					if (st == null)
						st = newQuestState((L2PcInstance)character);
					if (st.getQuestItemsCount(RACE_KEY) == 0)
						return "";					
					acticeCharacter.setCrystalCavernsZoneDoor(room);					
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				if (world.status == 8)
				{
					...
					for (L2DoorInstance door : InstanceManager.getInstance().getInstance(world.instanceId).getDoors())
						if (door.getDoorId() == doorId)
						{
							if (door.getOpen() && doorId == acticeCharacter.getCrystalCavernsDoor())
							{
								door.closeMe();								
								acticeCharacter.setCrystalCavernsZoneDoor(0);
							}
							break;
						}
	...
	{
		new CrystalCaverns(-1,qn,"zone_scripts/IsleOfPrayer");
		if (Config.ENABLE_LOADING_INFO_FOR_SCRIPTS)
			_log.info("Loaded Isle of Prayer: Crystal Caverns");
	}
}