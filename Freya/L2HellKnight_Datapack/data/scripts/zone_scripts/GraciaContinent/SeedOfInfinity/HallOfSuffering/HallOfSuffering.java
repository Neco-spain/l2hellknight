package zone_scripts.GraciaContinent.SeedOfInfinity.HallOfSuffering;
...
public class HallOfSuffering extends Quest
{
	...
	private class HoSWorld extends InstanceWorld
	{
	...
	private static final boolean debug = Config.HOS_DEBUG; //false
	...
	// default: 24h
	private static final int INSTANCEPENALTY = Config.HOS_INS_PENALTY;
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if (partyMember.getLevel() < Config.HOS_MIN_LEVEL) //75
			{
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		else
		{
			...
			world.status = 0;
			((HoSWorld)world).startTime = System.currentTimeMillis();
			((HoSWorld)world).ptLeaderName = player.getName();
			InstanceManager.getInstance().addWorld(world);
			_log.info("Hall Of Suffering started " + template + " Instance: " + instanceId + " created by player: " + player.getName());
			runTumors((HoSWorld)world);
	...
	protected boolean checkKillProgress(L2Npc mob, HoSWorld world)
	{
	...
	protected void runTumors(HoSWorld world)
	{
	...
	protected void runTwins(HoSWorld world)
	{
		world.status++;
	...
	private void calcRewardItemId(HoSWorld world)
	{
	...
	private String getPtLeaderText(L2PcInstance player, HoSWorld world)
	{
		String htmltext = HtmCache.getInstance().getHtm(player.getHtmlPrefix(),"/data/scripts/zone_scripts/GraciaContinent/SeedOfInfinity/HallOfSuffering/32530-10.htm");
	...
	public String onAdvEvent (String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof HoSWorld)
		{
			HoSWorld world = (HoSWorld) tmpworld;
	...
	public String onAttack (L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		if (tmpworld instanceof HoSWorld)
		{
			if (!((HoSWorld)tmpworld).isBossesAttacked)
			{
				((HoSWorld) tmpworld).isBossesAttacked = true;
			...
			{
				if (((HoSWorld)tmpworld).klanikus.isDead())
				{
					((HoSWorld)tmpworld).klanikus.setIsDead(false);
					((HoSWorld)tmpworld).klanikus.doDie(attacker);
					((HoSWorld)tmpworld).klodekus.doDie(attacker);
				}
				else if (((HoSWorld)tmpworld).klodekus.isDead())
				{
					((HoSWorld)tmpworld).klodekus.setIsDead(false);
					((HoSWorld)tmpworld).klodekus.doDie(attacker);
					((HoSWorld)tmpworld).klanikus.doDie(attacker);
				}
				else
	...
	public static void main(String[] args)
	{
		...
		new HallOfSuffering(-1,qn,"zone_scripts/GraciaContinent/SeedOfInfinity");
		if (Config.ENABLE_LOADING_INFO_FOR_SCRIPTS)
			_log.info("Loaded Gracia Area: Hall of Suffering");
	}
}