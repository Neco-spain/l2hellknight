...
import l2.brick.gameserver.datatables.SubPledgeSkillTree.SubUnitSkill;
import l2.brick.gameserver.instancemanager.InstanceManager;
import l2.brick.gameserver.model.L2SquadTrainer;
...
import l2.brick.gameserver.network.serverpackets.AcquireSkillList.SkillType;
...
public class L2CastleMagicianInstance extends L2NpcInstance implements L2SquadTrainer
{
	...
	private static final boolean validateGateCondition(L2PcInstance clanLeader, L2PcInstance player)
	{
		...
		if (!DM.onEscapeUse(player.getObjectId()))
		{
			player.sendMessage("You on DM Event, teleporting disabled.");
			return false;
		}
		
		if (!DM.onEscapeUse(clanLeader.getObjectId()))
		{
			// Need retail message if there's one.
			player.sendMessage("Couldn't teleport to clan leader. The requirements was not meet.");
			return false;
	...
	/* (non-Javadoc)
	 * @see l2.brick.gameserver.model.actor.L2SquadTrainer#showSubUnitSkillList(l2.brick.gameserver.model.actor.instance.L2PcInstance)
	 */
	@Override
