...
public class DMEventTeleporter implements Runnable
{
	...
	{
		long delay = (DM.isStarted() ? Config.DM_EVENT_RESPAWN_TELEPORT_DELAY : Config.DM_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000;
	...
	public void run()
	{
		...
		int DMInstance = DM.getDMEventInstance();
		if (DMInstance != 0)
		{
			if (DM.isStarted() && !_adminRemove)
