...
import l2rt.loginserver.crypt.Crypt;
...
public class LoginController
{
	...
	private LoginController() throws GeneralSecurityException
	{
		...
		try
		{
			DEFAULT_CRYPT = (Crypt) Class.forName("l2rt.loginserver.crypt." + Config.DEFAULT_PASSWORD_ENCODING).getMethod("getInstance", new Class[0]).invoke(null, new Object[0]);
			...
			for(String method : Config.LEGACY_PASSWORD_ENCODING.split(";"))
				if(!method.equalsIgnoreCase(Config.DEFAULT_PASSWORD_ENCODING))
					legacy.add((Crypt) Class.forName("l2rt.loginserver.crypt." + method).getMethod("getInstance", new Class[0]).invoke(null, new Object[0]));
		...
		try
		{
			...
			if(rset.next())
			{
				...
				if(rset.getInt("activated") == 1)
				{
                    ...
                    ok.setPoint(rset.getInt("points"));
				}
				else
				{
					//TODO System.out.println("activated != 1");
    ...
    public void rePoint(String name, L2LoginClient client) throws SQLException
    {
        ...
        statement.setInt(1, client.getPointL());
