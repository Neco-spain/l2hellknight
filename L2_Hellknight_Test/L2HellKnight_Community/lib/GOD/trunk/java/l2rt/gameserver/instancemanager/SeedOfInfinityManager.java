...
public class SeedOfInfinityManager
{
	...
	private static final Location[] MouthLocs = new Location[] { new Location(-183002, 205944, -12896),
			new Location(-183586, 205947, -12896), new Location(-183589, 206510, -12896),
	...
	private static final Location[] gatherTPLoc = new Location[] { new Location(-179537, 209551, -15504),
			new Location(-179779, 212540, -15520), new Location(-177028, 211135, -15520),
			new Location(-176355, 208043, -15520), new Location(-179284, 205990, -15520),
			new Location(-182268, 208218, -15520), new Location(-182069, 211140, -15520),
			new Location(-176036, 210002, -11948), new Location(-176039, 208203, -11949),
			new Location(-183288, 208205, -11939), new Location(-183290, 210004, -11939),
			new Location(-187776, 205696, -9536), new Location(-186327, 208286, -9536), new Location(-184429, 211155, -9536),
			new Location(-182811, 213871, -9504), new Location(-180921, 216789, -9536), new Location(-177264, 217760, -9536),
	...
	private static void CheckStage()
	{
		switch(getCurrentCycle())
		{
			...
			case 2:
				...
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(3), 3600000);
				break;
			case 3:
				ServerVariables.set("SeedOfInfinity_nextCycleTime", System.currentTimeMillis() + 86400000);
				Util.cancelFuture(false, new Future[] { thrid });
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(4), 86400000);
				break;
			case 4:
				int i = Rnd.get(172800000, 345600000);
				ServerVariables.set("SeedOfInfinity_nextCycleTime", System.currentTimeMillis() + i);
				Util.cancelFuture(false, new Future[] { thrid });
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(5), i);
				break;
			default:
				...
				Util.cancelFuture(false, new Future[] { thrid });
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(1), j);
				break;
		}
	}
		...
		if(getCurrentCycle() > 2)
		{
			if(l < System.currentTimeMillis())
			{
				if(getCurrentCycle() >= 5)
					thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(1), 500);
				else
					thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(getCurrentCycle() + 1), 500);
			}
			else if(getCurrentCycle() >= 5)
			{
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(1), l - System.currentTimeMillis());
			}
			else
			{
				SwitchCond(getCurrentCycle());
				thrid = ThreadPoolManager.getInstance().scheduleGeneral(new SwitchStage(getCurrentCycle() + 1), l - System.currentTimeMillis());
			}
	...
	private static class SwitchStage implements Runnable
	{
		int newStage;
