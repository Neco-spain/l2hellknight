	...
	public boolean CanBid(L2Player bidder)
	{
		...
		if(bidder_clan == null || bidder_clan.getLeaderId() != bidder.getObjectId() || bidder_clan.getLevel() < getMinClanLevel())
		{
			if(getMinClanLevel() == 2)
				...
				bidder.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.Auction.MinClanLevel", bidder).addNumber(getMinClanLevel()));
			return false;
		}
		...
		{
			bidder.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.Auction.MinClanMembers", bidder).addNumber(getMinClanMembers()));
			return false;
		}
		...
		if(getMinClanMembersAvgLevel() > 1)
		{
			...
			{
				bidder.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.Auction.MinClanMembersAvgLevel", bidder).addNumber(getMinClanMembersAvgLevel()).addNumber((long) Math.ceil(avg_level)));
				return false;
			}
		}
