...
//dddddSddddQfddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhddddddddddddddddddddddffffddddSdddcccddhhddddccdccddhhhhhhhhdd
public class GMViewCharacterInfo extends L2GameServerPacket
{
	...
	private int _patk, _patkspd, _pdef, evasion, accuracy, crit, _matk, _matkspd, _mCritRate;
	...
	public GMViewCharacterInfo(final L2Player cha)
	{
		...
		_mCritRate = (int)cha.getMagicCriticalRate(null, null);
	}
	...
	protected final void writeImpl()
	{
		...
		writeD(level);
		writeQ(_exp);
                writeF((float) (_exp - Experience.LEVEL[level]) / (Experience.LEVEL[level + 1] - Experience.LEVEL[level]));
		writeD(_str);
		writeD(_dex);
		writeD(_con);
		writeD(_int);
		...
		writeD(_patkspd);
		writeD(_mdef);
		writeD(0x00);		// mEvasion    //GOD
                writeD(0x00);       // mAccuracy   //GOD
                writeD(_mCritRate);       // mCritRate   //GOD
		writeD(pvp_flag);
		writeD(karma);
		writeD(_runSpd);
		writeD(_walkSpd);
