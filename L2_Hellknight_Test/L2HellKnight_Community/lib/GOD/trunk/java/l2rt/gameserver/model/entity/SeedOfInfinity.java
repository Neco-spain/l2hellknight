...
public class SeedOfInfinity extends Quest
{
	...
	protected static final int[][][] waves_spawn = new int[][][] { { { 22509, 2 }, { 22510, 3 } },
			{ { 22509, 1 }, { 22510, 3 }, { 22511, 3 } }, { { 22509, 2 }, { 22510, 2 }, { 22511, 2 }, { 22512, 3 } },
			{ { 22513, 2 }, { 22511, 2 }, { 22512, 2 }, { 22514, 3 } },
			{ { 22513, 2 }, { 22512, 3 }, { 22514, 3 }, { 22515, 2 } } };
	protected static final int[] MOBS = new int[] { 22509, 22510, 22511, 22512, 22513, 22514, 22515 };
	protected static final int[] SUPPLIES = new int[] { 0, 13777, 13778, 13779, 13780, 13781, 13782, 13783, 13784, 13785,
			13786 };
	protected static final int[] monsters = new int[] { 22516, 22517, 22518, 22519, 22520, 22521, 22522, 22524, 22526,
			22528, 22530, 22532, 22534 };
	...
	protected static final Location[] mark_cohemenes_loactions = new Location[] {
			new Location(-178418, 211653, -12029, 49151), new Location(-178417, 206558, -12032, 16384),
			new Location(-180911, 206551, -12028, 16384), new Location(-180911, 211652, -12028, 49151) };
	protected static final Location[] tumor_death_locations = new Location[] {
			new Location(-176036, 210002, -11948, 36863), new Location(-176039, 208203, -11949, 28672),
			new Location(-183288, 208205, -11939, 4096), new Location(-183290, 210004, -11939, 61439) };
	protected static final Location[] tumors_locations = new Location[] { new Location(-179779, 212540, -15520, 49151),
			new Location(-177028, 211135, -15520, 36863), new Location(-176355, 208043, -15520, 28672),
			new Location(-179284, 205990, -15520, 16384), new Location(-182268, 208218, -15520, 4096),
	...
	protected static L2Party getParty(long id)
	{
		return getParty(ReflectionTable.getInstance().get(id));
	}

	protected HallofErosionWorld getHallofErosionWorld(long id)
	{
		return (HallofErosionWorld) setWorld(id);
	}

	protected HeartOfInfinityWorld getHeartOfInfinityWorld(long id)
	{
		return (HeartOfInfinityWorld) setWorld(id);
	}

	private World setWorld(long id)
	{
		World world = null;
		lock.lock();
		try
	...
	protected static L2CommandChannel getCommandChannel(long id)
	{
	...
	protected Reflection enterCommandChannelInstance(L2Player player, int id, World world)
	{
	...
	protected static void endInstance(L2Party player, int time)
	{
		if(player.getReflection() != null)
			player.getReflection().startCollapseTimer(time * 1000);
	}

	protected HallofSufferingWorld getHallofSufferingWorld(long id)
	{
	...
	protected class InitialDelayTask implements Runnable
	{
		...
		public InitialDelayTask(L2Player player)
		{
			storeId = player.getStoredId().longValue();
		}
	...
	@Deprecated
	protected class BuffMobs implements Runnable
	{
		L2NpcInstance actor;
		int radius;
		int recTime;
		...
		public void run()
		{
			if(actor != null && !actor.isDead())
			{
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new BuffMobs(actor, radius, recTime), recTime);
			}
		}
	}
	...
	protected class TimeLimitCheck implements Runnable
	{
		...
		public void run()
		{
			...
			else
			{
				...
				_hiw.remainingTimeTask = ThreadPoolManager.getInstance().scheduleGeneral(new TimeLimitCheck(_hiw, _channel, _messeg), 300000);
			}
		}
	}
		...
		public void run()
		{
			if(!_hew.raidboss_spawned)
			{
				...
				else if(_messeg != null)
				{
					...
					_hew.remainingTimeTask = ThreadPoolManager.getInstance().scheduleGeneral(new TimeRemaining(_hew, _channel, _messeg), 300000);
				}
			}
		}
	}
