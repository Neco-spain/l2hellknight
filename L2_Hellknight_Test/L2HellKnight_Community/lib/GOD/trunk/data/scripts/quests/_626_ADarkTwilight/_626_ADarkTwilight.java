...
public class _626_ADarkTwilight extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("rew_exp"))
		{
			...
			st.addExpAndSp(162773 * ConfigSystem.getInt("RateQuestsReward"), 12500 * ConfigSystem.getInt("RateQuestsReward"), true);
		...
		{
			st.takeItems(BloodOfSaint, -1);
			st.giveItems(ADENA_ID, 100000 * ConfigSystem.getInt("RateQuestsReward"), true);
			htmltext = "dark_presbyter_q0626_0202.htm";
			st.exitCurrentQuest(true);
		}
		return htmltext;
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(BloodOfSaint, ConfigSystem.getInt("RateQuestsDrop"));
			if(st.getQuestItemsCount(BloodOfSaint) == 300)
				st.set("cond", "2");
		}
		return null;
