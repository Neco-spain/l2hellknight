...
public class _698_BlocktheLordsEscape extends SeedOfInfinity implements ScriptFile
{
	...
	public void initialInstance(L2Player player)
	{
		...
		world.bossSpawnTask = ThreadPoolManager.getInstance().scheduleGeneral(new SpawnBoss(world), world.boss_respawn_time * 1000);

		// стартуем таск на проверку лимита по времени
		world.timer = 25; // 25 мин даётся
		world.remainingTimeTask = ThreadPoolManager.getInstance().scheduleGeneral(new TimeLimitCheck(world, commandChannel, "Heart of Infinity Defend %time% minute(s) are remaining."), time_limit);
	...
	public String onKill(L2NpcInstance npc, QuestState qs)
	{
		...
		if(npcId == TUMOR_OF_DEATH2)
		{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new RespawnTumor(world, npc.getLoc(), addSpawnToInstance(DESTROYED_TUMOR2, npc.getLoc(), 0, world.instanceId)), TIME_TUMOR_RESPAWN * 1000);
		}
	...
	private class SpawnBoss implements Runnable
	{
