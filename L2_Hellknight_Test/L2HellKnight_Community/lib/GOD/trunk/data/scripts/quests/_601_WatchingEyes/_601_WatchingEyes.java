...
public class _601_WatchingEyes extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("eye_of_argos_q0601_0201.htm"))
		{
			...
			st.giveItems(ADENA_ID, (adena * ConfigSystem.getInt("RateQuestsReward")), true);
			if(item != 0)
			{
				st.giveItems(item, (5 * ConfigSystem.getInt("RateQuestsReward")), true);
				st.addExpAndSp((120000 * ConfigSystem.getInt("RateQuestsReward")), (10000 * ConfigSystem.getInt("RateQuestsReward")), true);
			}
    ...
    public String onKill(L2NpcInstance npc, QuestState st)
    {
        if(st.getInt("cond") == 1)
        {
            st.rollAndGive(PROOF_OF_AVENGER, ConfigSystem.getInt("RateQuestsDrop"), ConfigSystem.getInt("RateQuestsDrop"), 100, 50);
