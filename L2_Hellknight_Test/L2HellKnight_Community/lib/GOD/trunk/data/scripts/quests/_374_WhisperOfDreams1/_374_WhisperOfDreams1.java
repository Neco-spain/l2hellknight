...
public class _374_WhisperOfDreams1 extends Quest implements ScriptFile
{
	...
	private String render_shop()
	{
		...
		for(int i = 0; i < SHOP_LIST.length; i++)
		{
			...
			html += "<tr><td><a action=\"bypass -h Quest _374_WhisperOfDreams1 " + SHOP_PRICES[i][0] + "\"><font color=\"B09878\">" + (int) (SHOP_PRICES[i][2] * Config.getRateAdena(null)) + " adena</font></a></td></tr></table></td></tr>";
		}
	...
	public _374_WhisperOfDreams1()
	{
		...
		addKillId(new int[] { CB, DW });
		addQuestItem(new int[] { CB_TOOTH, DW_LIGHT });
	}
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(Util.isNumber(event))
		{
			...
			for(int[] element : SHOP_PRICES)
				if(evt == element[0])
				{
					int adena = (int) (element[2] * Config.getRateAdena(null));
