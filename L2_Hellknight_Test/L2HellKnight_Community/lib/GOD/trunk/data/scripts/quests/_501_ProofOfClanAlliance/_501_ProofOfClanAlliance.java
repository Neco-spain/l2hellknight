	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		if(st.getPlayer().isClanLeader())
		{
			...
			else if(event.equalsIgnoreCase("30759-07.htm"))
			{
				...
				st.getPlayer().setImobilised(true);
		...
		else if(event.equalsIgnoreCase("30757-04.htm"))
		{
			...
			if(Rnd.chance(50))
				st.getPlayer().reduceCurrentHp(st.getPlayer().getCurrentHp() * 8, st.getPlayer(), null, true, true, false, false);
