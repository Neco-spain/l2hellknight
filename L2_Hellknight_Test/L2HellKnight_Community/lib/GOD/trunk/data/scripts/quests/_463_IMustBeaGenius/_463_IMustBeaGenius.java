...
public class _463_IMustBeaGenius extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		if (npc.getNpcId() == _gutenhagen)
		{
			...
			else if (event.equalsIgnoreCase("32069-07.htm"))
			{
		        ...
		        st.getPlayer().setVar("NextQuest463", "" + (reDo.getTimeInMillis()));
			}
		}
		return htmltext;
	}
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		if (npcId == _gutenhagen)
		{
			if(id == CREATED)
            {
                String req = st.getPlayer().getVar("NextQuest463") == null ? "0" : st.getPlayer().getVar("NextQuest463");
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if (id == STARTED && cond == 1 && arrayContains(_mobs, npc.getNpcId()))
		{
			...
			if(_number > 0)
			{
				...
				npc.broadcastPacket(new NpcSay(npc, 0, "Att... attack... "+st.getPlayer().getName()+"... Ro... rogue... "+_number+".."));
			}
