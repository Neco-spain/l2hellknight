...
public class _251_NoSecrets extends Quest implements ScriptFile
{
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if(id == STARTED)
		{
            if(cond == 1)
            {
                if((npcId == MOB[0] || npcId == MOB[1]) && st.getQuestItemsCount(timetable) < 5 && Rnd.chance(5))
                {
                    st.giveItems(timetable, 1, false);
                    st.playSound(SOUND_ITEMGET);
                }
                if((npcId == MOB1[0] || npcId == MOB1[1] || npcId == MOB1[2] || npcId == MOB1[3] || npcId == MOB1[4]) && st.getQuestItemsCount(diary) < 10 && Rnd.chance(10))
                {
