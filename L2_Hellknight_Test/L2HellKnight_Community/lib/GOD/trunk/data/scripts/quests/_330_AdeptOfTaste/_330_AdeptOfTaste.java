...
public class _330_AdeptOfTaste extends Quest implements ScriptFile
{
	...
	private static final int[] ingredients = { Red_Mandragora_Sap, Honey, Dionian_Potato, Green_Moss_Bundle,
			Meat_of_Monster_Eye };
	private static final int[] spec_ingredients = { White_Mandragora_Sap, Golden_Honey, Brown_Moss_Bundle };
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Hobgoblin && st.getQuestItemsCount(Panos_Contract) > 0)
			st.rollAndGive(Hobgoblin_Amulet, 1, 1, 30, 100, true);
			...
			st.rollAndGive(Body_of_Monster_Eye, 1, 3, 30, 97, true);
		else if(npcId == Monster_Eye_Gazer && st.getQuestItemsCount(Rollants_Creature_Book) > 0)
			st.rollAndGive(Body_of_Monster_Eye, 1, 2, 30, 100, true);
	...
	private static void MandragoraDrop(QuestState st, int i1, int i2)
	{
		int i = Rnd.get(100);
		if(i < i1)
			st.rollAndGive(Red_Mandragora_Root, 1, 1, 40, 100, true);
		else if(i < i2)
			st.rollAndGive(White_Mandragora_Root, 1, 1, 40, 100, true);
	}
	...
	private static void BeeDrop(QuestState st, int i1, int i2)
	{
		int i = Rnd.get(100);
		if(i < i1)
			st.rollAndGive(Nectar, 1, 1, 20, 100, true);
		else if(i < i2)
			st.rollAndGive(Royal_Jelly, 1, 1, 10, 100, true);
	}
	...
	private static void AntDrop(QuestState st, int i1, int i2)
	{
		int i = Rnd.get(100);
		if(i < i1)
			st.rollAndGive(Green_Marsh_Moss, 1, 1, 20, 100, true);
		else if(i < i2)
			st.rollAndGive(Brown_Marsh_Moss, 1, 1, 20, 100, true);
	}
