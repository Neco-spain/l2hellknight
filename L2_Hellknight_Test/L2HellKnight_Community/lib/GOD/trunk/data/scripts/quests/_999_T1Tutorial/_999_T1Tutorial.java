...
public class _999_T1Tutorial extends Quest implements ScriptFile
{
	...
	private static int SCROLL_RECOVERY = 8594;
	...
	static
	{
		...
		events.put("30573_02", new Event("30573-03.htm", 0, 0, 0, VOUCHER_OF_FLAME, 0x31, SOULSHOT_NOVICE, 200, 0x2c, SOULSHOT_NOVICE, 200));
	...
	public void onLoad()
	{
		System.out.println("Loaded Quest: 999: T1 Tutorial Quest");
	}
	...
	{
		super("T1 Tutorial", false);

		for(int startNpc : new int[] { 30008, 30009, 30017, 30019, 30129, 30131, 30573, 30575, 30370, 30528, 30530, 30400,
				30401, 30402, 30403, 30404, 32133, 32134 })
			addStartNpc(startNpc);

		addTalkId(new int[] { 30018, 30020, 30021, 30011, 30012, 30056, 30574 });
		addFirstTalkId(new int[] { 30600, 30601, 30602, 30598, 30599, 32135 });
			...
			if(player.getRace().ordinal() == t.raceId)
			{
				...
				if(t.npcTyp == 1)
				{
					...
					else if(step == 1 && st.getQuestItemsCount(t.item) == 0 && Ex <= 2)
					{
						...
						else if(player.getClassId().equalsOrChildOf(ClassId.orcMage))
