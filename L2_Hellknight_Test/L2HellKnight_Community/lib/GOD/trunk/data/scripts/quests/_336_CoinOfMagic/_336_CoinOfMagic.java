...
public class _336_CoinOfMagic extends Quest implements ScriptFile
{
	...
	private static final int[][] DROPLIST = { { TimakOrcArcher, BLOOD_MEDUSA }, { TimakOrcSoldier, BLOOD_MEDUSA },
			{ TimakOrcShaman, BLOOD_MEDUSA }, { Lakin, BLOOD_MEDUSA }, { TorturedUndead, BLOOD_MEDUSA },
			{ HatarHanishee, BLOOD_MEDUSA },

			{ TimakOrc, GOLD_WYVERN }, { Shackle, GOLD_WYVERN }, { HeadlessKnight, GOLD_WYVERN },
			{ RoyalCaveServant, GOLD_WYVERN }, { MalrukSuccubusTuren, GOLD_WYVERN },

			{ Formor, SILVER_UNICORN }, { FormorElder, SILVER_UNICORN }, { VanorSilenosShaman, SILVER_UNICORN },
			{ TarlkBugbearHighWarrior, SILVER_UNICORN }, { OelMahum, SILVER_UNICORN }, { OelMahumWarrior, SILVER_UNICORN }, };

	private static final int[] UNKNOWN = { GraveLich, DoomServant, DoomArcher, DoomKnight, Kookaburra1, Kookaburra2,
			Kookaburra3, Kookaburra4, Antelope1, Antelope2, Antelope3, Antelope4, Bandersnatch1, Bandersnatch2,
			Bandersnatch3, Bandersnatch4, Buffalo1, Buffalo2, Buffalo3, Buffalo4, ClawsofSplendor, WisdomofSplendor,
			PunishmentofSplendor, WailingofSplendor, HungeredCorpse, BloodyGhost, NihilInvader, DarkGuard };
	...
	{
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30078.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30092.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30165.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30183.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30200.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30673.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30688.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30696.xml"));
		L2Multisell.getInstance().parseFile(new File(Config.DATAPACK_ROOT, "data/scripts/quests/_336_CoinOfMagic/30847.xml"));
	}
