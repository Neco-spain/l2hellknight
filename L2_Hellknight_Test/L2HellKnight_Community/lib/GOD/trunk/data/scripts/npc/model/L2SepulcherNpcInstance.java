...
public class L2SepulcherNpcInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2Player player, int val)
	{
		...
		switch(getNpcId())
		{
			...
			case 31487:
				...
				if(_spawnMonsterTask != null)
					_spawnMonsterTask.cancel(true);
				_spawnMonsterTask = ThreadPoolManager.getInstance().scheduleGeneral(new SpawnMonster(getNpcId()), 3500);
				return;
	...
	public void openNextDoor(int npcId)
	{
		...
		if(_closeTask != null)
			_closeTask.cancel(true);
		_closeTask = ThreadPoolManager.getInstance().scheduleGeneral(new CloseNextDoor(gk), 10000);
	}
	...
	private class CloseNextDoor implements Runnable
	{
		...
		public void run()
		{
			if(state == 0)
			{
				...
				state++;
				_closeTask = ThreadPoolManager.getInstance().scheduleGeneral(this, 10000);
			}
			else if(state == 1)
