...
public class March8 extends Functions implements ScriptFile
{
	...
	*/
	public void buyrecipe(String[] var)
	{
		...
		long need_adena = (long) (RECIPE_PRICE * Config.EVENT_MARCH8_PRICE_RATE * recipe_count);
	...
	public String DialogAppend_4301(Integer val)
	{
		...
		{
			price = Util.formatAdena((long) (RECIPE_PRICE * Config.EVENT_MARCH8_PRICE_RATE * cnt));
	...
	public void onLoad()
	{
		if(isActive())
		{
			_active = true;
			spawnEventManagers();
			System.out.println("Loaded Event: March 8 [state: activated]");
		}
		else
			System.out.println("Loaded Event: March 8 [state: deactivated]");
	}
	...
	{
		if(_active && SimpleCheckDrop(cha, killer) && Rnd.get(1000) <= Config.EVENT_MARCH8_DROP_CHANCE * killer.getPlayer().getRateItems() * Config.RATE_DROP_ITEMS * ((L2NpcInstance) cha).getTemplate().rateHp)
