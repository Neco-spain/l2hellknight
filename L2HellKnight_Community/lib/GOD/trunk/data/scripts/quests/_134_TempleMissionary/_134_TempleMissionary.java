...
public class _134_TempleMissionary extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("glyvka_q0134_11.htm") && _state == STARTED && st.getInt("cond") == 5)
		{
		...
		if(qs.getState() == STARTED && qs.getInt("cond") == 3)
			...
			else
			{
				...
				if(Rnd.chance(Cruma_Marshlands_Traitor_spawnchance))
					qs.addSpawn(Cruma_Marshlands_Traitor, npc.getX(), npc.getY(), npc.getZ(), 0, 100, 900000);
			}
		return null;
	}
