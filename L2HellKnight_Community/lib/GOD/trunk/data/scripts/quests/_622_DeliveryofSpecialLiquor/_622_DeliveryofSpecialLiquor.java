...
public class _622_DeliveryofSpecialLiquor extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("warehouse_keeper_lietta_q0622_0801.htm") && cond == 7 && st.getQuestItemsCount(FeeOfSpecialDrink) >= 5)
		{
			...
			{
				if(Rnd.chance(40))
					st.giveItems(RecipeSealedTateossianRing + (Config.ALT_100_RECIPES_S ? 1 : 0), 1);
				else if(Rnd.chance(40))
					st.giveItems(RecipeSealedTateossianEarring + (Config.ALT_100_RECIPES_S ? 1 : 0), 1);
				else
					st.giveItems(RecipeSealedTateossianNecklace + (Config.ALT_100_RECIPES_S ? 1 : 0), 1);
			}
			else
			{
