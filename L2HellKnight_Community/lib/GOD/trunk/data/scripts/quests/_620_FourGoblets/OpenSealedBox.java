...
public class OpenSealedBox
{
	...
	private FastMap<Integer, Long> rewards = new FastMap<Integer, Long>();
	private static final RewardGroup[] rewardgroups = { new RewardAdena(), new RewardRes1(), new RewardRes2(),
			new RewardEnchants(), new RewardParts() };
	...
	private boolean canGiveReward()
	{
		...
		for(Integer itemId : rewards.keySet())
		{
			L2ItemInstance item = st.getPlayer().getInventory().getItemByItemId(itemId);
	...
	{
		public abstract void apply(FastMap<Integer, Long> rewards);

		protected static void putReward(FastMap<Integer, Long> rewards, int item_id, long count)
		{
	...
	{
		@Override
		public void apply(FastMap<Integer, Long> rewards)
		{
