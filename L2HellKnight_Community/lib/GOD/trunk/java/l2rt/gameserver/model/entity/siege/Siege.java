...
public abstract class Siege
{
	...
	private boolean checkIfCanRegister(L2Player player)
	{
		...
		{
			player.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.siege.Siege.ClanLevelToSmall", player).addNumber(getSiegeClanMinLevel()));
			return false;
		}
		...
		{
			player.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.siege.Siege.AlreadyRegistered", player));
			return false;
		}

		if(isRegistrationOver())
		{
			player.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.siege.Siege.DeadlinePassed", player).addString(getSiegeUnit().getName()));
			return false;
		}

		if(isInProgress())
		{
			player.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.siege.Siege.NotTimeToCancel", player));
			return false;
		}
	...
	{
		if(getSiegeUnit().getOwnerId() <= 0)
			player.sendMessage(new CustomMessage("l2rt.gameserver.model.entity.siege.Siege.OwnedByNPC", player).addString(getSiegeUnit().getName()));
		...
		public void run()
		{
			...
			for(L2Player player : getPlayersInZone())
				if(player != null && !player.isDead() && !player.isInOfflineMode() && player.getClan() != null && player.getClan().getSiege() == Siege.this)
					player.setFame(player.getFame() + bonus * ConfigSystem.getInt("RateFameReward"), "Siege");
		}
	}
}