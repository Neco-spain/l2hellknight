...
import l2rt.extensions.listeners.events.L2Zone.L2ZoneEnterLeaveEvent;
...
import l2rt.gameserver.network.serverpackets.ExNotifyFlyMoveStart;
...
public class L2Zone
{
	...
	private void onZoneEnter(L2Object object)
	{
		...
		if (this.getId() == 61001 && object instanceof L2Player) {
			L2Player player = (L2Player) object;
			QuestState st = player.getQuestState("_10320_Quest1");
			if(st == null)
				player.showQuestMovie(101);
			else if (st != null && !(st.getStateName() == "Completed"))
				player.showQuestMovie(102);
		}
	...
	public void setActive(boolean value)
	{
		...
		if (getType() == ZoneType.Jump)
			setJumpTaskActive(value);
	}
	/**
	...
	public void setJumpTaskActive(boolean value)
	{
		if(value)
			new JumpTask().start();
		else
	 ...
	 * @see l2rt.gameserver.model.base.Race
	 * @return раса
	 */
	...
	/**
	 * Класс используется для отправки запроса на начало прижка всем кто в зоне
	 */
	private class JumpTask implements Runnable
	{
		public void run()
		{
			if(jumpTask != this || !isActive())
				return;
			...
			synchronized (_objects)
			{
				for(L2Object object : _objects)
				{
					...
					if (target.isAwaking())
						target.sendPacket(new ExNotifyFlyMoveStart());
				}
			}
