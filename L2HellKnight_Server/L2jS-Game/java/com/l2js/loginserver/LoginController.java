...
import com.l2js.loginserver.BanList.BanInfo;
import com.l2js.loginserver.GameServerTable.GameServerInfo;
import com.l2js.loginserver.network.L2LoginClient;
import com.l2js.loginserver.network.gameserverpackets.ServerStatus;
		...
		try
		{
			...
			if (expected == null)
			{
				if (Config.AUTO_CREATE_ACCOUNTS)
				{
					...
					{
						statement = con
								.prepareStatement("INSERT INTO accounts (login,password,lastactive,accessLevel,lastIP) values(?,?,?,?,?)");
				...
				else
				{
					...
					if (failedCount >= Config.LOGIN_TRY_BEFORE_BAN)
					{
						_log.info("Banning '" + address.getHostAddress() + "' for " + Config.LOGIN_BLOCK_AFTER_BAN
								+ " seconds due to " + failedCount + " invalid user name attempts");
			...
			else
			{
				...
				if (userIP != null)
				{
					...
					{
						if (Config.LOG_LOGIN_CONTROLLER)
							Log.add("'" + user + "' " + address.getHostAddress() + "/" + userIP
