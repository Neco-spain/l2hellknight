...
import com.l2js.gameserver.network.serverpackets.PetitionVotePacket;
...
public final class PetitionManager
{
	...
	public void sendPendingPetitionList(L2PcInstance activeChar)
	{
		...
		for (Petition currPetition : getPendingPetitions().values())
		{
			...
			StringUtil.append(htmlContent, "</td><td width=\"140\" align=right><font color=\"", (currPetition
					.getPetitioner().isOnline() ? "00FF00" : "999999"), "\">", currPetition.getPetitioner().getName(),
					"</font></td></tr>");
	...
	public int submitPetition(L2PcInstance petitioner, String petitionText, int petitionType)
	{
		...
		String msgContent = petitioner.getName() + " has submitted a new petition."; // (ID:
																						// " + newPetitionId + ").";
		GmListTable.broadcastToGMs(new CreatureSay(petitioner.getObjectId(), Say2.HERO_VOICE, "Petition System",
				msgContent));
