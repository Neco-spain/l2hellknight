		...
		for (Territory t : activeTerritoryList)
		{
			...
			for (TerritoryNPCSpawn ward : t.getOwnedWard())
				if (ward.getNpc() != null && t.getOwnerClan() != null)
				{
					...
					_territoryWards.add(new TerritoryWard(ward.getNpcId(), ward.getLocation().getX(), ward.getLocation().getY(), ward.getLocation().getZ(), 0, ward.getNpcId() + 13479,
							t.getCastleId(), ward.getNpc()));
				}
		...
		public void run()
		{
			...
			try
			{
				...
				if (timeRemaining > 7200000)
				{
					...
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining - 7200000); // Prepare
																																					// task
																																					// for
																																					// 2h
																																					// before
				...
				else if ((timeRemaining <= 7200000) && (timeRemaining > 1200000))
				{
					...
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining - 1200000); // Prepare
																																					// task
																																					// for
																																					// 20
																																					// mins
				...
				else if ((timeRemaining <= 1200000) && (timeRemaining > 600000))
				{
					...
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining - 600000); // Prepare
																																				// task
																																				// for
																																				// 10
																																				// mins
				...
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					...
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining - 300000); // Prepare
																																				// task
																																				// for
																																				// 5
																																				// mins
				...
				else if ((timeRemaining <= 300000) && (timeRemaining > 60000))
				{
					...
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining - 60000); // Prepare
																																				// task
																																				// for
																																				// 1
																																				// min
				...
				else if ((timeRemaining <= 60000) && (timeRemaining > 0))
				{
					...
					_isRegistrationOver = true;
					updatePlayerTWStateFlags(false);
					_scheduledStartTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartTWTask(), timeRemaining); // Prepare
																																		// task
																																		// for
																																		// TW
																																		// start.
				...
				else if ((timeRemaining + WARLENGTH) > 0)
				{
					...
					_isRegistrationOver = true;
					startTerritoryWar();
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), 1000); // Prepare
																															// task
																															// for
																															// TW
																															// end.
		...
		public void run()
		{
			try
			{
				...
				if (timeRemaining > 3600000)
				{
					...
					sm.addNumber(2);
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), timeRemaining - 3600000); // Prepare
																																				// task
																																				// for
																																				// 1
																																				// hr
				...
				else if ((timeRemaining <= 3600000) && (timeRemaining > 600000))
				{
					...
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), timeRemaining - 600000); // Prepare
																																			// task
																																			// for
																																			// 10
																																			// minute
				...
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					...
					sm.addNumber(Math.round(timeRemaining / 60000));
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), timeRemaining - 300000); // Prepare
																																			// task
																																			// for
																																			// 5
																																			// minute
				...
				else if ((timeRemaining <= 300000) && (timeRemaining > 10000))
				{
					...
					sm.addNumber(Math.round(timeRemaining / 60000));
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), timeRemaining - 10000); // Prepare
																																			// task
																																			// for
																																			// 10
																																			// seconds
				...
				else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					...
					announceToParticipants(sm, 0, 0);
					_scheduledEndTWTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndTWTask(), timeRemaining); // Prepare
																																	// task
																																	// for
																																	// second
																																	// count
