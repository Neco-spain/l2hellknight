...
import com.l2js.gameserver.model.zone.L2ZoneRespawn;
import com.l2js.gameserver.model.zone.L2ZoneType;
import com.l2js.gameserver.model.zone.form.ZoneCuboid;
import com.l2js.gameserver.model.zone.form.ZoneCylinder;
import com.l2js.gameserver.model.zone.form.ZoneNPoly;
import com.l2js.gameserver.model.zone.type.L2ArenaZone;
import com.l2js.gameserver.model.zone.type.L2OlympiadStadiumZone;
import com.l2js.gameserver.model.zone.type.L2RespawnZone;
import com.l2js.util.file.filter.XMLFilter;
...
public class ZoneManager
{
	...
	// private final FastMap<Integer, L2ZoneType> _zones = new FastMap<Integer,
	// L2ZoneType>();
					...
					if ("list".equalsIgnoreCase(n.getNodeName()))
					{
						...
						for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
						{
							if ("zone".equalsIgnoreCase(d.getNodeName()))
							{
								...
								{
									_log.warning("ZoneData: Missing minZ for zone: " + zoneId + " in file: "
											+ f.getName());
									continue;
								}
								...
								{
									_log.warning("ZoneData: Missing maxZ for zone: " + zoneId + " in file: "
											+ f.getName());
									continue;
								}
								...
								{
									_log.warning("ZoneData: Missing type for zone: " + zoneId + " in file: "
											+ f.getName());
									continue;
								}
								...
								{
									_log.warning("ZoneData: Missing shape for zone: " + zoneId + " in file: "
											+ f.getName());
									continue;
								}
								...
								try
								{
									newZone = Class.forName("com.l2js.gameserver.model.zone.type.L2" + zoneType);
								}
								catch (ClassNotFoundException e)
								{
									_log.warning("ZoneData: No such zone type: " + zoneType + " in file: "
											+ f.getName());
									continue;
								}
								...
								try
								{
									...
									{
										_log.warning("ZoneData: missing data for zone: " + zoneId
												+ " in both XML and SQL, file: " + f.getName());
										continue;
									}

									// Create this zone. Parsing for cuboids is
									// a
									// bit different than for other polygons
									// cuboids need exactly 2 points to be
									// defined.
									...
									if (zoneShape.equalsIgnoreCase("Cuboid"))
									{
										...
										else
										{
											_log.warning("ZoneData: Missing cuboid vertex in sql data for zone: "
													+ zoneId + " in file: " + f.getName());
											continue;
										}
									}
									else if (zoneShape.equalsIgnoreCase("NPoly"))
									{
										// nPoly needs to have at least 3
										// vertices
										if (coords.length > 2)
										{
										...
										{
											_log.warning("ZoneData: Bad data for zone: " + zoneId + " in file: "
													+ f.getName());
											continue;
										}
									}
									...
									{
										// A Cylinder zone requires a center
										// point
										// at x,y and a radius
										attrs = d.getAttributes();
										final int zoneRad = Integer.parseInt(attrs.getNamedItem("rad").getNodeValue());
										if (coords.length == 1 && zoneRad > 0)
											temp.setZone(new ZoneCylinder(coords[0][0], coords[0][1], minZ, maxZ,
													zoneRad));
										else
										{
											_log.warning("ZoneData: Bad data for zone: " + zoneId + " in file: "
									...
									{
										_log.warning("ZoneData: Unknown shape: " + zoneShape + " in file: "
												+ f.getName());
										continue;
									}
								}
								...
								{
									_log.log(Level.WARNING, "ZoneData: Failed to load zone " + zoneId
								...
								if (checkId(zoneId))
									_log.config("Caution: Zone (" + zoneId + ") from file: " + f.getName()
											+ " overrides previos definition.");
	 ...
	 * To minimise iteration processing retrieve zones from L2WorldRegion for a
