...
import com.l2js.gameserver.model.multisell.ListContainer;
...
import com.l2js.gameserver.network.serverpackets.MultiSellList;
	...
	private final Entry parseEntry(Node n, int entryId, ListContainer list)
	{
		...
		for (n = first; n != null; n = n.getNextSibling())
		{
			...
			if ("ingredient".equalsIgnoreCase(n.getNodeName()))
			{
				...
				attribute = n.getAttributes().getNamedItem("enchantLvl");
				if (attribute != null)
					enchantLvl = Integer.parseInt(attribute.getNodeValue());
				
				entry.addIngredient(new Ingredient(id, count, enchantLvl, isTaxIngredient, mantainIngredient));
			}
			else if ("production".equalsIgnoreCase(n.getNodeName()))
			{
				id = Integer.parseInt(n.getAttributes().getNamedItem("id").getNodeValue());
				count = (long) (Long.parseLong(n.getAttributes().getNamedItem("count").getNodeValue()) * list.getUseRate());
				
				attribute = n.getAttributes().getNamedItem("enchantLvl");
				if (attribute != null)
					enchantLvl = Integer.parseInt(attribute.getNodeValue());
				
				entry.addProduct(new Ingredient(id, count, enchantLvl, false, false));
			}
		}
