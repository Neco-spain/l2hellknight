...
public final class SkillTreesData
{
	...
	public void load()
	{
		...
		_log.info(getClass().getSimpleName() + ": Loaded " + _pledgeSkillTree.size() + " Pledge Skills, " + (_pledgeSkillTree.size() - residentialSkillCount) + " for Pledge and "
				+ residentialSkillCount + " Residential.");
	 ...
	 * @return all available skills for a given {@code player}, {@code classId}, {@code includeByFs} and
	 *         {@code includeAutoGet}.
	 */
		...
		for (L2SkillLearn temp : skills.values())
		{
			if (player.getLevel() >= temp.getGetLevel())
			{
				...
				for (SubClass subClass : player.getSubClasses().values())
				{
					...
					if ((subClassConds != null) && (subClass.getClassIndex() <= subClassConds.length) && (subClass.getClassIndex() == subClassConds[subClass.getClassIndex() - 1][1])
							&& (subClassConds[subClass.getClassIndex() - 1][0] <= subClass.getLevel()))
					{
						boolean knownSkill = false;
