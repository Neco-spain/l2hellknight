...
public class HitConditionBonus
{
	...
	protected static double getConditionBonus(L2Character attacker, L2Character target)
	{
		...
		// If (mod / 10) is less than 0, return 0, because we cant lower more
		// than 100%.
	...
	static
	{
		...
		if (file.exists())
		{
			...
			{
				if ("hitConditionBonus".equalsIgnoreCase(list.getNodeName())
						|| "list".equalsIgnoreCase(list.getNodeName()))
				{
					for (Node cond = list.getFirstChild(); cond != null; cond = cond.getNextSibling())
					{
						...
						{
							_log.log(Level.WARNING, "[hitConditionBonus] Could not parse condition: " + e.getMessage(),
									e);
						}
						finally
						{
