 ...
 */
package com.l2js.gameserver.skills;
...
import com.l2js.gameserver.datatables.ClassBalanceTable.TypeBalance;
...
import com.l2js.gameserver.instancemanager.SiegeManager;
import com.l2js.gameserver.instancemanager.ZoneManager;
import com.l2js.gameserver.model.Elementals;
...
import com.l2js.gameserver.model.L2Skill.SkillTraitType;
...
import com.l2js.gameserver.model.actor.instance.L2BabyPetInstance;
import com.l2js.gameserver.model.actor.instance.L2CubicInstance;
...
import com.l2js.gameserver.model.base.PlayerState;
...
import com.l2js.gameserver.model.zone.type.L2CastleZone;
import com.l2js.gameserver.model.zone.type.L2ClanHallZone;
import com.l2js.gameserver.model.zone.type.L2FortZone;
import com.l2js.gameserver.model.zone.type.L2MotherTreeZone;
import com.l2js.gameserver.network.SystemMessageId;
import com.l2js.gameserver.network.serverpackets.SystemMessage;
import com.l2js.gameserver.skills.conditions.ConditionPlayerState;
import com.l2js.gameserver.skills.conditions.ConditionUsingItemType;
...
public final class Formulas
{
	...
	public static final byte SHIELD_DEFENSE_SUCCEED = 1; // normal shield
															// defense
	...
	public static final byte SKILL_REFLECT_SUCCEED = 1; // normal reflect, some
														// damage reflected some
														// other not
	public static final byte SKILL_REFLECT_VENGEANCE = 2; // 100% of the damage
															// affect both
	...
	public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean dual, boolean ss)
	{
		 ...
		 * {
		 * if (attacker instanceof L2RaidBossInstance) damage *= 1; // was 10
		 * changed for temp fix
		 * // else
		 * // damage *= 2;
		 * // if (attacker instanceof L2NpcInstance || attacker instanceof
		 * L2SiegeGuardInstance){
		 * //damage = damage * attacker.getSTR() * attacker.getAccuracy() * 0.05
		 * / defence;
		 * // damage = damage * attacker.getSTR()* (attacker.getSTR() +
		 * attacker.getLevel()) * 0.025 / defence;
		...
		damage = balanceDamage(attacker, target, damage, TypeDam.PHYS);
		return damage;
	}
	...
	private static final double balanceDamage(L2Character attacker, L2Character target, double damage, TypeDam type)
	{
		...
		{
			int classIdA = ((L2PcInstance) attacker).getClassId().getId();
			int classIdB = ((L2PcInstance) target).getClassId().getId();
			
			if (Config.ENABLE_CLASS_VS_CLASS_SYSTEM)
				damage *= ClassVsClassTable.getInstance().getPower(classIdA, classIdB);
			...
			if (Config.ENABLE_CLASS_BALANCE_SYSTEM)
			{
				...
				switch (type)
				{
					case PHYS:
						...
						{
							case 0:
								damage *= cbt.getBalanceValue(TypeBalance.FxH, classIdA);
								break;
							case 1:
								damage *= cbt.getBalanceValue(TypeBalance.FxL, classIdA);
								break;
							case 2:
								damage *= cbt.getBalanceValue(TypeBalance.FxR, classIdA);
								break;
						}
						break;
					case MAGIC:
						...
						{
							case 0:
								damage *= cbt.getBalanceValue(TypeBalance.MxH, classIdA);
								break;
							case 1:
								damage *= cbt.getBalanceValue(TypeBalance.MxL, classIdA);
								break;
							case 2:
								damage *= cbt.getBalanceValue(TypeBalance.MxR, classIdA);
								break;
						}
						break;
				}
	...
	public static final double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		damage = balanceDamage(attacker, target, damage, TypeDam.MAGIC);
		return damage;
	}
	...
	public static final boolean calcLethalHit(L2Character activeChar, L2Character target, L2Skill skill)
	{
		if (!target.isRaid() && !(target instanceof L2DoorInstance) && !(target instanceof L2Npc && ((L2Npc) target).getNpcId() == 35062))
		{
			...
			// activeChar.sendMessage(Double.toString(calcLethal(activeChar,
			// target, skill.getLethalChance2(),skill.getMagicLevel())));
			// activeChar.sendMessage(Double.toString(calcLethal(activeChar,
			// target, skill.getLethalChance1(),skill.getMagicLevel())));
			...
			else if (skill.getLethalChance1() > 0 && Rnd.get(1000) < calcLethal(activeChar, target, skill.getLethalChance1(), skill.getMagicLevel()))
			{
				...
				/*
				 * else if (target instanceof L2Npc) // If is a monster remove
				 * first damage and after 50% of current hp
	...
	public static boolean calcEffectSuccess(L2Character attacker, L2Character target, EffectTemplate effect, L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		else if (type.equals(L2SkillType.CANCEL)) // CANCEL-type effects land
													// always
			return true;
		...
		if (attacker.isDebug() || Config.DEVELOPER)
		{
			...
			StringUtil.append(stat, skill.getName(), " eff.type:", type.toString(), " power:", String.valueOf(value), " stat:", String.format("%1.2f", statModifier), " res:",
					String.format("%1.2f", resMod), "(", String.format("%1.2f", profModifier), "/", String.format("%1.2f", vulnModifier), ") elem:", String.valueOf(elementModifier), " mAtk:",
					String.format("%1.2f", mAtkModifier), " ss:", String.valueOf(ssModifier), " lvl:", String.valueOf(deltamod), " total:", String.valueOf(rate));
	...
	public static boolean calcSkillSuccess(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		{
			final StringBuilder stat = new StringBuilder(100);
			StringUtil.append(stat, skill.getName(), " type:", skill.getSkillType().toString(), " power:", String.valueOf(value), " stat:", String.format("%1.2f", statModifier), " res:",
					String.format("%1.2f", resMod), "(", String.format("%1.2f", profModifier), "/", String.format("%1.2f", vulnModifier), ") elem:", String.valueOf(elementModifier), " mAtk:",
					String.format("%1.2f", mAtkModifier), " ss:", String.valueOf(ssModifier), " lvl:", String.valueOf(deltamod), " total:", String.valueOf(rate));
			final String result = stat.toString();
			if (attacker.isDebug())
	...
	public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, L2Character target, L2Skill skill, byte shld)
	{
		...
		if (attacker.getOwner().isDebug() || Config.DEVELOPER)
		{
			...
			StringUtil.append(stat, skill.getName(), " type:", skill.getSkillType().toString(), " power:", String.valueOf(value), " stat:", String.format("%1.2f", statModifier), " res:",
					String.format("%1.2f", resMod), "(", String.format("%1.2f", profModifier), "/", String.format("%1.2f", vulnModifier), ") elem:", String.valueOf(elementModifier), " mAtk:",
					String.format("%1.2f", mAtkModifier), " lvl:", String.valueOf(deltamod), " total:", String.valueOf(rate));
			final String result = stat.toString();
	...
	public static boolean calcMagicSuccess(L2Character attacker, L2Character target, L2Skill skill)
	{
		...
		if (target instanceof L2Attackable && !target.isRaid() && !target.isRaidMinion() && target.getLevel() >= Config.MIN_NPC_LVL_MAGIC_PENALTY && attacker.getActingPlayer() != null
		...
		{
			final StringBuilder stat = new StringBuilder(100);
			StringUtil.append(stat, skill.getName(), " lvlDiff:", String.valueOf(lvlDifference), " lvlMod:", String.format("%1.2f", lvlModifier), " res:", String.format("%1.2f", resModifier),
					" fail:", String.format("%1.2f", failureModifier), " tgt:", String.valueOf(targetModifier), " total:", String.valueOf(rate));
			final String result = stat.toString();
			if (attacker.isDebug())
				attacker.sendDebugMessage(result);
			if (Config.DEVELOPER)
	 ...
	 * Reflect failed</li> <li>Mormal reflect (just effects). <U>Only possible for skilltypes: BUFF, REFLECT,
	 * HEAL_PERCENT, MANAHEAL_PERCENT, HOT, CPHOT, MPHOT</U></li> <li>vengEance reflect (100% damage reflected but
	 * damage is also dealt to actor). <U>This is only possible for skills with skilltype PDAM, BLOW, CHARGEDAM, MDAM or
	 * DEATHLINK</U></li> <br>
	 * <br>
	 * 
	 * @param actor
	 ...
	 * Return True if the target is IN FRONT of the L2Character.<BR>
	 * <BR>
	 */
