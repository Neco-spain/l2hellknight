...
import com.l2js.gameserver.datatables.CharCustomTable.CustomType;
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS =
	{
			...
			"admin_sethero", "admin_setnoble", "admin_setdonator"
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_changename"))
		{
			try
			{
				...
				CommunityServerThread.getInstance().sendPacket(
						new WorldInfo(player, null, WorldInfo.TYPE_UPDATE_PLAYER_DATA));
		...
		else if (command.startsWith("admin_setcolor"))
		{
			...
			String color = String.valueOf(Config.CHARACTER_COLOR_NAME);
			long time = 0;
			...
			if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				if (time == -1 || color.equals("-1"))
				{
					CharCustomTable.getInstance().delete("COLORNAME", target.getObjectId(), true);
					activeChar.sendMessage("Removed the color of the name " + target.getName() + ".");
				}
				else
				{
					CharCustomTable.getInstance().add(CustomType.COLORNAME, target, Integer.decode("0x" + color),
							System.currentTimeMillis(), (time * 86400 * 1000));
					if (time == 0)
						activeChar.sendMessage("Added color in the name of " + target.getName() + ".");
					else
						activeChar.sendMessage("Added color in the name of " + target.getName() + " for "
								+ String.valueOf(time) + " days.");
				}
			}
			else
		...
		else if (command.startsWith("admin_settcolor"))
		{
			L2PcInstance target = null;
			String[] cmd = command.split(" ");
			String color = String.valueOf(Config.CHARACTER_COLOR_TITLE);
			long time = 0;
			...
			if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				if (time == -1 || color.equals("-1"))
				{
					CharCustomTable.getInstance().delete("COLORTITLE", target.getObjectId(), true);
					activeChar.sendMessage("Removed the color of the title " + target.getName() + ".");
				}
				else
				{
					CharCustomTable.getInstance().add(CustomType.COLORTITLE, target, Integer.decode("0x" + color),
							System.currentTimeMillis(), (time * 86400 * 1000));
					if (time == 0)
						activeChar.sendMessage("Added color in the title of " + target.getName() + ".");
					else
						activeChar.sendMessage("Added color in the title of " + target.getName() + " for "
								+ String.valueOf(time) + " days.");
				}
			}
			else
		...
		else if (command.startsWith("admin_sethero"))
		{
			...
			if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				if (time == -1)
				{
					CharCustomTable.getInstance().delete("HERO", target.getObjectId(), true);
					activeChar.sendMessage(target.getName() + " is not a hero now.");
				}
				else
				{
					CharCustomTable.getInstance().add(CustomType.HERO, target, 1, System.currentTimeMillis(),
							(time * 86400 * 1000));
					if (time == 0)
						activeChar.sendMessage(target.getName() + " is a hero now.");
					else
						activeChar.sendMessage(target.getName() + " is a hero now for " + String.valueOf(time)
								+ " days.");
				}
			}
			else
		...
		else if (command.startsWith("admin_setnoble"))
		{
			...
			if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				if (time == -1)
				{
					CharCustomTable.getInstance().delete("NOBLE", target.getObjectId(), true);
					activeChar.sendMessage(target.getName() + " is not a noble now.");
				}
				else
				{
					CharCustomTable.getInstance().add(CustomType.NOBLE, target, 1, System.currentTimeMillis(),
							(time * 86400 * 1000));
					if (time == 0)
						activeChar.sendMessage(target.getName() + " is a noble now.");
					else
						activeChar.sendMessage(target.getName() + " is a noble now for " + String.valueOf(time)
								+ " days.");
				}
			}
			else
		...
		else if (command.startsWith("admin_setdonator"))
		{
			L2PcInstance target = null;
			String[] cmd = command.split(" ");
			long time = 0;
			...
			if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				if (time == -1)
				{
					CharCustomTable.getInstance().delete("DONATOR", target.getObjectId(), true);
					activeChar.sendMessage(target.getName() + " is not a donator now.");
				}
				else
				{
					CharCustomTable.getInstance().add(CustomType.DONATOR, target, 1, System.currentTimeMillis(),
							(time * 86400 * 1000));
					if (time == 0)
						activeChar.sendMessage(target.getName() + " is a donator now.");
					else
						activeChar.sendMessage(target.getName() + " is a donator now for " + String.valueOf(time)
								+ " days.");
				}
			}
			else
	...
	private void listCharacters(L2PcInstance activeChar, int page)
	{
		...
		for (int i = charactersStart; i < charactersEnd; i++)
		{
			...
			StringUtil.append(replyMSG, "<tr><td width=80><a action=\"bypass -h admin_character_info ",
					...
					players[i].getTemplate().className, "</td><td width=40>", String.valueOf(players[i].getLevel()),
					"</td></tr>");
		}
	...
	private void setTargetKarma(L2PcInstance activeChar, int newKarma)
	{
		...
		if (newKarma >= 0)
		{
			...
			activeChar.sendMessage("Successfully Changed karma for " + player.getName() + " from (" + oldKarma
					+ ") to (" + newKarma + ").");
			if (Config.DEBUG)
				_log.fine("[SET KARMA] [GM]" + activeChar.getName() + " Changed karma for " + player.getName()
		...
		else
		{
			...
			if (Config.DEBUG)
				_log.fine("[SET KARMA] ERROR: [GM]" + activeChar.getName()
						+ " entered an incorrect value for new karma: " + newKarma + " for " + player.getName() + ".");
		}
	}
	...
	private void adminModifyCharacter(L2PcInstance activeChar, String modifications)
	{
		...
		// Admin information
		activeChar.sendMessage("Changed stats of " + player.getName() + "." + "  HP: " + hpval + "  MP: " + mpval
	...
	private void findCharacter(L2PcInstance activeChar, String CharacterToFind)
	{
		...
		{ // Add player info into new Table row
			...
			if (name.toLowerCase().contains(CharacterToFind.toLowerCase()))
			{
				...
				StringUtil.append(replyMSG, "<tr><td width=80><a action=\"bypass -h admin_character_list ", name,
						"\">", name, "</a></td><td width=110>", player.getTemplate().className, "</td><td width=40>",
	...
	private void findDualboxStrict(L2PcInstance activeChar, int multibox)
	{
		...
		{
			StringUtil.append(results, "<a action=\"bypass -h admin_find_ip " + dualboxIP.ip + "\">" + dualboxIP.ip
					+ " (" + dualboxIPs.get(dualboxIP) + ")</a><br1>");
		}
	...
	private void gatherPartyInfo(L2PcInstance target, L2PcInstance activeChar)
	{
		...
		for (L2PcInstance member : target.getParty().getPartyMembers())
		{
			...
			text.append("</td><td width=110 align=right>" + member.getClassId().toString()
					+ "</td></tr></table></td></tr>");
			color = !color;
		}
