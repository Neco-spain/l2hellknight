...
import com.l2js.gameserver.datatables.MultiSell;
import com.l2js.gameserver.datatables.NpcBufferTable;
...
import com.l2js.gameserver.model.actor.instance.L2OlympiadManagerInstance;
import com.l2js.gameserver.model.actor.instance.L2PcInstance;
import com.l2js.gameserver.model.olympiad.CompetitionType;
...
import com.l2js.gameserver.network.serverpackets.ExHeroList;
...
import com.l2js.util.L2FastList;
	...
	public final boolean useBypass(String command, L2PcInstance activeChar, L2Character target)
	{
		...
		try
		{
			...
			else if (command.toLowerCase().startsWith(COMMANDS[1])) // noble
			{
				...
				switch (val)
				{
					case 0: // H5 match selection
						...
						{
							html.setFile(activeChar.getHtmlPrefix(), Olympiad.OLYMPIAD_HTML_PATH
									+ "noble_unregister.htm");
							html.replace("%objectId%", String.valueOf(target.getObjectId()));
							activeChar.sendPacket(html);
						}
						break;
					...
					case 2: // show waiting list | TODO: cleanup (not used anymore)
						...
						if (Config.ALT_OLY_REG_DISPLAY > 0)
						{
							...
							html.replace("%listNonClassedTeam%", teams < Config.ALT_OLY_REG_DISPLAY ? FEWER_THAN
									: MORE_THAN);
					...
					case 10: // give tokens to player
						...
						if (passes > 0)
						{
							L2ItemInstance item = activeChar.getInventory().addItem("Olympiad", GATE_PASS, passes,
									activeChar, target);
			...
			else if (command.toLowerCase().startsWith(COMMANDS[2])) // buff
			{
				...
				if (params[1] == null)
				{
					_log.warning("Olympiad Buffer Warning: npcId = " + ((L2Npc) target).getNpcId()
							+ " has no buffGroup set in the bypass for the buff selected.");
					return false;
				}
				int buffGroup = Integer.parseInt(params[1]);

				int[] npcBuffGroupInfo = NpcBufferTable.getInstance().getSkillInfo(((L2Npc) target).getNpcId(),
						buffGroup);

				if (npcBuffGroupInfo == null)
				{
					_log.warning("Olympiad Buffer Warning: npcId = " + ((L2Npc) target).getNpcId() + " Location: "
							+ target.getX() + ", " + target.getY() + ", " + target.getZ() + " Player: "
							+ activeChar.getName() + " has tried to use skill group (" + buffGroup
				...
				if (activeChar.olyBuff > 0)
				{
					...
					{
						activeChar.olyBuff--;
						target.broadcastPacket(new MagicSkillUse(target, activeChar, skill.getId(), skill.getLevel(),
								0, 0));
						skill.getEffects(activeChar, activeChar);
						L2Summon summon = activeChar.getPet();
						if (summon != null)
						{
							target.broadcastPacket(new MagicSkillUse(target, summon, skill.getId(), skill.getLevel(),
									0, 0));
				...
				if (activeChar.olyBuff > 0)
				{
					html.setFile(activeChar.getHtmlPrefix(), activeChar.olyBuff == 5 ? Olympiad.OLYMPIAD_HTML_PATH
							+ "olympiad_buffs.htm" : Olympiad.OLYMPIAD_HTML_PATH + "olympiad_5buffs.htm");
					html.replace("%objectId%", String.valueOf(target.getObjectId()));
					activeChar.sendPacket(html);
				}
				else
			...
			else if (command.toLowerCase().startsWith(COMMANDS[3])) // olympiad
			{
				...
				switch (val)
				{
					case 2: // show rank for a specific class 
						...
						if (((classId >= 88) && (classId <= 118)) || ((classId >= 131) && (classId <= 134))
								|| (classId == 136))
						{
							L2FastList<String> names = Olympiad.getInstance().getClassLeaderBoard(classId);
							reply.setFile(activeChar.getHtmlPrefix(), Olympiad.OLYMPIAD_HTML_PATH
									+ "olympiad_ranking.htm");
