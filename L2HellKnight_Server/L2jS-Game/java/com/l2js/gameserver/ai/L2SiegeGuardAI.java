...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		...
		if (intention == AI_INTENTION_IDLE /* || intention == AI_INTENTION_ACTIVE */) // active becomes idle if only a
																						// summon is present
		{
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			if (hated != null)
			{
				...
				{
					// Set the L2Character movement type to run and send Server->Client packet ChangeMoveType to all
					// others L2PcInstance
					if (!_actor.isRunning())
						_actor.setRunning();
	...
	private void thinkAttack()
	{
		...
		if (_attackTimeout < GameTimeController.getGameTicks())
		{
			...
			if (_actor.isRunning())
			{
				// Set the actor movement type to walk and send Server->Client packet ChangeMoveType to all others
				// L2PcInstance
				_actor.setWalking();
	...
	private final void factionNotifyAndSupport()
	{
		...
		// for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(((L2NpcInstance)
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (!(cha instanceof L2Npc))
			{
				...
				{
					// heal friends
					if (!_actor.isAttackingDisabled() && cha.getCurrentHp() < cha.getMaxHp() * 0.6 && _actor.getCurrentHp() > _actor.getMaxHp() / 2 && _actor.getCurrentMp() > _actor.getMaxMp() / 2
							&& cha.isInCombat())
					{
	...
	{
		// if(getIntention() != AI_INTENTION_IDLE && (!_actor.isVisible() || !_actor.hasAI() ||
		// !_actor.isKnownPlayers()))
