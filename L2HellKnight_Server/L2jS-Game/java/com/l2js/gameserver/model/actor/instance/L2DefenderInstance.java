...
import com.l2js.gameserver.ai.L2SpecialSiegeGuardAI;
...
import com.l2js.gameserver.model.actor.knownlist.DefenderKnownList;
...
import com.l2js.gameserver.network.serverpackets.MyTargetSelected;
import com.l2js.gameserver.network.serverpackets.StatusUpdate;
import com.l2js.gameserver.network.serverpackets.ValidateLocation;
...
public class L2DefenderInstance extends L2Attackable
{
		...
		if ((_fort != null && _fort.getZone().isActive()) || (_castle != null && _castle.getZone().isActive()) || (_hall != null && _hall.getSiegeZone().isActive()))
		{
			...
			if (player != null
					&& ((player.getSiegeState() == 2 && !player.isRegisteredOnThisSiegeField(activeSiegeId))
							|| (player.getSiegeState() == 1 && !TerritoryWarManager.getInstance().isAllyField(player, activeSiegeId)) || player.getSiegeState() == 0))
			{
				return true;
			}
		}
	...
	public void onAction(L2PcInstance player, boolean interact)
	{
		...
		if (this != player.getTarget())
		{
			...
			// Send a Server->Client packet ValidateLocation to correct the L2NpcInstance position and heading on the
			// client
		...
		if (!(attacker instanceof L2DefenderInstance))
		{
			if (damage == 0 && aggro <= 1 && attacker instanceof L2Playable)
			{
				...
				if ((_fort != null && _fort.getZone().isActive()) || (_castle != null && _castle.getZone().isActive()) || _hall != null && _hall.getSiegeZone().isActive())
				{
					int activeSiegeId = (_fort != null ? _fort.getFortId() : (_castle != null ? _castle.getCastleId() : (_hall != null ? _hall.getId() : 0)));
					if (player != null
							&& ((player.getSiegeState() == 2 && player.isRegisteredOnThisSiegeField(activeSiegeId)) || (player.getSiegeState() == 1 && TerritoryWarManager.getInstance().isAllyField(
									player, activeSiegeId))))
					{
						return;
					}
				}
