...
import com.l2js.gameserver.model.L2Party.MessageType;
...
import com.l2js.gameserver.util.PlayerEventStatus;
...
public final class L2PcInstance extends L2Playable
{
	...
	public void setProtection(boolean protect)
	{
		if (Config.DEVELOPER && (protect || _protectEndTime > 0))
			_log.warning(getName() + ": Protection " + (protect ? "ON " + (GameTimeController.getGameTicks() + Config.PLAYER_SPAWN_PROTECTION * GameTimeController.TICKS_PER_SECOND) : "OFF")
	...
	public void setTeleportProtection(boolean protect)
	{
		if (Config.DEVELOPER && (protect || _teleportProtectEndTime > 0))
			_log.warning(getName() + ": Tele Protection " + (protect ? "ON " + (GameTimeController.getGameTicks() + Config.PLAYER_TELEPORT_PROTECTION * GameTimeController.TICKS_PER_SECOND) : "OFF")
					+ " (currently " + GameTimeController.getGameTicks() + ")");
		...
		switch (skill.getSkillType())
		{
			...
			case SUMMON:
			{
				if (!((L2SkillSummon) skill).isCubic()
						&& (getPet() != null || isMounted() || CharSummonTable.getInstance().getPets().contains(getObjectId()) || CharSummonTable.getInstance().getPets().contains(getObjectId())))
				{
					if (Config.DEBUG)
	...
	public final void broadcastUserInfo()
	{
		...
		if (TerritoryWarManager.getInstance().isTWInProgress()
				&& (TerritoryWarManager.getInstance().checkIsRegistered(-1, getObjectId()) || TerritoryWarManager.getInstance().checkIsRegistered(-1, getClan())))
		{
	 ...
	 * Remove the L2ItemInstance from the world and send server->client GetItem packets</li> <li>Send a System Message
	 * to the L2PcInstance : YOU_PICKED_UP_S1_ADENA or YOU_PICKED_UP_S1_S2</li> <li>Add the Item to the L2PcInstance
	 * inventory</li> <li>Send a Server->Client packet InventoryUpdate to this L2PcInstance with NewItem (use a new
	 * slot) or ModifiedItem (increase amount)</li> <li>Send a Server->Client packet StatusUpdate to this L2PcInstance
	...
	public void increasePvpKills(L2Character target)
	{
		if (target instanceof L2PcInstance && AntiFeedManager.getInstance().check(this, target))
		{
			...
			if (Config.PVP_ALLOW_REWARD)
			{
				for (String s : Config.PVP_REWARD)
				{
					try
					{
						int itemId = Integer.parseInt(s.split(",")[0]);
						long amount = Long.parseLong(s.split(",")[1]);
						addItem("PvP Reward", itemId, amount, this, true);
					}
					catch (Exception e)
					{
						continue;
		...
		if (!L2Event.isParticipant(this))
			if (!L2Event.isParticipant(this))
				if (lvl < ExperienceTable.getInstance().getMaxLevel())
					...
					lostExp = Math.round((getStat().getExpForLevel(ExperienceTable.getInstance().getMaxLevel()) - getStat().getExpForLevel(ExperienceTable.getInstance().getMaxLevel() - 1))
							* percentLost / 100);
	...
	private void restoreSkills()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				// Add the L2Skill object to the L2Character _skills and its Func objects to the calculator set of the
				// L2Character
	...
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		if ((TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) || (DMEvent.isStarted() && DMEvent.isPlayerParticipant(getObjectId()))
		...
		if (attacker instanceof L2Playable)
		{
			...
			if (getClan() != null)
			{
				...
				if (getClan() != null && cha.getClan() != null && getClan().isAtWarWith(cha.getClanId()) && cha.getClan().isAtWarWith(getClanId()) && getWantsPeace() == 0 && cha.getWantsPeace() == 0
						&& !isAcademyMember())
					return true;
			}
		}
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		{
			boolean isCastle = (((L2DoorInstance) target).getCastle() != null && ((L2DoorInstance) target).getCastle().getCastleId() > 0 && ((L2DoorInstance) target).getCastle().getSiege()
					.getIsInProgress());
			boolean isFort = (((L2DoorInstance) target).getFort() != null && ((L2DoorInstance) target).getFort().getFortId() > 0 && ((L2DoorInstance) target).getFort().getSiege().getIsInProgress() && !((L2DoorInstance) target)
					.getIsCommanderDoor());
		...
		if ((skill.getId() == 13 || skill.getId() == 299 || skill.getId() == 448)
				&& ((!SiegeManager.getInstance().checkIfOkToSummon(this, false) && !FortSiegeManager.getInstance().checkIfOkToSummon(this, false)) || (SevenSigns.getInstance()
						.checkSummonConditions(this))))
			return false;
		...
		if (skill.isOffensive())
		{
			...
			if (target.getActingPlayer() != null && getSiegeState() > 0 && isInsideZone(L2Character.ZONE_SIEGE) && target.getActingPlayer().getSiegeState() == getSiegeState()
	 ...
	 * <li>Inventory contains item <li>Item owner id == this.owner id <li>It isnt pet control item while mounting pet or
	 * pet summoned <li>It isnt active enchant item <li>It isnt cursed weapon/item <li>It isnt wear item <br>
	 * 
	 * @param objectId
	 *            : item object id
	 * @param action
	...
	public void startLookingForFishTask()
	{
		if (!isDead() && _taskforfish == null)
		{
			...
			if (_lure != null)
			{
				...
				else if (lureid == 6520 || lureid == 6523 || lureid == 6526 || (lureid >= 8505 && lureid <= 8513) || (lureid >= 7610 && lureid <= 7613) || (lureid >= 7807 && lureid <= 7809)
			...
			_taskforfish = ThreadPoolManager.getInstance().scheduleEffectAtFixedRate(new LookingForFishTask(_fish.getWaitTime(), _fish.getFishGuts(), _fish.getType(), isNoob, isUpperGrade), 10000,
					checkDelay);
		}
	}
	...
	{
		if ((getKarma() > 0 || Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE) && !(killer instanceof L2PcInstance) && !(this.isGM()) && !(this.getCharmOfLuck() && killer.isRaid())
				&& !isPhoenixBlessed() && !isLucky() && !(TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) && !(DMEvent.isStarted() && DMEvent.isPlayerParticipant(getObjectId()))
				&& !(LMEvent.isStarted() && LMEvent.isPlayerParticipant(getObjectId())) && !(this.isInsideZone(L2Character.ZONE_PVP) || this.isInsideZone(L2Character.ZONE_SIEGE)))
	...
	public boolean teleportBookmarkCondition(int type)
	{
		...
		else if (this.isInWater())
		{
			...
			return false;
				...
				&& (this.isInsideZone(ZONE_SIEGE) || this.isInsideZone(ZONE_CLANHALL) || this.isInsideZone(ZONE_JAIL) || this.isInsideZone(ZONE_CASTLE) || this.isInsideZone(ZONE_NOSUMMONFRIEND) || this
	...
	/**
	 * Return the _createDate of the L2PcInstance.<BR>
	 * <BR>
	 */
	...
	public int checkBirthDay()
	{
		...
		if (now.get(Calendar.MONTH) == _createDate.get(Calendar.MONTH) && now.get(Calendar.DAY_OF_MONTH) == _createDate.get(Calendar.DAY_OF_MONTH)
	...
	public boolean canMakeSocialAction()
	{
		if (getPrivateStoreType() == 0 && getActiveRequester() == null && !isAlikeDead() && (!isAllSkillsDisabled() || isInDuel()) && !isCastingNow() && !isCastingSimultaneouslyNow()
				&& getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE && !AttackStanceTaskManager.getInstance().getAttackStanceTask(this) && !isInOlympiadMode())
		{
			return true;
		}
		else
