...
import com.l2js.gameserver.datatables.GMSkillTable;
import com.l2js.gameserver.datatables.HeroSkillTable;
...
import com.l2js.gameserver.model.actor.instance.L2ArtefactInstance;
import com.l2js.gameserver.model.actor.instance.L2ChestInstance;
...
import com.l2js.gameserver.skills.BaseStats;
import com.l2js.gameserver.skills.Env;
import com.l2js.gameserver.skills.Formulas;
import com.l2js.gameserver.skills.Stats;
import com.l2js.gameserver.skills.conditions.Condition;
import com.l2js.gameserver.skills.funcs.Func;
import com.l2js.gameserver.skills.funcs.FuncTemplate;
import com.l2js.gameserver.templates.StatsSet;
...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	private final int _cancelDebuffRate;
	...
	private final Map<String, Byte> _negateAbnormals; // lists the effect abnormal types with order below the presented
														// that are canceled
	...
	private final int _baseCritRate;  // percent of success for skill critical hit (especially for PDAM & BLOW - they're
										// not affected by rCrit values or buffs). Default loads -1 for all other skills
										// but 0 to PDAM & BLOW
	private final int _lethalEffect1;     // percent of success for lethal 1st effect (hit cp to 1 or if mob hp to 50%)
										// (only for PDAM skills)
	private final int _lethalEffect2;     // percent of success for lethal 2nd effect (hit cp,hp to 1 or if mob hp to 1)
										// (only for PDAM skills)
	private final boolean _directHpDmg;  // If true then dmg is being make directly
	private final boolean _isTriggeredSkill;      // If true the skill will take activation buff slot instead of a normal
												// buff slot
	...
	protected L2Skill(StatsSet set)
	{
		...
		_cancelDebuffRate = set.getInteger("cancelDebuffRate", 100);
		
		_abnormalLvl = set.getInteger("abnormalLvl", -1);
		_effectAbnormalLvl = set.getInteger("effectAbnormalLvl", -1); // support for a separate effect abnormal lvl,
																		// e.g. poison inside a different skill
		...
		if (negateAbnormals != null && negateAbnormals != "")
		{
			...
			for (String ngtStack : negateAbnormals.split(";"))
			{
				...
				else if (ngt.length == 2) // Both abnormalType and abnormalLvl are present
				{
					...
					throw new IllegalArgumentException("SkillId: " + _id + ": Incorrect negate Abnormals for " + ngtStack
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst, L2Character target)
	{
		...
		// (ex : ONE, SELF, HOLY, PET, AURA, AURA_CLOSE, AREA, MULTIFACE, PARTY, CLAN, CORPSE_PLAYER, CORPSE_MOB,
		// CORPSE_CLAN, UNLOCKABLE, ITEM, UNDEAD)
		...
		// The skill can only be used on the L2Character targeted, or on the caster itself
			...
			case TARGET_PARTY_MEMBER:
			{
				if ((target != null && target == activeChar)
						|| (target != null && activeChar.isInParty() && target.isInParty() && activeChar.getParty().getPartyLeaderOID() == target.getParty().getPartyLeaderOID())
					...
					if (player != null && (targetPlayer != null || targetPet != null))
					{
						...
						if (getSkillType() == L2SkillType.RESURRECT)
						{
							if (targetPlayer != null)
							{
								...
								if (targetPlayer.isFestivalParticipant()) // Check to see if the current player target
																			// is in a festival.
								{
									condGood = false;
								...
								if (targetPlayer.isReviveRequested())
								{
									if (targetPlayer.isRevivingPet())
										player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.MASTER_CANNOT_RES)); // While
																																// a
																																// pet
																																// is
																																// attempting
										...
										player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.RES_HAS_ALREADY_BEEN_PROPOSED)); // Resurrection
																																			// is
																																			// already
																																			// been
																																			// proposed.
								...
								if (targetPet.getOwner() != player)
								{
									if (targetPet.getOwner().isReviveRequested())
									{
										if (targetPet.getOwner().isRevivingPet())
											...
											player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANNOT_RES_PET2)); // A
																																// pet
																																// cannot
																																// be
																																// resurrected
	...
	// L2jS ADD Custom
	public final int getCancelDebuffRate()
	{
		return _cancelDebuffRate;
	}
}
