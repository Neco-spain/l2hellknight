...
import com.l2js.gameserver.skills.funcs.Lambda;
...
public abstract class L2Effect
{
	 ...
	 * remove it and update client magic icon</li><BR>
	 * <BR>
	 * 
	 */
	...
	{
		if (!effectCanBeStolen() || this.getEffectType() == L2EffectType.TRANSFORMATION || this.getSkill().isPassive() || this.getSkill().isToggle() || this.getSkill().isDebuff()
				|| this.getSkill().isHeroSkill() || this.getSkill().isGMSkill() || (this.getSkill().isPotion() && (this.getSkill().getId() != 2274 && this.getSkill().getId() != 2341)) // Hardcode
																																														// for
																																														// now
																																														// :<
				|| this.isHerbEffect() || !this.getSkill().canBeDispeled())
			return false;
		return true;
	}
