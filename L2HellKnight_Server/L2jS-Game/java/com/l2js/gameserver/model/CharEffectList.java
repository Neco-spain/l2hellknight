...
import com.l2js.gameserver.network.serverpackets.AbnormalStatusUpdate;
import com.l2js.gameserver.network.serverpackets.ExOlympiadSpelledInfo;
import com.l2js.gameserver.network.serverpackets.PartySpelled;
...
public class CharEffectList
{
	...
	private FastList<L2Effect> _passives; // They bypass most of the actions, keep in mind that those arent included in
											// getAllEffects()
	...
	{
		if ((_buffs == null || _buffs.isEmpty()) || checkSkill._effectTemplates == null || checkSkill._effectTemplates.length < 1 || checkSkill._effectTemplates[0].abnormalType == null
	...
	public final void stopSkillEffects(L2SkillType skillType, int negateLvl)
	{
		if (_buffs != null && !_buffs.isEmpty())
		{
			...
			{
				if (e != null
						&& (e.getSkill().getSkillType() == skillType || (e.getSkill().getEffectType() != null && e.getSkill().getEffectType() == skillType))
						&& (negateLvl == -1 || (e.getSkill().getEffectType() != null && e.getSkill().getEffectAbnormalLvl() >= 0 && e.getSkill().getEffectAbnormalLvl() <= negateLvl) || (e.getSkill()
								.getAbnormalLvl() >= 0 && e.getSkill().getAbnormalLvl() <= negateLvl)))
					e.exit();
			}
		}
		if (_debuffs != null && !_debuffs.isEmpty())
	...
	protected void addEffectFromQueue(L2Effect newEffect)
	{
			...
			if ("none".equals(newEffect.getAbnormalType()))
			{
				...
				if (newEffect.setInUse(true))
				{
					for (L2Effect eff : _passives)
					{
						...
						// Check and remove if there is already such effect in order to prevent passive effects
						// overstack.
		...
		if (effectToRemove != effectToAdd)
		{
			...
			{
				// Remove all Func objects corresponding to this stacked effect from the Calculator set of the
				// L2Character
			...
			if (effectToAdd != null)
			{
				...
				if (effectToAdd.setInUse(true))
					// Add all Func objects corresponding to this stacked effect to the Calculator set of the
					// L2Character
