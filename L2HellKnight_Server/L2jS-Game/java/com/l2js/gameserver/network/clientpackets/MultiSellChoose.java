...
import com.l2js.gameserver.model.L2Augmentation;
...
import com.l2js.gameserver.model.multisell.Entry;
import com.l2js.gameserver.model.multisell.Ingredient;
		...
		for (Entry entry : list.getEntries())
		{
			if (entry.getEntryId() == _entryId)
			{
				...
				for (Ingredient e : entry.getIngredients())
				{
					...
					// whenever they need to be applied. Uniqueness of items is thus judged by item id AND enchantment
					// level
					...
					for (Ingredient e : entry.getIngredients())
					{
						...
						else
						{
							...
							if (e.getEnchantLevel() > 0)
							{
								...
								{
									if (i.getEnchantLevel() == e.getEnchantLvl())
									{
										itemToTake = i;
										break;
									}
								...
								itemToTake = inv.getItemByItemId(e.getItemId()); // initialize and initial guess for the
																					// item to take.
								
							itemToTake = inv.getItemByItemId(e.getItemId()); // initialize and initial guess for the
																				// item to take.
							if (itemToTake == null)
							{ // this is a cheat, transaction will be aborted and if any items already taken will not be
								// returned back to inventory!
								_log.severe("Character: " + player.getName() + " is trying to cheat in multisell, id:" + _listId + ":" + _entryId);
								player.setMultiSell(null);
								return;
							}
							...
							{
								// if it's a stackable item, just reduce the amount from the first (only) instance that
								...
								else
								{
									...
									// level
									{
										 ...
										 * given by:
										 * m*(2n-m+1)/2 where m is the number of items to be exchanged and n is the
										 * total
										 * number of inventory items that have a matching id.
										 * With algorithm 2 (sort), sorting takes n*log(n) time and the choice is done
										 * in a single cycle
										 * for case b (just grab the m first items) or in linear time for case a (find
										 ...
										 * (2nlogn-logn*logn+logn)/2 = nlog(n) - log(n*n) + log(n) = nlog(n) +
										 * log(n/n*n) =
										 * nlog(n) + log(1/n) = nlog(n) - log(n) = (n-1)log(n)
										 * So for m < log(n) then m*(2n-m+1)/2 > (n-1)log(n) and m*(2n-m+1)/2 > nlog(n)
										 * IDEALLY:
										 * In order to best optimize the performance, choose which algorithm to run,
										 * based on whether 2^m > n
										 ...
										 * things simple.
										 * If, in the future, it becomes necessary that we optimize, the above
										 * discussion should make it clear
										 * what optimization exactly is necessary (based on the comments under
										 * "IDEALLY").
										 */
										...
										for (int i = 1; i <= (e.getItemCount() * _amount); i++)
										{
											...
											if (itemToTake.getEnchantLevel() > 0)
											{
												for (L2ItemInstance item : inventoryContents)
												{
													...
													{
														itemToTake = item;
														// nothing will have enchantment less than 0. If a
														// zero-enchanted
