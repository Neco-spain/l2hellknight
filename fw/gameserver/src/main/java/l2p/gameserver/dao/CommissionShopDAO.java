...
import l2p.gameserver.templates.item.ExItemType;
...
public class CommissionShopDAO {
    ...
    public void saveNewItem(int objectId, int seller_id, String item_name, long price, ExItemType exItemType,
                            int sale_days, long sale_end_time, String player_name) {
        Connection con = null;
    ...
    public Queue<List<CommissionItemInfo>> getRegisteredItems(ExItemType[] types, int rareType, int grade, String searchName) {
        ...
        try {
            ...
            while (rset.next() && i <= 999) {
                ...
                if ((item = container.getItemByObjectId(objectId)) == null)
                    continue;
                if (rareType == 1 && !item.getTemplate().isBlessed()
                        || rareType == 0 && item.getTemplate().isBlessed()) {
                    continue;
                }
                ...
                if (!searchName.isEmpty()
                        && !rset.getString("item_name").toLowerCase().contains(searchName.toLowerCase())) {
                    continue;
                }
                CommissionItemInfo itemInfo = new CommissionItemInfo(item);
                itemInfo.setAuctionId(rset.getLong("auction_id"));
