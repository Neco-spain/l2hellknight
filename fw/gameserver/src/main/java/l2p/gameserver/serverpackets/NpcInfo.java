...
public class NpcInfo extends L2GameServerPacket {
    ...
    private boolean _showName, _showTitle;
    private int _state;
    ...
    public NpcInfo(NpcInstance cha, Creature attacker) {
        ...
        _showTitle = cha.isShowTitle();
        if (_showName && (Config.SERVER_SIDE_NPC_NAME || cha.getTemplate().displayId != 0 || !cha.getName().equals(cha.getTemplate().name)))
            _name = cha.getName();
        if (_showTitle && (Config.SERVER_SIDE_NPC_TITLE || cha.getTemplate().displayId != 0 || !cha.getTitle().equals(cha.getTemplate().title)))
            _title = cha.getTitle();
    ...
    protected final void writeImpl() {
        ...
        writeC(_showTitle ? 0x01 : 0x00); // show title
