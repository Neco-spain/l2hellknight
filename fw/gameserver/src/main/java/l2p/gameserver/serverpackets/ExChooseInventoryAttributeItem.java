...
public class ExChooseInventoryAttributeItem extends L2GameServerPacket {
    ...
    private final List<Integer> attributable_items;

    public ExChooseInventoryAttributeItem(List<Integer> attributable_items, ItemInstance item) {
        this.attributable_items = attributable_items;
    ...
    protected final void writeImpl() {
        ...
        writeD(attributable_items.size()); //equipable items count
        for (int itemObjId : attributable_items) {
            writeD(itemObjId);
        }
    }
}