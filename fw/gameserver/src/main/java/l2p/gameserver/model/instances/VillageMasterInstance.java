        ...
        if (command.startsWith("create_ally") && command.length() > 12) {
            ...
            switch (intVal) {
                ...
                case 5: // Смена саба на другой из уже взятых - обработка выбора из case 2
                    ...
                    _class = player.getActiveClass().toString();
                    player.setActiveSubClass(classId, true);
                    _newClass = player.getActiveClass().toString();
                    ...
                    player.sendPacket(new SystemMessage2(SystemMsg.YOU_HAVE_SUCCESSFULLY_SWITCHED_S1_TO_S2).addString(_class).addString(_newClass));
                    // completed.
                    break;
            ...
            try {
                newClassId = Integer.parseInt(command.substring(12, command.length()));
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            player.getQuestState("_10360_CertificationofFate").exitCurrentQuest(false);
            content.append("Congratulations!.<br>You have successfully change your class.");
