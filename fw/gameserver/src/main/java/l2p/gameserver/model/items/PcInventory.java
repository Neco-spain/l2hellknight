...
public class PcInventory extends Inventory {
    ...
    private static final int[] arrowQuiver = {
            32249, // NG
            32250, // D
            32251, // C
            32252, // B
    ...
    public ItemInstance findArrowForBow(ItemTemplate bow) {
        int findForGrade = bow.getCrystalType().externalOrdinal;
        int quiverId = arrowQuiver[findForGrade];
        ItemInstance ret = null;
        if ((ret = getItemByItemId(quiverId)) != null)
           return ret;
        if (findForGrade != 6)
        {
            int[] arrowsId = arrows[findForGrade];
            for (int id : arrowsId)
    ...
    private static final int[] boltsQuiver = {
            32256, // NG
            32257, // D
            32258, // C
            32259, // B
    ...
    public boolean isQuiverInLeftHand() {
        return (ArrayUtils.contains(arrowQuiver, getPaperdollItemId(Inventory.PAPERDOLL_LHAND)) || ArrayUtils.contains(boltsQuiver, getPaperdollItemId(Inventory.PAPERDOLL_LHAND)));
    }

    public ItemInstance findArrowForCrossbow(ItemTemplate xbow) {
        int findForGrade = xbow.getCrystalType().externalOrdinal;
        int boltQuiverId = boltsQuiver[findForGrade];
        ItemInstance ret = null;
        if ((ret = getItemByItemId(boltQuiverId)) != null)
                return ret;

        if (findForGrade != 6)
        {
            int[] boltsId = bolts[findForGrade];
            for (int id : boltsId)
                if ((ret = getItemByItemId(id)) != null)
                    return ret;
        }
