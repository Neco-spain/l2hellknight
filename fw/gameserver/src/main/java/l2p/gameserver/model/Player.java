...
import static l2p.gameserver.serverpackets.ExSetCompassZoneCode.*;
...
public final class Player extends Playable implements PlayerGroup {
    ...
    /**
     * The current higher Expertise of the L2Player (None=0, D=1, C=2, B=3, A=4, S=5, S80=6, S84=7, R=7, R95=8, R99=10)
     */
    ...
    private int jumpLocId;

    private SummonList _summonList = new SummonList(this);
    ...
    public void setJumpLocId(int jumpLocId) {
        this.jumpLocId = jumpLocId;
    }

    public int getJumpLocId() {
        return jumpLocId;
    }
    ...
    public void changeClass(Player player, int index) {
        Connection con = null;
        PreparedStatement statement = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement((new StringBuilder()).append("SELECT class_id FROM character_subclasses WHERE `index`=").append(index).append(" AND char_obj_id=").append(objectId).toString());
            String _class;
            String _newClass;
            for (ResultSet rset = statement.executeQuery(); rset.next(); sendPacket(new SystemMessage2(SystemMsg.YOU_HAVE_SUCCESSFULLY_SWITCHED_S1_TO_S2).addString(_class).addString(_newClass))) {
                _class = getActiveClass().toString();
                int class_id = rset.getInt("class_id");
                setActiveSubClass(class_id, true);
                _newClass = getActiveClass().toString();
                Skill skill = SkillTable.getInstance().getInfo(1570, 1);
                EffectTemplate et[] = skill.getEffectTemplates();
                int etl = et.length;
                for (int i = 0; i < etl; i++) {
                    EffectTemplate et1 = et[i];
                    Env env = new Env(player, player, skill);
                    Effect effect = et1.getEffect(env);
                    effect.setPeriod(skill.getReuseDelay());
    ...
    public int getSubIndex() {
        ...
        try {
            con = DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement((new StringBuilder()).append("SELECT `index` FROM character_subclasses WHERE char_obj_id=").append(objectId).toString());
            for (ResultSet rset = statement.executeQuery(); rset.next(); )
                subIndex = rset.getInt("index");
    ...
    public synchronized void setClassId(final int id, boolean noban, boolean fromQuest) {
        ...
        if (!getSubClasses().containsKey(id)) {
            ...
            if (fromQuest) {
                ...
                broadcastPacket(new MagicSkillUse(this, this, 0, 5103, 1, 1000, 0, -1));
    ...
    private void rewardSkills(boolean send) {
        ...
        if (Config.AUTO_LEARN_SKILLS) {
            ...
            for (SkillLearn skill : SkillAcquireHolder.getInstance().getAvailableSkills(this, AcquireType.NORMAL, false)) {
                if (skill.getCost() == 0 && skill.getRequiredItems().isEmpty()) {
    ...
    public void broadcastStatusUpdate() {
        if (!needStatusUpdate()) //По идее еще должно срезать траффик. Будут глюки с отображением - убрать это условие.
            return;
    ...
    public List<L2GameServerPacket> addPacketList(Player forPlayer, Creature dropper) {
        ...
        if (isCastingNow()) {
            ...
            if (castingSkill != null && castingTarget != null && castingTarget.isCreature() && getAnimationEndTime() > 0)
                list.add(new MagicSkillUse(this, castingTarget, 0, castingSkill.getId(), castingSkill.getLevel(), (int) (animationEndTime - System.currentTimeMillis()), 0, castingSkill.getReuseGroupId()));
        }
        ...
        if (levels > 0) {
            ...
            if (mentorId != 0) {
                ...
                if (mentorPlayer != null) {
                    // выдача наставнику Sign of Tutor за лвлап ученика
    ...
    public void reduceArrowCount() {
        ...
        if (!getInventory().isQuiverInLeftHand())
    ...
    public static Player restore(final int objectId) {
        ...
        try {
            ...
            rset4 = statement4.executeQuery("SELECT `race` FROM `character_subclasses` WHERE `char_obj_id`=" + objectId + " LIMIT 1");

            if (rset.next() && rset2.next() && rset4.next()) {
                //final int baseClassId = rset.getInt("baseClass_id");
                final int classId = rset2.getInt("class_id");
                final boolean female = rset.getInt("sex") == 1;
                int RaceId = rset4.getInt("race");
            ...
            try {
                ...
                statement = con.prepareStatement(//
                        "UPDATE characters SET face=?,hairStyle=?,hairColor=?,sex=?,x=?,y=?,z=?" + //
                                ...
                                "onlinetime=?,pledge_type=?,pledge_rank=?,lvl_joined_academy=?,apprentice=?,key_bindings=?,pcBangPoints=?,char_name=?,fame=?,bookmarks=? WHERE obj_Id=? LIMIT 1");
    ...
    public void setNoble(boolean noble) {
        if (noble) //broadcast skill animation: Presentation - Attain Noblesse
            broadcastPacket(new MagicSkillUse(this, this, 0, 6673, 1, 1000, 0, -1));
        _noble = noble;
    }
    ...
    public boolean addSubClass(final int classId, boolean storeOld, int certification) {
        ...
        try {
            ...
            statement.setInt(16, getSubIndex());
            statement.setInt(17, getBaseRaceId());
            statement.execute();
        } catch (final Exception e) {
