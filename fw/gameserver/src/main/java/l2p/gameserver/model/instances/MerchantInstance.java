...
import l2p.gameserver.serverpackets.ExGetPremiumItemList;
...
public class MerchantInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (actualCommand.equalsIgnoreCase("Buy") || actualCommand.equalsIgnoreCase("Sell")) {
            ...
            if (player.getInventory().getCountOf(33800) == 0 && player.getLevel() < 86)
            {
                player.getInventory().addItem(33800,1);
                player.sendPacket(SystemMessage2.obtainItems(33800,1,0));  // Cain проверить надо ли
            }
            else
            ...
            {
                player.getInventory().addItem(33805,40);
                player.getInventory().removeItemByItemId(33800,1);
            }
            else
                player.sendPacket(new SystemMessage2(SystemMsg.YOU_ARE_NO_PRIORITY_RIGHTS_ON_A_SWEEPER));
        } else
