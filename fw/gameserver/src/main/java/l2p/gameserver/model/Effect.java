...
public abstract class Effect extends RunnableImpl implements Comparable<Effect>, FuncOwner {
    ...
    public void addSpelledIcon(ExAbnormalStatusUpdateFromTarget ps) {
        if (!isActive() || isHidden())
            return;
        int duration = _skill.isToggle() ? AbnormalStatusUpdate.INFINITIVE_EFFECT : getTimeLeft();
        ps.addSpelledEffect(_displayId, _displayLevel, getTemplate()._alterData, duration, getEffector().getObjectId());
    }
