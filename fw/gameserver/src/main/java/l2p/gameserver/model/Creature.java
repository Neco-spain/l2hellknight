...
public abstract class Creature extends GameObject {
    ...
    public void altUseSkill(Skill skill, Creature target) {
        ...
        //TODO: Возможно следующую строку можно убрать совсем. На оффе MagicSkillUse отсылается для Toggle скилов
        //if (!skill.isToggle())
        broadcastPacket(new MagicSkillUse(this, target, 0, skill.getDisplayId(), level, skill.getHitTime(), reuseDelay, skill.getReuseGroupId()));
        ...
        if (!skill.isHideUseMessage())
            ...
            else if (skill.getItemConsumeId()[0] > 0)
        ...
        if (Rnd.chance(trigger.getChance()) && trigger.checkCondition(this, realTarget, aimTarget, owner, damage) && skill.checkCondition(this, aimTarget, false, true, true)) {
            ...
            if (trigger.getType() != TriggerType.SUPPORT_MAGICAL_SKILL_USE)
                for (Creature cha : targets)
                    broadcastPacket(new MagicSkillUse(this, cha, 0, displayId, displayLevel, 0, 0, skill.getReuseGroupId()));
    ...
    public void doCast(Skill skill, Creature target, boolean forceUse) {
        ...
        broadcastPacket(new MagicSkillUse(this, target, 0, skill.getDisplayId(), level, skillTime, reuseDelay, skill.getReuseGroupId()));
    ...
    public int getMEvasionRate(final Creature target) {
        return (int) (calcStat(Stats.MEVASION_RATE, 0, target, null));
    }

    public int getMAccuracy(final Creature target) {
    ...
    public void updateEffectIcons() {
        ...
        ExAbnormalStatusUpdateFromTarget packet = new ExAbnormalStatusUpdateFromTarget(this);
        for (Effect effect : effects) {
            if (effect.isInUse()) {
                effect.addSpelledIcon(packet);
            }
        }
