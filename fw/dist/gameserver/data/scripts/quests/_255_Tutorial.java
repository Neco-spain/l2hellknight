...
public class _255_Tutorial extends Quest implements ScriptFile, OnPlayerEnterListener, OnLevelChangeListener {
    ...
    public void onPlayerEnter(Player player) {
        ...
        else if (player.getLevel() >= 85 && player.getActiveClass().isBase() && player.getClassId().getLevel() == 4) {
            int _cId = 139;
            for (; _cId <= 146; _cId++) {
                if (ClassId.VALUES[_cId].childOf(player.getClassId()))
                    break;
            }
            if (!player.getVarB("GermunkusUSM")) {
                player.sendPacket(new ExCallToChangeClass(_cId, true),
                        new ExShowScreenMessage(NpcString.FREE_THE_GIANT_FROM_HIS_IMPRISONMENT_AND_AWAKEN_YOUR_TRUE_POWER,
    ...
    public void onLevelChange(Player player, int oldLvl, int newLvl) {
        ...
        if (player.getLevel() >= 85 && player.getActiveClass().isBase() && player.getClassId().getLevel() == 4) {
            int _cId = 139;
            for (; _cId <= 146; _cId++) {
                if (ClassId.VALUES[_cId].childOf(player.getClassId()))
                    break;
            ...
            if (!player.getVarB("GermunkusUSM")) {
                player.sendPacket(new ExCallToChangeClass(_cId, false),
                        new ExShowScreenMessage(NpcString.FREE_THE_GIANT_FROM_HIS_IMPRISONMENT_AND_AWAKEN_YOUR_TRUE_POWER,
                                10000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, ExShowScreenMessage.STRING_TYPE,
                                -1, false, 1));
            }
