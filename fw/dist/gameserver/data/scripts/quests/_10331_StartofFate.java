...
public class _10331_StartofFate extends Quest implements ScriptFile {
    ...
    /* TODO: Нужно решить следующие проблемы:
      * - У генератора нет свечения и эффект Current Shock должен применяться с этой пассивкой свечения
      * - При завершении квеста отобразить магазин с товарами за Proof of Courage
      */
    ...
    public String onEvent(String event, QuestState qs, NpcInstance npc) {
        ...
        // Opening last door
            ...
            qs.getPlayer().sendPacket(new ExShowScreenMessage(NpcString.MARK_OF_BELIS_CAN_BE_ACQUIRED_FROM_ENEMIES,
                    2000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER));
            ...
            if (player.isMageClass() && player.getRace() != Race.kamael) {
                ...
                if (player.getClassId() == ClassId.mage) {
                    ...
                    if (prof.equals("elven_wizard"))
                        ...
                        player.setClassId(ClassId.elvenWizard.ordinal(), false, true);
                    ...
                    if (prof.equals("dark_wizard"))
                        player.setClassId(ClassId.darkWizard.ordinal(), false, true);
                    else if (prof.equals("shillien_oracle"))
                        player.setClassId(ClassId.shillienOracle.ordinal(), false, true);
                } else if (player.getClassId() == ClassId.orcMage) {
                    if (prof.equals("shaman"))
                        player.setClassId(ClassId.orcShaman.ordinal(), false, true);
                }
                ...
                if (player.getClassId() == ClassId.fighter) {
                    ...
                    if (prof.equals("elven_knight"))
                        player.setClassId(ClassId.elvenKnight.ordinal(), false, true);
                    else if (prof.equals("elven_scout"))
                        player.setClassId(ClassId.elvenScout.ordinal(), false, true);
                } else if (player.getClassId() == ClassId.darkFighter) {
                    if (prof.equals("palus_knight"))
                        player.setClassId(ClassId.palusKnight.ordinal(), false, true);
                    else if (prof.equals("assassin"))
                        player.setClassId(ClassId.assassin.ordinal(), false, true);
                } else if (player.getClassId() == ClassId.orcFighter) {
                    if (prof.equals("raider"))
                        player.setClassId(ClassId.orcRaider.ordinal(), false, true);
                    else if (prof.equals("monk"))
                        player.setClassId(ClassId.orcMonk.ordinal(), false, true);
        ...
        switch (npcId) {
            ...
            case VALFAR:
                if (cond == 0) {
                    ...
                    if ((race == Race.human && npcId != FRANCO) ||
                            (race == Race.elf && npcId != RIVIAN) ||
                            (race == Race.darkelf && npcId != DEVON) ||
                            (race == Race.orc && npcId != TOOK) ||
                            (race == Race.dwarf && npcId != MOKA) ||
                            (race == Race.kamael && npcId != VALFAR))
                        break;
