...
public class MobsAI extends Fighter {
    ...
    protected void onEvtAttacked(Creature attacker, int damage) {
        ...
        if (ROOM_ID == 2) {
            if (IS_LAST_GROUP && getActor().getCurrentHpPercents() < 80
                    && getActor().getEffectList().getEffectsBySkillId(ULTIMATE_BUFF_ID) == null) {
    ...
    protected void onEvtScriptEvent(String event, Object arg1, Object arg2) {
        ...
        } else if (event.equalsIgnoreCase("SELECT_ME")) {/* Заускается только для ROOM_ID == 1 */
            if (ROOM_ID == 1)
                selectMe();
    ...
    protected void onEvtDead(Creature killer) {
        ...
        if (ROOM_ID == 1) { /* Заускается только для ROOM_ID == 1 */
