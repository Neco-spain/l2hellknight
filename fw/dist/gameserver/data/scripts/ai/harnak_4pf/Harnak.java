...
public class Harnak extends Fighter {
    ...
    protected void onEvtAttacked(Creature attacker, int damage) {
        ...
        if (!firstMsg) {
            firstMsg = true;
            getActor().broadcastPacket(new ExShowScreenMessage(NpcString.FREE_ME_FROM_THIS_BINDING_OF_LIGHT,
            ...
            secondMsg = true;
            getActor().broadcastPacket(new ExShowScreenMessage(NpcString.DESTROY_THE_GHOST_OF_HARNAK_THIS_CORRUPTED_CREATURE,
                    10000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, ExShowScreenMessage.STRING_TYPE,
                    0, false, 0));
        } else if (!thirdMsg && getActor().getCurrentHpPercents() <= 60.0) {
            thirdMsg = true;
            getActor().broadcastPacket(new ExShowScreenMessage(NpcString.FREE_ME_AND_I_PROMISE_YOU_THE_POWER_OF_GIANTS,
                    10000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, ExShowScreenMessage.STRING_TYPE,
                    0, false, 0));
